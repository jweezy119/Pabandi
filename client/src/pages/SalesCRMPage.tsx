...
export const SalesCRMPage: React.FC = () => {
  ...
  const [tab, setTab] = useState<'contacts' | 'deals' | 'pipeline'>('contacts');
  ...
  const load = async () => {
    ...
    try {
      const [contactsRes, dealsRes, pipelineRes] = await Promise.all([
        api.get('/crm/contacts').catch(() => ({ data: { data: [] } })),
        api.get('/crm/deals').catch(() => ({ data: { data: [] } })),
        api.get('/crm/pipeline').catch(() => ({ data: { data: { pipeline: [] } } })),
      ]);
  ...
  const addDeal = async () => {
    ...
    try {
      await api.post('/crm/deals', { ...dealForm, value: Number(dealForm.value) || 0 });
    ...
    <div className="min-h-screen pb-24 md:pb-0 mt-16" style={{ background: tokens.color.background }}>
      <div className="max-w-6xl mx-auto px-4 py-6 md:py-10">
        <div className="mb-6">
          ...
          <p className="text-sm mt-1" style={{ color: tokens.color.textDim }}>Manage contacts, deals, and your sales pipeline.</p>
        </div>

        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {(['contacts', 'deals', 'pipeline'] as const).map(s => (
            <button key={s} onClick={() => setTab(s)}
              ...
              {s === 'contacts' ? '👥 Contacts' : s === 'deals' ? '💼 Deals' : '📊 Pipeline'}
            </button>
          ))}
        </div>
