    ...
    <div className="max-w-3xl mx-auto p-6 animate-fade-in pb-20 font-body" style={{ background: tokens.color.background, color: tokens.color.text }}>
      ...
      <div className="grid grid-cols-3 gap-4 mt-6">
        <div className="rounded-2xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <p className="text-xs" style={{ color: tokens.color.textDim }}>Total paid to you</p>
          <p className="text-2xl font-black text-green-400">${d.totalNetUsdc}</p>
        </div>
        <div className="rounded-2xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <p className="text-xs" style={{ color: tokens.color.textDim }}>Payouts</p>
          <p className="text-2xl font-black">{d.count}</p>
        </div>
        <div className="rounded-2xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <p className="text-xs" style={{ color: tokens.color.textDim }}>Saved vs 7% remittance</p>
      ...
      <div className="mt-8">
        ...
        {!d.payouts || d.payouts.length === 0 ? (
          <p className="text-sm" style={{ color: tokens.color.textDim }}>No milestone payouts yet. When a client releases a draw, it lands here instantly.</p>
        ) : (
              ...
              <div key={p.id} className="rounded-xl p-3 text-sm" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div className="flex flex-wrap gap-2 justify-between items-center sm:flex-nowrap">
                  <div>
                    ...
                    <span className="ml-2 text-xs" style={{ color: tokens.color.textDim }}>fee ${p.feeUsdc} ({p.settlementBps}bps)</span>
                  </div>
                  <div className="text-right">
                    <span className="font-bold text-green-400">+${p.netUsdc}</span>
                    <div className="text-xs" style={{ color: tokens.color.textDim }}>{new Date(p.createdAt).toLocaleDateString()}</div>
                  </div>
                </div>
                {p.milestoneId && (
