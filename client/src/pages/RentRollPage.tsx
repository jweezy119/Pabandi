    ...
    <div className="min-h-screen" style={{ background: tokens.color.background }}>
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-[var(--warm-ink)]">📊 Rent Roll</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          <Surface className="text-center">
            ...
            <div className="text-xl font-bold text-[var(--sage)]">${totalIncome.toLocaleString()}</div>
          </Surface>
          <Surface className="text-center">
            <div className="text-xs mb-1" style={{ color: tokens.color.textDim }}>Total Expenses</div>
            <div className="text-xl font-bold text-[var(--terracotta)]">${totalExpenses.toLocaleString()}</div>
          </Surface>
          <Surface className="text-center">
            <div className="text-xs mb-1" style={{ color: tokens.color.textDim }}>Net Operating Income</div>
            <div className="text-xl font-bold text-[var(--clay)]">${totalNOI.toLocaleString()}</div>
          </Surface>
          <Surface className="text-center">
            <div className="text-xs mb-1" style={{ color: tokens.color.textDim }}>Occupancy</div>
            <div className="text-xl font-bold text-[var(--warm-ink)]">{occupancyRate}%</div>
          ...
          <Surface className="text-center">
            ...
            <div className="text-xl font-bold text-[var(--warm-ink)]">{leases.filter(l => l.status === 'ACTIVE').length}</div>
          </Surface>
        </div>

        {/* Property Breakdown */}
        <h2 className="text-lg font-bold text-[var(--warm-ink)] mb-3">By Property</h2>
        {properties.length === 0 ? (
              ...
              return (
                <Surface key={p.id}>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="font-bold text-[var(--warm-ink)]">{p.title}</div>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="p-2 rounded-lg bg-[var(--warm-sand)]">
                      ...
                      <div className="font-bold text-[var(--sage)]">${(fin.income || 0).toLocaleString()}</div>
                    </div>
                    <div className="p-2 rounded-lg bg-[var(--warm-sand)]">
                      <div className="text-xs" style={{ color: tokens.color.textDim }}>Expenses</div>
                      <div className="font-bold text-[var(--terracotta)]">${(fin.expenses || 0).toLocaleString()}</div>
                    </div>
                    <div className="p-2 rounded-lg bg-[var(--warm-sand)]">
                      <div className="text-xs" style={{ color: tokens.color.textDim }}>NOI</div>
                      <div className="font-bold text-[var(--clay)]">${(fin.noi || 0).toLocaleString()}</div>
                    </div>
                  </div>
                  {propertyLeases.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-[rgba(191,179,163,0.15)]">
                      <div className="text-xs mb-2" style={{ color: tokens.color.textDim }}>Active Leases</div>
                      <div className="space-y-1">
                        {propertyLeases.map((l: any) => (
                          <div key={l.id} className="flex items-center justify-between text-xs">
                            <span className="text-[var(--warm-ink)]">{l.tenantName || l.tenantEmail}</span>
                            <span className="text-[var(--sage)]">${l.rentAmount}/{l.rentPeriod === 'MONTH' ? 'mo' : 'wk'}</span>
                          </div>
                        ))}
                      </div>
                    </div>
