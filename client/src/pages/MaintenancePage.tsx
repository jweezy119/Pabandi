    ...
    <div className="min-h-screen" style={{ background: 'var(--cream)' }}>
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-[var(--warm-ink)]">🔧 Maintenance</h1>
            <p className="text-sm" style={{ color: 'var(--soft-stone)' }}>Track requests, assign vendors, and manage costs</p>
          </div>
          <Link to="/property-manager" className="text-sm text-[var(--clay)] hover:text-[var(--clay)]">← Back to CRM</Link>
        </div>

        {err && <div className="mb-4 px-4 py-3 rounded-xl text-sm" style={{ background: tokens.color.danger + '15', color: 'var(--terracotta)' }}>{err}</div>}

        {/* Filter */}
        <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
          {statusOptions.map(s => (
            <button key={s} onClick={() => setFilter(s)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${filter === s ? 'bg-[var(--clay)]/15 text-[var(--clay)] border border-[var(--clay)]/30' : 'bg-[var(--warm-sand)] text-[var(--soft-stone)] border border-[rgba(191,179,163,0.3)]'}`}>
        ...
        {showForm && (
          <Surface className="mb-6">
            <h3 className="text-lg font-bold text-[var(--warm-ink)] mb-4">New Maintenance Request</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="text-xs font-semibold text-[var(--warm-ink)] mb-1 block">Title *</label>
              ...
              <div className="md:col-span-2">
                <label className="text-xs font-semibold text-[var(--warm-ink)] mb-1 block">Description</label>
              ...
              <div>
                <label className="text-xs font-semibold text-[var(--warm-ink)] mb-1 block">Priority</label>
              ...
              <div>
                <label className="text-xs font-semibold text-[var(--warm-ink)] mb-1 block">Vendor</label>
              ...
              <div>
                <label className="text-xs font-semibold text-[var(--warm-ink)] mb-1 block">Estimated Cost</label>
              ...
              <div>
                <label className="text-xs font-semibold text-[var(--warm-ink)] mb-1 block">Vendor Notes</label>
        ...
        {filteredRequests.length === 0 ? (
          <p className="text-center py-8" style={{ color: 'var(--soft-stone)' }}>No maintenance requests found.</p>
        ) : (
              ...
              return (
                <Surface key={r.id}>
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <div className="font-bold text-[var(--warm-ink)]">{r.title}</div>
                  ...
                  {r.description && <p className="text-sm mb-2" style={{ color: 'var(--soft-stone)' }}>{r.description}</p>}
                  <div className="flex items-center gap-3 text-xs" style={{ color: 'var(--soft-stone)' }}>
