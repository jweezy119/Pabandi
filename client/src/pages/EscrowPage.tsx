      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {([
            ...
            <button key={t.id} onClick={() => setTab(t.id as any)}
              className={`px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap transition-all ${tab === t.id ? 'bg-[var(--clay)]/20 text-[var(--terracotta)] border border-[var(--clay)]/30' : 'bg-[var(--cream)] text-[var(--soft-stone)] border border-[var(--soft-stone)]/30 hover:bg-[var(--warm-sand)]'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === 'how' && (
          <div className="space-y-6">
            <Surface>
              <h3 className="text-lg font-bold text-[var(--warm-ink)] mb-4">How Pabandi Escrow Protects You</h3>
              <div className="space-y-4">
                {[
                  ...
                  <div key={s.step} className="flex gap-4 p-4 rounded-xl bg-[var(--cream)]">
                    <div className="text-3xl">{s.icon}</div>
                    <div>
                      <div className="font-bold text-[var(--warm-ink)]">Step {s.step}: {s.title}</div>
            ...
            <Surface>
              <h3 className="text-lg font-bold text-[var(--warm-ink)] mb-4">Why Use Escrow?</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center">
                  <div className="text-3xl mb-2">🛡️</div>
                  <div className="font-bold text-[var(--warm-ink)]">No More Scams</div>
                  <p className="text-sm mt-1" style={{ color: tokens.color.textDim }}>Seller can't run away with your money</p>
                </div>
                <div className="text-center">
                  <div className="text-3xl mb-2">👁️</div>
                  <div className="font-bold text-[var(--warm-ink)]">Inspect Before Paying</div>
                  <p className="text-sm mt-1" style={{ color: tokens.color.textDim }}>Check the item before funds release</p>
                </div>
                <div className="text-center">
                  <div className="text-3xl mb-2">⚖️</div>
                  <div className="font-bold text-[var(--warm-ink)]">Fair Disputes</div>
...
const NewTransactionForm: React.FC = () => {
  ...
  if (submitted) {
    return (
      <Surface className="text-center">
        ...
        <h3 className="text-xl font-bold text-[var(--warm-ink)] mb-2">Escrow Created!</h3>
        <p className="text-sm" style={{ color: tokens.color.textDim }}>
          Transaction ID: <span className="font-mono text-[var(--terracotta)]">esc-{Date.now().toString(36)}</span>
        </p>
        ...
        <div className="mt-4 p-4 rounded-xl bg-[var(--cream)]">
          ...
          <div className="font-bold text-[var(--muted-ochre)]">⏳ Awaiting Funding</div>
        </div>
      </Surface>
    );
  }
  ...
  return (
    <Surface>
      <h3 className="text-lg font-bold text-[var(--warm-ink)] mb-4">New Secured Transaction</h3>
      <div className="space-y-4">
        <div>
          <label className="text-sm font-semibold text-[var(--warm-ink)] mb-2 block">Item Title *</label>
        ...
        <div>
          <label className="text-sm font-semibold text-[var(--warm-ink)] mb-2 block">Amount (USD) *</label>
        ...
        <div>
          <label className="text-sm font-semibold text-[var(--warm-ink)] mb-2 block">Counterparty Email</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-sm font-semibold text-[var(--warm-ink)] mb-2 block">SafeMeet Location</label>
          ...
          <div>
            <label className="text-sm font-semibold text-[var(--warm-ink)] mb-2 block">Meetup Date</label>
        ...
        <div>
          <label className="text-sm font-semibold text-[var(--warm-ink)] mb-2 block">Notes</label>
...
const ActiveTransactions: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-bold text-[var(--warm-ink)]">Active Transactions</h3>
      {mockTransactions.map((t) => (
        <Surface key={t.id}>
          <div className="flex items-center justify-between mb-2">
            <div className="font-semibold text-[var(--warm-ink)]">{t.item}</div>
          ...
          <div className="text-sm space-y-1" style={{ color: tokens.color.textDim }}>
            <div>ID: <span className="font-mono text-[var(--terracotta)]">{t.id}</span></div>
            <div>Amount: <span className="font-bold text-[var(--warm-ink)]">${t.amount}</span></div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-[var(--warm-ink)]">My Disputes</h3>
        <Button size="sm">+ File New Dispute</Button>
      </div>
      {mockDisputes.map((d) => (
        <Surface key={d.id}>
          <div className="flex items-center justify-between mb-2">
            <div className="font-semibold text-[var(--warm-ink)]">{d.item}</div>
            <Badge tone="warning">{d.status}</Badge>
          </div>
          <div className="text-sm space-y-1" style={{ color: tokens.color.textDim }}>
            <div>Dispute ID: <span className="font-mono text-[var(--terracotta)]">{d.id}</span></div>
            <div>Amount in dispute: <span className="font-bold text-[var(--warm-ink)]">${d.amount}</span></div>
            <div>Against: {d.against}</div>
            <div>Filed: {d.filed}</div>
          </div>
          <div className="mt-4 p-3 rounded-xl bg-[var(--muted-ochre)]/10 border border-[var(--muted-ochre)]/20">
            <p className="text-xs text-[var(--muted-ochre)]">⏳ Awaiting community jury review. 3 of 5 jurors have voted.</p>
          </div>
        </Surface>
      ))}
    </div>
