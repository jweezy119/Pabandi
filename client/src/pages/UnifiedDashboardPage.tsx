...
export const UnifiedDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      setWidgets([
        { id: '1', title: 'Properties', value: 12, change: '+2 this month', icon: '🏠', color: 'var(--clay)', link: '/property-manager' },
        { id: '2', title: 'Active Listings', value: 5, change: '+1 this week', icon: '📋', color: 'var(--sage)', link: '/marketplace' },
        { id: '3', title: 'PAB Balance', value: '2,500', change: '+125 today', icon: '💰', color: 'var(--muted-ochre)', link: '/token' },
        ...
        { id: '6', title: 'Pending Apps', value: 8, change: '2 need review', icon: '📝', color: 'var(--terracotta)', link: '/applications' },
      ]);
      setActivities([
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-6">
          {quickActions.map((action) => (
            <Link key={action.label} to={action.link}>
              <Surface className="p-3 text-center hover:bg-[var(--warm-sand)] transition-all cursor-pointer">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
            {widgets.map((widget) => (
              <Link key={widget.id} to={widget.link || '#'}>
                <Surface className="p-4 hover:bg-[var(--warm-sand)] transition-all cursor-pointer">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Surface className="p-4 lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/notifications" className="text-xs text-[var(--clay)] hover:text-[var(--clay)]">View all →</Link>
            </div>
            <div className="space-y-3">
              {activities.map((activity) => (
                <div key={activity.id} className="flex items-start gap-3 p-3 rounded-xl bg-[var(--warm-sand)]">
          ...
          <div className="space-y-4">
            ...
            <Surface className="p-4">
              ...
              <div className="space-y-2">
                <Link to="/ai/analyze" className="block p-2 rounded-lg bg-[var(--warm-sand)] hover:bg-[var(--warm-sand)] text-sm text-[var(--warm-ink)]">Property Valuation</Link>
                <Link to="/ai/lease-anomaly" className="block p-2 rounded-lg bg-[var(--warm-sand)] hover:bg-[var(--warm-sand)] text-sm text-[var(--warm-ink)]">Lease Analyzer</Link>
                <Link to="/ai/lease-anomaly" className="block p-2 rounded-lg bg-[var(--warm-sand)] hover:bg-[var(--warm-sand)] text-sm text-[var(--warm-ink)]">Maintenance Assistant</Link>
              </div>
            </Surface>
          </div>
        </div>
