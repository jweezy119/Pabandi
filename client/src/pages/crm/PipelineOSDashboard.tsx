    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
      {stages.map(stage => {
        ...
        return (
          <div
            key={stage.id}
            className="bg-white/5 rounded-xl p-3 min-h-[200px] border border-white/10"
            onDragOver={e => e.preventDefault()}
            onDrop={() => handleDrop(stage.id)}
          >
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-semibold text-white">{stage.label}</h4>
              <span className="text-xs text-gray-500 bg-white/10 px-2 py-0.5 rounded-full">{stageClients.length}</span>
            </div>
            <div className="space-y-2">
              {stageClients.map(client => (
                <div
                  key={client.id}
                  className="p-3 rounded-lg bg-white/10 border border-white/10 cursor-grab active:cursor-grabbing hover:border-white/20 transition-all"
                  draggable
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400">{client.totalJobs} jobs</span>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-white">Revenue Alerts</h3>
      ...
      {alerts.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-gray-400">All clear. No alerts.</p>
        </div>
      ) : (
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-800 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-400 text-sm">Loading PipelineOS...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">Trust-Aware Revenue Engine</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <Card className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">Total Clients</p>
          ...
          <Card className="p-5">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-white mt-1">{clients.filter(c => c.stage === 'vip').length}</p>
              </div>
          ...
          <Card className="p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">At Risk</p>
                <p className="text-2xl font-bold text-white mt-1">{clients.filter(c => c.stage === 'at_risk').length}</p>
              </div>
              <div className="p-3 rounded-xl bg-gradient-to-br from-red-500 to-rose-400 shadow-lg">
          ...
          {(['kanban', 'clients', 'alerts'] as const).map((t) => (
            <button
              ...
              className={`flex-1 px-4 py-2.5 text-sm font-medium rounded-lg transition-all capitalize cursor-pointer ${
                activeTab === t ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
            >
              ...
              {clients.map(client => (
                <div key={client.id} className="flex items-center gap-4 p-4 rounded-xl bg-white/5 border border-white/10">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-400 mt-1">{client.stage} · {client.totalJobs} jobs</p>
                  </div>
                </div>
              ))}
            </div>
