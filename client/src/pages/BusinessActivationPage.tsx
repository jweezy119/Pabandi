...
export default function BusinessActivationPage({ businessId }: BusinessActivationPageProps) {
  ...
  return (
    <div className="min-h-screen pb-10 font-body" style={{ background: 'var(--cream)", color: 'var(--warm-ink)", fontFamily: tokens.font.body }}>
      <div className="mx-auto max-w-5xl px-4 py-6 sm:py-10">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm" style={{ color: 'var(--soft-stone)" }}>Complete these steps to unlock bookings, escrow protection, and $PAB rewards.</p>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-3">
          {[
            ...
            <button
              key={item.label}
              onClick={item.action}
              className="rounded-2xl border border-[rgba(191,179,163,0.2)] bg-[var(--warm-sand)] p-4 text-left transition-colors hover:-translate-y-[2px] hover:border-[rgba(191,179,163,0.3)]"
            >
              <span className="block font-headline font-bold text-sm sm:text-base mb-1" style={{ color: 'var(--warm-ink)" }}>{item.label}</span>
              <span className="inline-flex items-center gap-1 text-[11px] sm:text-xs font-bold" style={{ color: 'var(--clay)" }}>
        ...
        <Surface className="mt-6 space-y-3">
          <h3 className="font-headline text-xl font-bold" style={{ color: 'var(--warm-ink)" }}>Activation checklist</h3>
          <div className="flex flex-wrap gap-2">
            {steps.map((step) => (
              <span key={step.label} className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-xs sm:text-sm font-bold ${
                step.done
                  ? 'border-[var(--sage)]/40 bg-[rgba(var(--sage),0.1)] text-[var(--sage)]'
                  : 'border-[rgba(191,179,163,0.3)] bg-[var(--warm-sand)] text-[var(--warm-ink)]/70'
              }`}>
        ...
        <div className="mt-6 space-y-3">
          <h3 className="font-headline text-xl font-bold" style={{ color: 'var(--warm-ink)" }}>Payment & checkout</h3>
