      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all font-medium text-sm touch-target
                ${selectedBusinessType === option.value
                  ...
                  : 'bg-dark-800/50 text-[var(--soft-stone)] hover:bg-dark-700/50 hover:text-[var(--warm-ink)] border border-transparent'}
              `}
            >
        ...
        {selectedBusinessType !== 'ALL' && (
          <div className="mb-4 p-4 bg-brand-500/10 border border-brand-500/20 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-[var(--warm-ink)]">{getBusinessTypeConfig(selectedBusinessType as BusinessType).label}</h3>
                <p className="text-sm text-[var(--soft-stone)] mt-1">{getBusinessTypeConfig(selectedBusinessType as BusinessType).description}</p>
              </div>
              <div className="ml-auto text-right">
                <p className="text-xs text-[var(--soft-stone)]">Active entities</p>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-6">
          {quickActions.map((action) => (
            <Link key={action.label} to={action.link}>
              <Surface className="p-3 text-center hover:bg-[var(--cream)] transition-all cursor-pointer group">
                <div className="text-xl mb-1 group-hover:scale-110 transition-transform">{action.icon}</div>
                <div className="text-xs font-semibold text-[var(--warm-ink)] group-hover:text-brand-400">{action.label}</div>
              </Surface>
            </Link>
          ))}
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
            {widgets.map((widget) => (
              <Link key={widget.id} to={widget.link || '#'}>
                <Surface className="p-4 hover:bg-[var(--cream)] transition-all cursor-pointer group">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-lg group-hover:scale-110 transition-transform">{widget.icon}</span>
                    <span className="text-xs font-semibold text-[var(--soft-stone)] group-hover:text-brand-400">{widget.title}</span>
                  </div>
                  <div className="text-xl font-bold group-hover:text-brand-400" style={{ color: widget.color }}>{widget.value}</div>
                  {widget.change && (
                    <div className="text-xs mt-1 text-[var(--soft-stone)] group-hover:text-brand-300">{widget.change}</div>
                  )}
                </Surface>
              </Link>
            ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Surface className="p-4 lg:col-span-2">
            ...
            <div className="space-y-3">
              {activities.map((activity) => (
                <div key={activity.id} className="flex items-start gap-3 p-3 rounded-xl bg-[var(--cream)] hover:bg-[var(--warm-sand)] transition-colors cursor-pointer">
                  <div className="text-lg">{activity.icon}</div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-[var(--warm-ink)] font-medium">{activity.message}</div>
                    <div className="text-xs mt-1 text-[var(--soft-stone)]">{activity.time}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            ...
            <Link to="/sales-crm">
              <Surface className="p-4 hover:bg-[var(--warm-sand)] transition-colors cursor-pointer">
                <h3 className="text-base font-bold text-[var(--warm-ink)] mb-3">📈 Sales CRM</h3>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-[var(--warm-ink)] font-bold">{crmDeals.length}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span style={{ color: tokens.color.textDim }}>Open Tasks</span>
                    <span className="text-[var(--muted-ochre)] font-bold">{crmTasks.filter((t: any) => t.status !== 'COMPLETED').length}</span>
                  </div>
                </div>
