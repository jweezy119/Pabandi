...
export default function LiquidityTerminalPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center" style={{ background: tokens.color.background }}>
        <Surface className="w-full max-w-sm">
          <h2 className="mb-2 font-headline text-2xl font-bold text-[var(--warm-ink)] text-center">LP Terminal Auth</h2>
          <p className="mb-6 text-xs text-[var(--warm-ink)]/70 text-center">Enter your secure LP API Key to connect to the trading engine.</p>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            <input 
              ...
              className="rounded-lg border border-[var(--soft-stone)]/30 bg-black/50 px-4 py-2 text-sm text-[var(--warm-ink)] focus:border-indigo-500 focus:outline-none"
            />
  ...
  return (
    <div className="flex min-h-screen flex-col font-body" style={{ background: tokens.color.background, color: tokens.color.text }}>
      ...
      <header className="sticky top-0 z-10 border-b border-white/[0.05] bg-black/80 backdrop-blur-sm px-4 py-3 sm:px-6 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <h1 className="font-headline text-xl font-bold text-[var(--warm-ink)]">LP Terminal</h1>
          <div className="flex items-center gap-1.5 rounded-full bg-[var(--cream)] px-2.5 py-1 text-[10px] font-bold tracking-widest text-[var(--warm-ink)]/60">
            <div className={`h-2 w-2 rounded-full ${connected ? 'bg-[var(--sage)] shadow-[0_0_8px_rgba(16,185,129,0.8)]' : 'bg-rose-500'}`} />
            {connected ? 'LIVE' : 'RECONNECTING'}
          </div>
        </div>
        <button onClick={handleLogout} className="text-xs font-semibold text-[var(--warm-ink)]/50 hover:text-[var(--warm-ink)]">Disconnect</button>
      </header>
      ...
      <main className="mx-auto w-full max-w-4xl p-4 sm:p-6 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          {[
            ...
            <Surface key={stat.label} className="p-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--warm-ink)]/50">{stat.label}</p>
              <p className="mt-1 font-headline text-xl font-bold text-[var(--warm-ink)]">{stat.val}</p>
            </Surface>
          ))}
        </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-bold text-[var(--warm-ink)]/90">Action Required <span className="ml-2 rounded bg-[var(--clay)]/20 px-2 py-0.5 text-xs text-[var(--terracotta)]">{pendingIntents.length}</span></h2>
          </div>
          
          <div className="flex flex-col gap-3">
            {pendingIntents.length === 0 && (
              <Surface className="flex flex-col items-center justify-center py-12 text-center opacity-50">
                <BoltIcon className="h-8 w-8 mb-2 text-[var(--clay)]" />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[11px] font-mono text-[var(--warm-ink)]/50 flex items-center gap-1">
                        <ClockIcon className="h-3 w-3" /> SLA 60s
                      </span>
                    </div>
                    <div className="flex items-baseline gap-2">
                      <span className="font-headline text-2xl font-bold text-[var(--warm-ink)]">{intent.amountUsdc}</span>
                      <span className="text-xs text-[var(--warm-ink)]/60">USDC</span>
                      <span className="text-[var(--warm-ink)]/30 mx-1">→</span>
                      <span className="font-headline text-xl font-bold text-[var(--sage)]">{(intent.amountUsdc * intent.minRatePkr).toLocaleString()}</span>
                      <span className="text-xs text-[var(--sage)]/60">PKR</span>
                    </div>
                    <p className="mt-1 text-xs text-[var(--warm-ink)]/70">
                      Send to <strong className="text-[var(--warm-ink)]">{intent.destinationType}</strong>: {intent.destinationRef}
                    </p>
                  </div>
        ...
        <section className="mt-4">
          <h2 className="text-sm font-bold text-[var(--warm-ink)]/50 mb-3">Recent Activity</h2>
          <div className="flex flex-col gap-2">
            {activeIntents.slice(0, 10).map(intent => (
              <div key={intent.id} className="flex items-center justify-between rounded-lg border border-[var(--soft-stone)]/30 bg-white/[0.02] p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-[var(--warm-ink)]/80">
                      {intent.amountUsdc} USDC to {intent.destinationType}
                    </p>
                    <p className="text-[10px] text-[var(--warm-ink)]/40">ID: {intent.id.slice(-8)}</p>
                  </div>
                </div>
            ...
            {activeIntents.length === 0 && (
              <p className="text-xs text-center text-[var(--warm-ink)]/30 py-4">No recent activity.</p>
            )}
          </div>
        </section>
