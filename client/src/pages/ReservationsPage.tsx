...
const STATUS_CONFIG: Record<string, { label: string; accent: string }> = {
  CONFIRMED: { label: 'Confirmed', accent: 'var(--sage)' },
  PENDING: { label: 'Pending', accent: 'var(--muted-ochre)' },
  CANCELLED: { label: 'Cancelled', accent: 'var(--soft-stone)' },
  NO_SHOW: { label: 'No-Show', accent: 'var(--terracotta)' },
  COMPLETED: { label: 'Completed', accent: 'var(--clay)' },
  PENDING_CONCIERGE: { label: 'Concierge Booking', accent: 'var(--muted-ochre)' },
  FAILED_CONCIERGE: { label: 'Concierge Failed', accent: 'var(--terracotta)' },
};
    ...
    <div className="min-h-screen flex flex-col" style={{ background: tokens.color.background, color: tokens.color.text, fontFamily: tokens.font.body }}>
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-6 sm:px-6 sm:py-8 md:mb-8">
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex w-full overflow-x-auto rounded-xl border border-[rgba(191,179,163,0.2)] bg-[var(--warm-sand)] p-1.5 sm:w-auto">
            {FILTER_TABS.map((f) => (
              <button
                key={f.value}
                type="button"
          ...
          <Link to="/book" className="flex items-center justify-center gap-2 rounded-xl bg-[var(--clay)] px-5 py-3 text-center font-bold text-[var(--warm-ink)] shadow-sm transition-opacity hover:opacity-90 sm:w-auto">
            New Booking
          </Link>
        </div>
        ...
        {!isLoading && reservations.length === 0 && (
          <div className="rounded-2xl border border-dashed border-[rgba(191,179,163,0.25)] bg-[var(--warm-sand)] py-16 text-center">
                ...
                <Surface key={r.id} className="flex flex-col gap-5">
                  <div className="flex justify-between items-start gap-3">
                    <div className="flex flex-col gap-1">
                      <div className="inline-flex w-fit items-center gap-2 rounded-full px-2 py-1 text-[11px] font-bold uppercase tracking-wider text-[var(--warm-ink)]" style={{ background: `${status.accent}25`, color: status.accent, border: `1px solid ${status.accent}40` }}>
                        {status.label}
                      </div>
                  ...
                  <div className="flex flex-col gap-3 rounded-lg bg-[var(--warm-sand)] p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <span className={`inline-flex items-center gap-1 rounded-full bg-[var(--warm-sand)] px-2 py-1 text-[10px] font-bold ${deposit.className}`}>
