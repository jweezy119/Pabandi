...
export const FreightPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'loads' | 'post' | 'carriers' | 'my-loads' | 'messages'>('dashboard');
...
const Dashboard: React.FC<{ stats: any; loads: any[]; carriers: any[]; onNavigate: (tab: any) => void }> = ({ stats, loads, carriers, onNavigate }) => (
  <div className="space-y-6">
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div className="bg-[#111827] rounded-xl p-4 border border-white/5">
        <div className="text-2xl font-bold text-emerald-400">${((stats.totalRevenue || 0) / 1000).toFixed(1)}K</div>
...
// ── Load Board ─────────────────────────────────────────────────────────────
const LoadBoard: React.FC<any> = ({ loads, loading, searchOrigin, searchDest, setSearchOrigin, setSearchDest, onSearch, onSelect }) => (
  <div className="space-y-4">
    ...
    {loading ? (
      ...
      <div className="space-y-3">
        {loads.map((load: any) => (
          <div key={load.id} onClick={() => onSelect(load)} className="bg-[#111827] rounded-xl p-4 border border-white/5 hover:border-orange-400/30 transition-all cursor-pointer">
...
const MessagesView: React.FC = () => (
  <div className="bg-[#111827] rounded-xl p-12 text-center border border-white/5">
    ...
    <p className="text-slate-400">Direct messaging with carriers and shippers coming soon.</p>
  </div>
);
