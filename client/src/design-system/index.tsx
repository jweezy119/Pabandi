...
export function Button({ children, onClick, variant = 'default', size = 'md', className = '', disabled = false, type = 'button', style, loading = false, leftIcon, rightIcon }: ButtonProps) {
  const base = [
  ...
  const variants: Record<Variant, string> = {
    ...
    primary: 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-[0_0_20px_rgba(99,102,241,0.35)] hover:shadow-[0_0_30px_rgba(99,102,241,0.55)] active:scale-[0.97]',
  };

  return (
    <button onClick={onClick} type={type} className={`${base} ${sizes[size]} ${variants[variant]} ${className}`} style={style} disabled={disabled || loading}>
      {loading ? (
