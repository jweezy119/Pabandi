...
async function seedPakistanBusinesses() {
  ...
  const categories = ['RESTAURANT', 'RETAIL', 'SERVICE', 'HEALTHCARE', 'EDUCATION', 'OTHER'] as const;
  ...
  for (const category of categories) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const businessNames = BUSINESS_NAMES[category.toLowerCase() as keyof typeof BUSINESS_NAMES] || BUSINESS_NAMES.other;
  ...
  console.log('\n📊 Businesses by City:');
  for (const city of byCity.sort((a: any, b: any) => b._count.id - a._count.id)) {
  ...
  for (const cat of byCategory.sort((a: any, b: any) => b._count.id - a._count.id)) {
