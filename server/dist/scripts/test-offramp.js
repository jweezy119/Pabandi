...
async function runOfframpTest() {
    ...
    const intentRes = await axios_1.default.post(`${API_URL}/intent`, intentPayload, {
    ...
    const statusRes = await axios_1.default.get(`${API_URL}/intent/${intentId}`);
    ...
    try {
        const proofRes = await axios_1.default.post(`${API_URL}/lp/submit-proof`, {
            intentId: intentId,
            lpWallet: lpWallet,
    ...
    console.log(`\n[4] Final status check...`);
    const finalStatus = await axios_1.default.get(`${API_URL}/intent/${intentId}`);
    console.log(`Final Status: ${finalStatus.data.intent.status}`);
}
runOfframpTest().catch(console.error);
//# sourceMappingURL=test-offramp.js.map