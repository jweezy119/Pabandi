...
function categoryFromTags(tags) {
    if (tags.shop === 'beauty' || tags.shop === 'hairdresser' || tags.amenity === 'hairdresser')
        return 'SALON';
    if (tags.shop === 'massage')
        return 'SPA';
    if (tags.amenity === 'clinic' || tags.amenity === 'hospital' || tags.amenity === 'doctor' || tags.amenity === 'dentist')
        return 'CLINIC';
    if (tags.leisure === 'fitness_centre' || tags.amenity === 'gym')
        return 'FITNESS_CENTER';
    if (tags.tourism === 'hotel' || tags.tourism === 'motel' || tags.tourism === 'guest_house')
        return 'PROPERTY_RENTAL';
    if (tags.amenity === 'restaurant' || tags.amenity === 'cafe' || tags.amenity === 'fast_food' || tags.amenity === 'food_court' || tags.amenity === 'bar')
        return 'RESTAURANT';
    if (tags.shop)
        return 'OTHER';
    if (tags.office)
    ...
    for (const url of endpoints) {
        for (let attempt = 0; attempt < 3; attempt++) {
            try {
                ...
                const res = await axios_1.default.post(url, `data=${encodeURIComponent(query)}`, {
...
function fallbackPoisForCity(city) {
...
async function fetchOsmNodesForCity(city) {
    const elements = [];
...
async function upsertBusinesses(pois) {
    let created = 0;
    let skipped = 0;
    let errors = 0;
...
//# sourceMappingURL=seed-osm-businesses.js.map