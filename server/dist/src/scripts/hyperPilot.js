...
function makeTenants(n, seed) {
    const r = rng(seed);
    const out = [];
    for (let i = 0; i < n; i++) {
        const score = Math.max(5, Math.min(99, Math.round(50 + (r() + r() + r() - 1.5) * 38)));
        out.push({ id: `T${i}`, trustScore: score, trustBand: ptp_spec_1.ptpEngine.scoreToRiskBand(score) });
    }
    return out;
}
const SCENARIOS = {
...
function runPilot(scenario, tenants) {
    ...
    for (const ll of LANDLORDS) {
        ...
        for (const t of tenants) {
            ...
            if (r() < sc.defaultRate * (0.5 + riskIdx))
                defaults++;
        }
    }
...
async function main() {
    ...
    (0, fs_1.writeFileSync)('hyperPilot.report.json', JSON.stringify(report, null, 2));
    (0, fs_1.writeFileSync)('hyperPilot.dashboard.html', renderHtml(results));
...
//# sourceMappingURL=hyperPilot.js.map