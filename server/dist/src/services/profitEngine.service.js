...
exports.profitEngine = void 0;
exports.quoteFee = quoteFee;
exports.decideReinvestment = decideReinvestment;
exports.applyReinvestmentCycle = applyReinvestmentCycle;
/**
...
function quoteFee(amountPab) {
    let chosen = TIERS[0];
    for (const t of TIERS)
        if (amountPab >= t.minPab)
            chosen = t;
 ...
 */
function decideReinvestment(opts) {
 ...
 */
async function applyReinvestmentCycle(summary) {
    ...
    const thin = await database_1.prisma.web3Agent.findMany({
        where: { isActive: true },
    ...
    for (const a of thin) {
        if (perAgent <= 0)
            break;
        try {
            await database_1.prisma.web3Agent.update({ where: { id: a.id }, data: { balancePab: { increment: perAgent } } });
    ...
    logger_1.logger.info(`[ProfitEngine] cycle ${summary.cycle}: reinvested ${pabToAgents} PAB → ${thin.length} agents, ${solToBuffer} SOL → buffer; retained ${(collectedPab - pabToAgents)} PAB profit`);
    return { fundedAgents: thin.length, pabToAgents, solToBuffer };
}
exports.profitEngine = { quoteFee, decideReinvestment, applyReinvestmentCycle, TIERS };
