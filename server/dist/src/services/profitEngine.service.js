...
exports.profitEngine = exports.ProfitEngine = void 0;
const database_1 = require("../utils/database");
const compounding_service_1 = require("./compounding.service");
// ─── Configuration ───────────────────────────────────────
...
class ProfitEngine {
    ...
    get feeRate() {
        return compounding_service_1.compoundingService.getCurrentSettings().feeRate;
    }
    // Use dynamic task value from compounding service
    get taskValue() {
        return compounding_service_1.compoundingService.getCurrentSettings().taskValue;
    }
    // ─── RATE LIMIT CHECK ──────────────────────────────────
    isRateLimited() {
        const now = Date.now();
    ...
    async runCycle() {
        ...
        try {
            ...
            const solver = agents.slice(1).find(a => a.capabilities.some(c => poster.capabilities.includes(c))) || agents[1];
    ...
    async completeProject(projectId, solverId, posterId) {
        await database_1.prisma.agentEscrow.updateMany({
            where: { projectId },
        ...
        const escrow = await database_1.prisma.agentEscrow.findUnique({ where: { projectId } });
        const project = await database_1.prisma.agentProject.findUnique({ where: { id: projectId } });
    ...
    recordCycle(startTime, projectId, revenue, pab, success) {
    ...
    async deployIdleCapital(amountUsd) {
...
exports.ProfitEngine = ProfitEngine;
exports.profitEngine = new ProfitEngine();
//# sourceMappingURL=profitEngine.service.js.map