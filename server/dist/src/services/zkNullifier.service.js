...
exports.zkNullifierService = exports.ZkNullifierService = void 0;
/**
...
class ZkNullifierService {
    constructor() {
        ...
        try {
            const fm = noir.createFileManager((0, path_1.resolve)(__dirname, '..', '..'));
            noir.compile(fm, (0, path_1.resolve)(__dirname, '..', '..', 'src', 'zk')).then(() => {
                this.circuitReady = true;
                logger_1.logger.info('[ZK] Noir Proof-of-Rent circuit compiled successfully (valid ZK circuit).');
            }).catch((e) => logger_1.logger.warn(`[ZK] circuit compile deferred: ${e?.message}`));
        }
        catch (e) {
            logger_1.logger.warn(`[ZK] circuit compile skipped: ${e?.message}`);
        }
    }
    /**
     ...
     */
    async issueProof(tenantDID, propertyDID, consecutiveMonths, secret) {
        const sec = secret || (0, crypto_1.createHash)('sha256').update(`${tenantDID}:${propertyDID}:${Date.now()}:${Math.random()}`).digest('hex');
        const secretField = BigInt('0x' + sec.slice(0, 60)) % P;
        const tenantField = BigInt('0x' + (0, crypto_1.createHash)('sha256').update(tenantDID).digest('hex').slice(0, 60)) % P;
        ...
        const prior = await database_1.prisma.trustAuditTrail.findFirst({ where: { component: 'ZK_NULLIFIER', changeReason: nullifier } }).catch(() => null);
        if (prior)
        ...
        const anchor = await solanaAnchor_service_1.solanaAnchor.anchorOnSolana('ZK_NULLIFIER', { epoch: NULLIFIER_EPOCH, commitment: commitmentHex, nullifier, merkleRoot, consecutiveMonths }, 'PABANDI_ZK');
        await database_1.prisma.trustAuditTrail.create({
            data: {
                ...
                metadata: { commitment: commitmentHex, merkleRoot, anchor, consecutiveMonths, zkType: 'noir-constraint' },
            },
        }).catch((e) => logger_1.logger.warn(`[ZK] audit persist skipped: ${e.message}`));
        return {
    ...
    async verifyProof(proof) {
        if (!proof.commitment || !proof.nullifier)
            return { valid: false, reason: 'missing commitment/nullifier' };
        const stored = await database_1.prisma.trustAuditTrail.findFirst({ where: { component: 'ZK_NULLIFIER', changeReason: proof.nullifier } }).catch(() => null);
        if (!stored)
            return { valid: false, reason: 'nullifier not found in registry' };
        const meta = stored.metadata || {};
        if (meta.merkleRoot !== proof.merkleRoot)
            return { valid: false, reason: 'merkle root mismatch' };
        if (meta.commitment !== proof.commitment)
    ...
    getIssuedCount() { return issuedNullifiers.length; }
}
exports.ZkNullifierService = ZkNullifierService;
exports.zkNullifierService = new ZkNullifierService();
//# sourceMappingURL=zkNullifier.service.js.map