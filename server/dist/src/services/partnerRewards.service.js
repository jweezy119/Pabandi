...
exports.partnerRewardsService = void 0;
const client_1 = require("@prisma/client");
const crypto_1 = __importDefault(require("crypto"));
const prisma = new client_1.PrismaClient();
exports.partnerRewardsService = {
    // ── Partner Enrollment ───────────────────────────────────────────────────
    async enrollBusiness(businessId, commissionRate = 0.02) {
    ...
    async redeemOffer(userId, offerId) {
        ...
        if (offer.totalRedeemed >= offer.maxRedemptions)
            throw new Error('Offer fully redeemed');
        ...
        const redemptionCode = `RWD-${crypto_1.default.randomBytes(4).toString('hex').toUpperCase()}`;
    ...
    async confirmRedemption(userId, redemptionCode, purchaseAmount) {
    ...
    async getRewardByCode(redemptionCode) {
    ...
    // ── Analytics ────────────────────────────────────────────────────────────
    async getPartnerAnalytics(businessId) {
...
//# sourceMappingURL=partnerRewards.service.js.map