...
exports.autonomousEconomyService = exports.AutonomousEconomyService = void 0;
const web3_js_1 = require("@solana/web3.js");
const database_1 = require("../utils/database");
const logger_1 = require("../utils/logger");
const JITOSOL_MINT = new web3_js_1.PublicKey('J1toso1uCk3RLmjorhTtrVwY9HJ7X8V9yYac6Y7kGCPn'); // mainnet
class AutonomousEconomyService {
    conn() {
        return new web3_js_1.Connection(process.env.SOLANA_RPC_URL || 'https://api.mainnet-beta.solana.com', 'confirmed');
    }
    treasury() {
        ...
        try {
            ...
            try {
                ...
                return web3_js_1.Keypair.fromSecretKey(new Uint8Array(b));
            }
            catch {
                return null;
            }
    ...
    feeWallet() {
        return new web3_js_1.PublicKey(process.env.FEE_TREASURY_WALLET || '5AR6fsezB8NTYQWwP1DxysuKPZAEY12yeVt22hL6FvdG');
    }
    yieldVault() {
        ...
        return new web3_js_1.PublicKey(process.env.YIELD_VAULT_WALLET || process.env.FEE_TREASURY_WALLET || '5AR6fsezB8NTYQWwP1DxysuKPZAEY12yeVt22hL6FvdG');
    }
    /** Net on-chain SOL revenue (fee wallet inflow) for the profitability report. */
    async netSolRevenue(sinceDays = 30) {
    ...
    async quoteRake(payer, solAmount) {
    ...
    async demoBook(opts) {
        ...
        if (!agentId && opts?.gigId) {
            try {
                const gig = await database_1.prisma.project.findUnique({ where: { id: opts.gigId } });
                if (gig?.bestAgentId)
                    agentId = gig.bestAgentId;
                else {
                    const bids = await database_1.prisma.projectBid.findMany({ where: { projectId: opts.gigId, status: 'PENDING' }, take: 1 });
                    if (bids[0])
                        agentId = bids[0].agentId;
                }
            }
            catch { /* ignore */ }
        }
        ...
        if (stakePab > 0 && agentId) {
            try {
                ...
                await database_1.prisma.web3Agent.update({ where: { id: agentId }, data: { balancePab: { decrement: stakePab } } }).catch(() => { });
            }
            catch { /* ignore */ }
        }
        const ref = `demo:${Date.now()}`;
        await database_1.prisma.treasuryPosition.create({ data: { bucket: 'PLATFORM_FEE', amount: rake, status: 'DEPLOYED', txHash: ref, meta: { asset: 'SOL', source: 'HUMAN_RAKE', simulated: true, solAmount, rakeSol: rake, referralCode: opts?.referralCode, referralSol, partnerId: opts?.partnerId, partnerSol, agentId: opts?.agentId, gigId: opts?.gigId, stakePab, note: 'demo booking' } } }).catch(() => { });
        if (referralSol > 0)
            await database_1.prisma.treasuryPosition.create({ data: { bucket: 'REFERRAL_EARNED', amount: referralSol, status: 'PENDING', txHash: `${ref}:ref`, meta: { asset: 'SOL', source: 'REFERRAL', referralCode: opts?.referralCode, simulated: true } } }).catch(() => { });
        if (partnerSol > 0)
            await database_1.prisma.treasuryPosition.create({ data: { bucket: 'PARTNER_FEE', amount: partnerSol, status: 'DEPLOYED', txHash: `${ref}:partner`, meta: { asset: 'SOL', source: 'PARTNER_FEE', partnerId: opts?.partnerId, simulated: true } } }).catch(() => { });
    ...
    async businessDashboard(referralCode) {
        const [posted, bookings, refEarned, rakeRows] = await Promise.all([
            database_1.prisma.gigEvent.count({ where: { kind: 'POST', referralCode } }),
            database_1.prisma.treasuryPosition.count({ where: { bucket: 'PLATFORM_FEE', meta: { path: ['referralCode'], equals: referralCode } } }),
            database_1.prisma.treasuryPosition.findMany({ where: { bucket: 'REFERRAL_EARNED', meta: { path: ['referralCode'], equals: referralCode } } }),
            database_1.prisma.treasuryPosition.findMany({ where: { bucket: 'PLATFORM_FEE', meta: { path: ['referralCode'], equals: referralCode } } }),
        ]);
        let referralSolEarned = 0;
        for (const r of refEarned)
            if (r.meta?.asset !== 'PAB')
                referralSolEarned += r.amount || 0;
        let rakeSolEarned = 0, pabStaked = 0;
        for (const r of rakeRows) {
            rakeSolEarned += r.meta?.rakeSol || 0;
            pabStaked += r.meta?.stakePab || 0;
        }
    ...
    async chargeRake(payer, solAmount, bookingRef, opts) {
        const conn = this.conn();
        const kp = this.treasury();
        if (!kp)
            throw new Error('SOLANA_PRIVATE_KEY not set');
        const feeWallet = this.feeWallet();
        const payerPub = new web3_js_1.PublicKey(payer);
        const rake = +(solAmount * 0.01).toFixed(6);
        ...
        const ixs = [
            web3_js_1.SystemProgram.transfer({ fromPubkey: payerPub, toPubkey: feeWallet, lamports: Math.round(rake * web3_js_1.LAMPORTS_PER_SOL) }),
            web3_js_1.SystemProgram.transfer({ fromPubkey: payerPub, toPubkey: kp.publicKey, lamports: Math.round(net * web3_js_1.LAMPORTS_PER_SOL) }),
        ];
        if (partnerSol > 0)
            ixs.push(web3_js_1.SystemProgram.transfer({ fromPubkey: payerPub, toPubkey: feeWallet, lamports: Math.round(partnerSol * web3_js_1.LAMPORTS_PER_SOL) }));
     ...
     */
    async confirmRake(bookingRef, txHash) {
        const conn = this.conn();
        const pending = await database_1.prisma.treasuryPosition.findFirst({
        ...
        const rake = pending.meta?.rakeSol || 0;
        const referralCode = pending.meta?.referralCode;
        const referralSol = pending.meta?.referralSol || 0;
        const partnerId = pending.meta?.partnerId;
        const partnerSol = pending.meta?.partnerSol || 0;
        await database_1.prisma.treasuryPosition.update({
            where: { id: pending.id },
            data: { status: 'DEPLOYED', meta: { ...pending.meta, txHash, source: 'PLATFORM_FEE', confirmedAt: new Date().toISOString() } },
        });
     ...
     */
    async routeToYield(user, solAmount, bookingRef, opts) {
        const conn = this.conn();
        const kp = this.treasury();
        if (!kp)
            throw new Error('SOLANA_PRIVATE_KEY not set');
        ...
        const ixs = [
            web3_js_1.SystemProgram.transfer({ fromPubkey: userPub, toPubkey: feeWallet, lamports: Math.round(fee * web3_js_1.LAMPORTS_PER_SOL) }),
            web3_js_1.SystemProgram.transfer({ fromPubkey: userPub, toPubkey: this.yieldVault(), lamports: Math.round(stakeAmt * web3_js_1.LAMPORTS_PER_SOL) }),
        ];
        if (partnerSol > 0)
            ixs.push(web3_js_1.SystemProgram.transfer({ fromPubkey: userPub, toPubkey: feeWallet, lamports: Math.round(partnerSol * web3_js_1.LAMPORTS_PER_SOL) }));
        const tx = new web3_js_1.Transaction().add(...ixs);
        tx.feePayer = userPub;
        const { blockhash } = await conn.getLatestBlockhash();
        tx.recentBlockhash = blockhash;
    ...
    async quoteYield(user, solAmount) {
        const fee = +(solAmount * Number(process.env.PLATFORM_YIELD_FEE || 0.005)).toFixed(6);
    ...
    async confirmYield(bookingRef, txHash) {
        const conn = this.conn();
        const pending = await database_1.prisma.treasuryPosition.findFirst({
            where: { txHash: bookingRef, bucket: 'PENDING_CHARGE', status: 'PENDING' },
        });
        if (!pending)
            return { confirmed: false, platformFeeSol: 0 };
        try {
            const info = await conn.getTransaction(txHash, { commitment: 'confirmed' });
            if (!info)
                return { confirmed: false, platformFeeSol: 0 };
        ...
        const fee = pending.meta?.feeSol || 0;
        const partnerId = pending.meta?.partnerId;
        const partnerSol = pending.meta?.partnerSol || 0;
        await database_1.prisma.treasuryPosition.update({
            where: { id: pending.id },
    ...
    async referralStats(referralCode) {
        const rows = await database_1.prisma.treasuryPosition.findMany({
        ...
        for (const r of rows) {
            if (r.meta?.asset === 'PAB')
                earnedPab += r.amount || 0;
            else
                earnedSol += r.amount || 0;
        }
    ...
    async leaderboard(limit = 10) {
        const [refRows, partRows] = await Promise.all([
            database_1.prisma.treasuryPosition.findMany({ where: { bucket: 'REFERRAL_EARNED' } }),
            database_1.prisma.treasuryPosition.findMany({ where: { bucket: 'PARTNER_FEE' } }),
        ]);
        const refMap = new Map();
        for (const r of refRows) {
            if (r.meta?.demo)
                continue; // exclude demo conversions from public board
            const code = r.meta?.referralCode || 'unknown';
            const e = refMap.get(code) || { earnedSol: 0, earnedPab: 0, claims: 0 };
            if (r.meta?.asset === 'PAB')
                e.earnedPab += r.amount || 0;
            else
                e.earnedSol += r.amount || 0;
            e.claims += 1;
            refMap.set(code, e);
        }
        const partMap = new Map();
        for (const r of partRows) {
            const pid = r.meta?.partnerId || 'unknown';
    ...
    async autonomousReinvest() {
        const conn = this.conn();
        const kp = this.treasury();
        if (!kp)
            return { stakedSol: 0, note: 'treasury key missing' };
        ...
        logger_1.logger.info(`[AutoEcon] Reinvestment gate: would stake ${stake} SOL to JitoSOL (guarded; executes when balance is real).`);
        return { stakedSol: stake, note: 'reinvestment decision computed (stake gate armed, no-op at current balance)' };
    }
}
exports.AutonomousEconomyService = AutonomousEconomyService;
exports.autonomousEconomyService = new AutonomousEconomyService();
//# sourceMappingURL=autonomousEconomy.service.js.map