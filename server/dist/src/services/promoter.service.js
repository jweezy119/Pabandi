...
exports.promoterService = {
     ...
     */
    async registerPromoter(userId, data) {
        // Check if user already has a promoter profile
        const existing = await database_1.prisma.promoter.findUnique({
            where: { userId },
        });
        if (existing) {
            throw new Error('User already has a promoter profile');
        }
        return database_1.prisma.promoter.create({
            data: {
                userId,
                name: data.name,
                phone: data.phone,
    ...
    async getPromoterByUserId(userId) {
        return database_1.prisma.promoter.findUnique({
            where: { userId },
            include: {
                promoCodes: true,
                reviews: {
    ...
    async getPromoterStats(promoterId) {
        ...
        const totalBookings = await database_1.prisma.bottleReservation.count({
            where: { promoterId },
        });
        // Total commission earned
        const commissionResult = await database_1.prisma.referralLedger.aggregate({
            where: {
        ...
        const totalGuestLists = await database_1.prisma.guestList.count({
            where: { promoterId },
        });
        const conversionRate = totalGuestLists > 0
            ...
            : 0;
        // Average rating
        const reviews = await database_1.prisma.promoterReview.findMany({
            where: { promoterId },
        });
        ...
        const bookings = await database_1.prisma.bottleReservation.findMany({
            where: { promoterId },
            include: {
        ...
        const guestLists = await database_1.prisma.guestList.findMany({
            where: { promoterId },
     ...
     */
    async generateReferralCode(promoterId) {
        const promoter = await database_1.prisma.promoter.findUnique({
            where: { id: promoterId },
        });
        if (!promoter) {
     ...
     */
    async getPromoterBookings(promoterId, options) {
        ...
        const [bookings, total] = await Promise.all([
            database_1.prisma.bottleReservation.findMany({
                where,
                include: {
            ...
            database_1.prisma.bottleReservation.count({ where }),
        ]);
    ...
    async getPromoterLeaderboard(limit = 10) {
        const results = await database_1.prisma.referralLedger.groupBy({
            by: ['profileId'],
        ...
        const leaderboard = await Promise.all(results.map(async (entry) => {
            const promoter = await database_1.prisma.promoter.findUnique({
    ...
    async creditPromoter(promoterId, amount, bookingId) {
        ...
        return database_1.prisma.referralLedger.create({
            data: {
                profileId: promoter.userId,
                type: 'BOOKING_COMMISSION',
                amount,
     ...
     */
    async getPromoterWallet(promoterId) {
        const promoter = await database_1.prisma.promoter.findUnique({
            where: { id: promoterId },
        });
        if (!promoter) {
        ...
        const entries = await database_1.prisma.referralLedger.findMany({
            where: {
                profileId: promoter.userId,
                type: 'BOOKING_COMMISSION',
            },
