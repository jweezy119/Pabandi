...
class BadgeService {
     ...
     */
    generatePseudonymousId(userId) {
        const salt = process.env.BADGE_SALT || process.env.JWT_SECRET || 'pabandi_badge_salt_v1';
        return crypto_1.default.createHmac('sha256', salt).update(userId).digest('hex').slice(0, 32);
    }
    /**
     ...
     */
    async resolveUserFromPseudonymousId(pseudonymousId) {
        try {
            ...
            logger_1.logger.error('Error resolving pseudonymous ID:', error);
            return null;
        }
    }
    /**
     ...
     */
    computeSocialTrustBoost(identities) {
        const breakdown = {};
        let totalBoost = 0;
        for (const identity of identities) {
            ...
            if (identity.platform === 'LINKEDIN') {
                if (identity.isVerified)
                    boost += 1;
                if (identity.completeness && identity.completeness >= 0.9)
                    boost += 1;
                if (identity.accountAgeDays && identity.accountAgeDays > 365 * 3)
                    boost += 1;
            }
            ...
            if (['WHATSAPP', 'INSTAGRAM', 'FACEBOOK'].includes(identity.platform)) {
                if (identity.isVerified)
                    boost += 1;
                if (identity.accountAgeDays && identity.accountAgeDays > 365 * 2)
                    boost += 1;
            }
            // X (Twitter) bonuses
            if (identity.platform === 'X_TWITTER') {
                if (identity.isVerified)
                    boost += 1;
                if (identity.accountAgeDays && identity.accountAgeDays > 365)
                    boost += 1;
            }
            // TikTok bonuses
            ...
            if (['FIVERR', 'UPWORK'].includes(identity.platform)) {
                if (identity.rating && identity.rating >= 4.8)
                    boost += 2;
                if (identity.completionRate && identity.completionRate >= 0.95)
                    boost += 2;
                if (identity.accountAgeDays && identity.accountAgeDays > 365 * 2)
                    boost += 1;
            }
     ...
     */
    async computeBadgeStatus(userId) {
        const [user, stats, identities] = await Promise.all([
            ...
            database_1.prisma.socialIdentity.findMany({ where: { userId } }),
        ]);
        if (!user)
        ...
        const tier = effectiveScore >= 80 ? 'EXCELLENT' : effectiveScore >= 50 ? 'AVERAGE' : 'RISKY';
        // Dynamic badge list
        const badges = [];
        if (completed >= 1)
            badges.push('First Booking');
        if (completed >= 5)
            badges.push('5-Booking Streak');
        if (completed >= 10)
            badges.push('Star Patron');
        if (noShows === 0 && totalBookings >= 3)
            badges.push('Perfect Record');
        if (identities.find(i => i.platform === 'LINKEDIN') && completed >= 10)
            badges.push('LinkedIn Luminary');
        ...
        const salt = process.env.BADGE_SALT || process.env.JWT_SECRET || 'pabandi_badge_salt_v1';
        const signedHash = 'sha256:' + crypto_1.default.createHmac('sha256', salt).update(payloadStr).digest('hex');
        // --- Autonomous Blockchain Sync ---
        database_1.prisma.wallet.findUnique({ where: { userId } }).then(async (wallet) => {
            if (wallet?.address) {
                const aiTrustProfile = await dashscope_service_1.dashscopeService.generateTrustProfile(userId);
                blockchain_service_1.blockchainService.checkAndMintEligibleBadge(wallet.address, pseudonymousId, effectiveScore, totalBookings, attendanceRate, aiTrustProfile).catch(err => logger_1.logger.error('[AutonomousSync] Error syncing badge:', err.message));
            }
        }).catch(err => logger_1.logger.error('[AutonomousSync] Error fetching wallet:', err.message));
        return {
            pseudonymousId,
            tier,
     ...
     */
    async getShareCard(userId, platform) {
