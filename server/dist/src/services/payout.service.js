    ...
    async request(userId, amountUsdc, method = 'BANK', destinationRef) {
        ...
        if (method === 'BANK') {
            ...
            try {
                ...
                logger_1.logger.info(`[Payout] CashApp payment link ${link.id} created for ${userId} net $${quote.netAmount}.`);
            }
            catch (e) {
                logger_1.logger.warn(`[Payout] CashApp payout failed, simulated fallback: ${e.message}`);
                destRef = 'SIMULATED';
            }
        }
