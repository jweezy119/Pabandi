...
exports.paypalService = {
     ...
     */
    async createCheckoutUrl(amount, currency, reservationId, returnUrl, cancelUrl) {
        ...
        if (!PAYPAL_CLIENT_ID || !PAYPAL_CLIENT_SECRET) {
            logger_1.logger.warn('PayPal credentials not set');
        ...
        try {
            ...
            logger_1.logger.info(`PayPal order created for reservation: ${reservationId} (${currency.toUpperCase()} ${amountStr})`);
            return approveLink;
        }
        catch (error) {
            logger_1.logger.error('PayPal checkout creation failed', error.message);
            return `${frontendUrl}/reservations?paypal_disabled=true&ref=${reservationId}`;
        }
    },
    /**
    ...
    async captureOrder(orderId) {
        try {
            ...
            logger_1.logger.info(`PayPal order ${orderId} captured successfully`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('PayPal capture failed', error.message);
            return false;
        }
    },
    /**
     ...
     */
    async refundDeposit(captureId, amountCents) {
        if (!PAYPAL_CLIENT_ID) {
            logger_1.logger.warn('PayPal credentials not set — skipping refund');
            return true;
        }
        try {
            const token = await getAccessToken();
            ...
            logger_1.logger.info(`PayPal refund issued for capture: ${captureId}`);
            return true;
        }
        catch (error) {
            logger_1.logger.error('PayPal refund failed', error.message);
            return false;
        }
    },
    /**
    ...
    async verifyWebhook(headers, rawBody, webhookId) {
        if (!PAYPAL_CLIENT_ID) {
            logger_1.logger.warn('PayPal webhook verification skipped: no credentials');
        ...
        try {
            ...
            logger_1.logger.error('PayPal webhook verification failed', error.message);
            return false;
        }
    },
};
