...
exports.nightlifeIntegrationService = void 0;
const client_1 = require("@prisma/client");
const crypto_1 = __importDefault(require("crypto"));
const prisma = new client_1.PrismaClient();
exports.nightlifeIntegrationService = {
    // ═══════════════════════════════════════════════════════════════════════════
    // 1. PROMOTER OS — Unified dashboard for promoters to manage everything
    // ═══════════════════════════════════════════════════════════════════════════
    async createPromoterProfile(userId, data) {
        return prisma.promoter.create({
            data: { userId, ...data },
        });
    },
    async getPromoterDashboard(promoterId) {
        ...
        const totalGuests = promoter.guestLists.reduce((sum, g) => sum + g.partySize, 0);
        const arrivedGuests = promoter.guestLists.filter((g) => g.status === 'ARRIVED').length;
        const noShowGuests = promoter.guestLists.filter((g) => g.status === 'NO_SHOW').length;
        const avgRating = promoter.reviews.length > 0
            ? promoter.reviews.reduce((sum, r) => sum + r.rating, 0) / promoter.reviews.length
            : 0;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async createSmartGuestList(data) {
        ...
        return prisma.guestList.create({
            data: {
                ...
                status: 'CONFIRMED',
                noShowProbability,
                confirmationCode: `GL-${crypto_1.default.randomBytes(3).toString('hex').toUpperCase()}`,
                depositAmount,
                depositStatus: depositAmount > 0 ? 'PENDING' : 'WAIVED',
            },
        });
    },
    calculateDeposit(noShowProbability, venueCapacity) {
        if (noShowProbability < 0.2)
            return 0;
        if (noShowProbability < 0.4)
            return 10;
        if (noShowProbability < 0.6)
            return 25;
        if (noShowProbability < 0.8)
            return 50;
        return 100;
    },
    ...
    async generateQRCode(guestListId) {
    ...
    async verifyEntry(confirmationCode, venueId) {
        const guestList = await prisma.guestList.findUnique({
            where: { confirmationCode },
        });
        if (!guestList)
            return { valid: false, reason: 'Code not found' };
        if (guestList.venueId !== venueId)
            return { valid: false, reason: 'Wrong venue' };
        if (guestList.status === 'ARRIVED')
            return { valid: false, reason: 'Already checked in' };
        if (guestList.status === 'CANCELLED')
    ...
    async syncInstagramVenue(instagramHandle) {
    ...
    async sendWhatsAppConfirmation(phone, guestList) {
    ...
    async syncEventbriteEvent(eventbriteId) {
    ...
    async sendSMSConfirmation(phone, guestList) {
    ...
    async getVenueDashboard(venueId) {
        ...
        const arrived = todayGuests.filter((g) => g.status === 'ARRIVED').length;
        const waiting = todayGuests.filter((g) => g.status === 'WAITING').length;
        const confirmed = todayGuests.filter((g) => g.status === 'CONFIRMED').length;
        const noShows = todayGuests.filter((g) => g.status === 'NO_SHOW').length;
        const totalBottleSpend = (venue.bottleReservations || []).reduce((sum, r) => sum + r.totalPrice, 0);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async checkPromoterFraud(promoterId) {
        const promoter = await prisma.promoter.findUnique({
            where: { id: promoterId },
            include: {
                guestLists: true,
        ...
        let riskScore = 0;
        // High no-show rate
        const noShowRate = promoter.guestLists.filter((g) => g.status === 'NO_SHOW').length / promoter.guestLists.length;
        if (noShowRate > 0.5) {
            riskScore += 30;
            reasons.push(`High no-show rate: ${(noShowRate * 100).toFixed(0)}%`);
        }
        // Low review average
        const avgRating = promoter.reviews.reduce((sum, r) => sum + r.rating, 0) / promoter.reviews.length;
        ...
        const allGuestNames = promoter.guestLists.flatMap((g) => g.guestNames || []);
        ...
        const confirmed = promoter.guestLists.filter((g) => g.status === 'CONFIRMED').length;
        const arrived = promoter.guestLists.filter((g) => g.status === 'ARRIVED').length;
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getRealTimeCapacity(venueId) {
        const venue = await prisma.bookingVenue.findUnique({ where: { id: venueId } });
        if (!venue)
            return null;
        const now = new Date();
    ...
    async addToWaitlist(venueId, userId, partySize) {
        return prisma.guestList.create({
            data: {
                userId,
                venueId,
                ...
                confirmationCode: `WL-${crypto_1.default.randomBytes(3).toString('hex').toUpperCase()}`,
                date: new Date(),
            },
        });
    },
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async calculatePromoterPayout(promoterId, venueId, date) {
...
//# sourceMappingURL=nightlifeIntegration.service.js.map