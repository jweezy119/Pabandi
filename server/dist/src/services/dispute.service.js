...
const trustArbitrator_service_1 = require("./trustArbitrator.service");
const pabTokenStaking_service_1 = require("./pabTokenStaking.service");
const logger_1 = require("../utils/logger");
const prisma = new client_1.PrismaClient();
const reliabilityService = { updateScoreForReservationActivity: async (..._args) => null, getUserReliabilityProfile: async () => null };
class DisputeService {
     ...
     */
    async createDispute(reservationId, reportedById, userId, description, evidenceUrls, stakedAmount = 10) {
        ...
        const dispute = await prisma.dispute.create({
            data: {
                ...
                reportedById,
                userId,
                type: client_1.DisputeType.QUALITY_ISSUE, // default type for now
                description,
                evidenceUrls,
                outcome: client_1.DisputeOutcome.PENDING,
                stakedAmount
            }
        });
        ...
        if (claimAmount < 500) {
            ...
            const arbitrationResult = await trustArbitrator_service_1.trustArbitratorService.arbitrate(arbitratorEvidence);
            if (!arbitrationResult.needsHumanReview) {
                // AI resolved it — apply outcome
                const outcome = arbitrationResult.ruling === 'BUYER_WINS' ? client_1.DisputeOutcome.UPHELD
                    : arbitrationResult.ruling === 'SELLER_WINS' ? client_1.DisputeOutcome.DISMISSED
                        : client_1.DisputeOutcome.RESOLVED; // REFUND_HALF
                await this.resolveDispute(dispute, outcome);
                // Apply $PAB rewards/slashes
                if (arbitrationResult.pabReward) {
                    await pabTokenStaking_service_1.pabTokenStakingService.rewardUser(arbitrationResult.pabReward.recipient, 'DISPUTE_WON').catch(() => { });
                }
                if (arbitrationResult.pabSlash) {
                    // Slashing is handled by the staking service via dispute resolution
                    logger_1.logger.info(`[Dispute] AI arbitrator slashed ${arbitrationResult.pabSlash.amount} $PAB from user ${arbitrationResult.pabSlash.target}`);
                }
                logger_1.logger.info(`[Dispute] AI arbitration resolved dispute ${dispute.id}: ${arbitrationResult.ruling} (confidence: ${arbitrationResult.confidence})`);
            }
            else {
                logger_1.logger.info(`[Dispute] Dispute ${dispute.id} escalated to human jury (AI confidence: ${arbitrationResult.confidence})`);
            }
        }
        return dispute;
    }
    ...
    async fileContextDispute(opts) {
        ...
        if (existing)
            throw new Error('A dispute for this context already exists.');
        const dispute = await prisma.dispute.create({
            data: {
                reservationId: contextType === 'RESERVATION' ? contextId : undefined,
                reportedById,
                userId: againstId,
                type: opts.type || client_1.DisputeType.QUALITY_ISSUE,
                description,
                evidenceUrls,
                outcome: client_1.DisputeOutcome.PENDING,
                stakedAmount,
        ...
        if (claimAmount && claimAmount < 500) {
            const arb = await trustArbitrator_service_1.trustArbitratorService.arbitrate({
                disputeId: dispute.id,
                claimAmount,
                currency: 'USD',
                customerId: dispute.userId ?? undefined,
            ...
            if (!arb.needsHumanReview) {
                const outcome = arb.ruling === 'BUYER_WINS' ? client_1.DisputeOutcome.UPHELD
                    : arb.ruling === 'SELLER_WINS' ? client_1.DisputeOutcome.DISMISSED
                        : client_1.DisputeOutcome.RESOLVED;
                await this.resolveDispute(dispute, outcome);
            }
        }
        return dispute;
    ...
    async resolveClaimAmount(contextType, contextId) {
     ...
     */
    async castVote(disputeId, jurorId, voteForId, reason) {
        ...
        if (!dispute || dispute.outcome !== client_1.DisputeOutcome.PENDING) {
     ...
     */
    async checkAndResolveDispute(disputeId) {
        ...
        for (const vote of votes) {
            if (vote.voteForId === dispute.reportedById)
                votesForReporter++;
            if (vote.voteForId === dispute.userId)
                votesForUser++;
        }
        // Threshold logic: first to 3 votes wins (out of 5 possible jurors)
        if (votesForReporter >= 3) {
            await this.resolveDispute(dispute, client_1.DisputeOutcome.UPHELD);
        }
        else if (votesForUser >= 3) {
            await this.resolveDispute(dispute, client_1.DisputeOutcome.DISMISSED);
        }
    }
    async assignJurors(disputeId, excludeIds = []) {
        ...
        const assignments = toAssign.map(jurorId => prisma.juryVote.create({
            data: {
                disputeId,
                jurorId,
                voteForId: '',
    ...
    async resolveDispute(dispute, outcome) {
        ...
        if ((dispute.contextType === 'MILESTONE' || dispute.contextType === 'OFFRAMP' || dispute.contextType === 'PAYOUT')
            && outcome === client_1.DisputeOutcome.UPHELD) {
            await this.clawbackContext(dispute);
        }
        // Apply trust score penalties based on outcome
        if (outcome === client_1.DisputeOutcome.UPHELD) {
        ...
        else if (outcome === client_1.DisputeOutcome.DISMISSED) {
        ...
        // Log on-chain
        if (dispute.reportedById) {
            await blockchain_service_1.blockchainService.logTrustAttestationOnSolana(dispute.reportedById, dispute.reservationId || 'UNKNOWN', 'DISPUTE_FILED', {
                outcome: outcome
            });
        }
        ...
        try {
            if (dispute.contextType === 'MILESTONE') {
                ...
                if (wp) {
                    ...
                    logger_1.logger.info(`[Dispute] UPHELD milestone dispute ${dispute.id} — clawed back $${wp.netUsdc} from worker ${wp.userId}`);
                }
            }
                ...
                if (p && p.status === 'SETTLED') {
                    ...
                    logger_1.logger.info(`[Dispute] UPHELD payout dispute ${dispute.id} — reversed $${p.netUsdc} payout ${p.id}`);
                }
            }
        }
        catch (e) {
            logger_1.logger.error(`[Dispute] clawback failed for ${dispute.id}: ${e.message}`);
        }
    }
}
