...
function decrypt(text) {
    ...
    const decipher = crypto_1.default.createDecipheriv(ALGORITHM, getEncKey(), iv);
...
exports.walletService = {
    async createWallet(userId) {
        try {
            ...
            if (existing)
                return { success: true, wallet: existing, created: false };
    ...
    async getWallet(userId) {
        try {
            ...
            logger_1.logger.error(`Failed to get wallet: ${error.message}`);
            return { success: false, message: 'Failed to get wallet' };
        }
    },
    async updateBalance(userId, amount, currency = 'PAB') {
        try {
            ...
            if (currency === 'PAB')
                ...
                updateData.usdcBalance = wallet.usdcBalance + amount;
            const updated = await prisma.wallet.update({ where: { userId }, data: updateData });
            return { success: true, wallet: updated };
        }
        catch (error) {
            logger_1.logger.error(`Failed to update balance: ${error.message}`);
            return { success: false, message: 'Failed to update balance' };
        }
    },
    async claimAirdrop(userId) {
        try {
            const wallet = await prisma.wallet.findUnique({ where: { userId } });
            if (!wallet)
                return { success: false, message: 'Wallet not found' };
            if (wallet.airdropClaimed)
            ...
            logger_1.logger.error(`Failed to claim airdrop: ${error.message}`);
