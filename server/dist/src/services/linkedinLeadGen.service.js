...
exports.linkedinLeadGenService = exports.LinkedInLeadGenService = exports.CONTENT_TEMPLATES = exports.LINKEDIN_PERSONAS = void 0;
/**
...
const trustVeil_service_1 = require("./trustVeil.service");
const crypto_1 = __importDefault(require("crypto"));
exports.LINKEDIN_PERSONAS = [
    {
        id: 'freelance-dev',
...
class LinkedInLeadGenService {
     ...
     */
    generateIcebreaker(persona, firstName, techStack, mutualConnections) {
     ...
     */
    generateLinkedInPost(persona, contentType) {
        const templates = exports.CONTENT_TEMPLATES[contentType];
     ...
     */
    scheduleContentBatch(personas, count = 3, startDate = new Date()) {
        const scheduled = [];
        const contentTypes = ['trustScore', 'revenue', 'socialProof'];
        for (let i = 0; i < count; i++) {
            ...
            logger_1.logger.info(`[LinkedIn] Scheduled post for ${persona.id} at ${scheduledAt.toISOString()}`);
        }
        return scheduled;
    }
    /**
     ...
     */
    async captureLead(linkedinId, linkedinName, linkedinUrl, email, persona, source) {
        const leadId = `lead_${crypto_1.default.randomBytes(6).toString('hex')}`;
        const lead = {
            leadId,
            linkedinId,
            linkedinName,
        ...
        try {
            await database_1.prisma.trustAuditTrail.create({
                data: {
                    ...
                    severity: 'positive',
                    currentHash: crypto_1.default.randomBytes(32).toString('hex'),
                    metadata: {
                        leadId,
                        linkedinId,
                        persona,
            ...
            logger_1.logger.warn(`[LinkedInLeadGen] Audit trail write skipped: ${dbErr.message}`);
        }
        logger_1.logger.info(`[LinkedInLeadGen] Captured lead ${leadId} (${persona}) from ${source}`);
        return lead;
    }
    /**
     ...
     */
    async runDMCampaign(persona, prospects) {
        let sent = 0;
        let failed = 0;
        const errors = [];
        for (const prospect of prospects) {
            try {
                ...
                const message = this.generateIcebreaker(persona, prospect.firstName, prospect.techStack, prospect.mutualConnections);
                ...
                sent++;
                logger_1.logger.info(`[LinkedInLeadGen] DM sent to ${prospect.firstName} — "${message.substring(0, 60)}..."`);
                ...
                logger_1.logger.error(`[LinkedInLeadGen] DM failed for ${prospect.firstName}: ${err.message}`);
            }
        }
        logger_1.logger.info(`[LinkedInLeadGen] Campaign complete: ${sent} sent, ${failed} failed (${persona.name})`);
     ...
     */
    async convertLead(leadId, paymentMethod) {
        ...
        try {
            ...
            const businessId = `biz_${crypto_1.default.randomBytes(6).toString('hex')}`;
            // Auto-generate a TrustFlux result for the new user
            // (they'll have no history, so it's a fresh start)
            const defaultFlux = await trustFlux_service_1.trustFluxService.computeTrustFlux(businessId);
            // Start with a TrustVeil proof at baseline score
            const baselineScore = 50; // everyone starts at 50
            const proof = await trustVeil_service_1.trustVeilService.issueProof(businessId, baselineScore, 40);
            ...
            logger_1.logger.info(`[LinkedInLeadGen] Converted lead ${leadId} → business ${businessId}`);
            return { success: true, businessId, freeTrialDays: 7 };
        }
        catch (err) {
            logger_1.logger.error(`[LinkedInLeadGen] Lead conversion failed: ${err.message}`);
            return { success: false, businessId: '', freeTrialDays: 0 };
        }
    }
    ...
    async scrapeProfile(linkedinUrl) {
        ...
        logger_1.logger.info(`[LinkedInLeadGen] Scraped profile: ${linkedinUrl}`);
        return profile;
    }
    /** Generate a role-specific LinkedIn post targeting hiring managers. */
    generateRolePost(roleTitle, industry, companyName) {
...
#Pabandi #Hiring #Freelancers #NoMoreGhosting #${industry.replace(/\s+/g, '')}`;
    ...
    async runFullFunnel(personas, postCount = 3, dmLimit = 50) {
        const errors = [];
        let dmsToSend = 0;
        let leadsCaptured = 0;
        try {
            ...
            logger_1.logger.info(`[LinkedInLeadGen] Scheduled ${scheduled.length} posts`);
            ...
            for (let i = 0; i < Math.min(dmLimit, 50); i++) {
                ...
                const lead = await this.captureLead(`linkedin_${firstName.toLowerCase()}_${i}`, firstName, `https://linkedin.com/in/${firstName.toLowerCase()}-${i}`, `user${i}@example.com`, persona.id, 'linkedin-dm');
                dmsToSend++;
                leadsCaptured++;
            }
        }
...
exports.LinkedInLeadGenService = LinkedInLeadGenService;
exports.linkedinLeadGenService = new LinkedInLeadGenService();
//# sourceMappingURL=linkedinLeadGen.service.js.map