...
exports.nightlifeTokenomicsService = void 0;
...
exports.nightlifeTokenomicsService = {
    ...
    async rewardGuestReview(userId, reviewId, isFirstReview) {
    ...
    async rewardGuestReferral(referrerId, referredUserId) {
        await prisma.agentTransaction.create({
            data: {
                userId: referrerId,
    ...
    async rewardBottlePurchase(userId, amount, bottleReservationId) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async stakePromoterTier(userId, tier) {
    ...
    async unstakePromoterTier(userId) {
        const existingStake = await prisma.agentStake.findFirst({
            where: { agentId: userId, type: 'PROMOTER_TIER', txStatus: 'ACTIVE' },
        });
        if (!existingStake)
            return { error: 'No active stake found' };
        await prisma.agentStake.update({
            where: { id: existingStake.id },
            data: { txStatus: 'UNSTAKED', unstakedAt: new Date() },
        });
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async payVenueSubscription(venueId, amountUsd) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async depositGuestListSpot(userId, guestListId, amountPab) {
        await prisma.agentTransaction.create({
            data: {
                agentId: userId,
    ...
    async returnGuestListDeposit(userId, guestListId, showUp) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    async getTokenomicsStats(period = 'week') {
        const now = new Date();
        ...
        for (const tx of transactions) {
            if (tx.amount > 0)
                stats.totalDistributed += tx.amount;
            else
                stats.totalCollected += Math.abs(tx.amount);
            if (tx.type.includes('STAKE'))
                stats.totalStaked += Math.abs(tx.amount);
    ...
    async getUserNightlifeBalance(userId) {
...
//# sourceMappingURL=nightlifeTokenomics.service.js.map