...
exports.bottleBookingService = void 0;
const logger_1 = require("../utils/logger");
const database_1 = require("../utils/database");
const email_service_1 = require("./email.service");
exports.bottleBookingService = {
    /**
    ...
    async createBooking(data) {
        ...
        const table = await database_1.prisma.tableType.findUnique({
        ...
        const bottlePackage = await database_1.prisma.bottlePackage.findUnique({
        ...
        const existingReservations = await database_1.prisma.bottleReservation.findMany({
            where: {
                tableTypeId,
        ...
        if (venue.coverCharges && venue.coverCharges.length > 0) {
            ...
            const applicableCharge = venue.coverCharges.find((c) => {
                if (!c.daysOfWeek.includes(dayOfWeek))
                    return false;
        ...
        if (promoCode) {
            promoCodeRecord = await database_1.prisma.promoCode.findUnique({
        ...
        const booking = await database_1.prisma.bottleReservation.create({
            data: {
                userId,
                venueId,
                tableTypeId,
        ...
        try {
            await email_service_1.emailService.sendBookingConfirmation(booking);
        }
        catch (err) {
            logger_1.logger.warn(`Failed to send booking confirmation email: ${err.message}`);
        }
        return booking;
    },
    /**
     ...
     */
    async calculateDynamicPricing(venueId, date) {
        const targetDate = new Date(date);
        const dayOfWeek = targetDate.getDay();
        let multiplier = 1.0;
        ...
        // Check for events
        const events = await database_1.prisma.nightlifeEvent.findMany({
            where: {
                venueId,
                date: {
        ...
        const reservations = await database_1.prisma.bottleReservation.count({
            where: {
                venueId,
                date: {
                    gte: new Date(targetDate.setHours(0, 0, 0, 0)),
        ...
        const venue = await database_1.prisma.bookingVenue.findUnique({ where: { id: venueId } });
    ...
    async confirmBooking(bookingId) {
        const booking = await database_1.prisma.bottleReservation.findUnique({
            where: { id: bookingId },
        });
        if (!booking) {
        ...
        return database_1.prisma.bottleReservation.update({
            where: { id: bookingId },
    ...
    async getUserBookings(userId) {
        return database_1.prisma.bottleReservation.findMany({
            where: { userId },
            include: {
                venue: {
    ...
    async getBookingById(bookingId) {
        return database_1.prisma.bottleReservation.findUnique({
            where: { id: bookingId },
            include: {
                venue: true,
                tableType: true,
     ...
     */
    async checkInBooking(bookingId, staffId) {
        const booking = await database_1.prisma.bottleReservation.findUnique({
            where: { id: bookingId },
        });
        if (!booking) {
     ...
     */
    async creditPromoter(promoterId, amount, bookingId) {
        const promoter = await database_1.prisma.promoter.findUnique({
            where: { id: promoterId },
        });
        if (!promoter) {
            logger_1.logger.warn(`Promoter ${promoterId} not found for commission credit`);
            return null;
        }
        // Create a ledger entry for the commission
        // Note: Using ReferralLedger as the wallet/commission tracking mechanism
        const ledgerEntry = await database_1.prisma.referralLedger.create({
            data: {
        ...
        try {
            await email_service_1.emailService.sendPromoterCommissionNotification(promoter, amount, bookingId);
        }
        catch (err) {
            logger_1.logger.warn(`Failed to send promoter commission notification: ${err.message}`);
        }
        return ledgerEntry;
    },
};
//# sourceMappingURL=bottleBooking.service.js.map