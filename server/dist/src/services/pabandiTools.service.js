...
exports.accessLabel = exports.pabandiToolsRegistry = void 0;
exports.pabandiPlatformDoc = pabandiPlatformDoc;
exports.toolAccessOk = toolAccessOk;
/**
...
function pabandiPlatformDoc() {
    const tools = exports.pabandiToolsRegistry.map((t) => ({
        ...t,
    ...
    return {
        ...
        mcp: {
            ...
            tools: exports.pabandiToolsRegistry.filter((t) => t.category !== 'sdk' && !t.exclusiveNote)
                .map((t) => ({
...
async function toolAccessOk(toolName, req, options) {
    // Normalize tool name: accept "pabandi:short", "pabandi_short", or raw "name"
    const normalized = toolName
        .replace(/^pabandi[:_]/, '')
        .replace(/_/g, ' ')
        .toLowerCase();
    const mcptool = exports.pabandiToolsRegistry.find((t) => {
    ...
    if (mcptool.access === 'public')
        return { ok: true, note: 'public tool' };
    if (mcptool.access === 'owner' && req?.user?.id)
        return { ok: true, note: 'owner-authenticated' };
    if (mcptool.access === 'verified' && (req?.user?.role === 'BUSINESS_OWNER' || options?.verifiedRail)) {
        if (options?.businessId) {
            const conns = await database_1.prisma.openFinanceConnection.findMany({ where: { businessId: options.businessId } });
            if (conns.some((c) => c.status === 'VERIFIED'))
                return { ok: true, note: 'business has attested rail' };
        }
    ...
    if (mcptool.access === 'exclusive') {
        const user = req?.user ? await database_1.prisma.user.findUnique({ where: { id: req.user.id }, select: { trustScore: true } }) : null;
        const band = user ? ptp_spec_1.ptpEngine.scoreToRiskBand(user.trustScore ?? 70) : 'E';
        if (band === 'A' || band === 'B')
            return { ok: true, note: `PTP Band ${band} — exclusive rail unlocked` };
...
//# sourceMappingURL=pabandiTools.service.js.map