...
exports.osintMCPClient = exports.OsintMCPClientService = void 0;
const logger_1 = require("../../utils/logger");
const courtListener_service_1 = require("./courtListener.service");
/**
 ...
 */
class OsintMCPClientService {
    /**
     * 1. Maigret MCP - Individual & Identity Fraud
     * Cross-references usernames across social media and forums.
     */
    async queryMaigretMCP(username) {
        logger_1.logger.info(`[MCP Client] Querying Maigret MCP for username: ${username}`);
     ...
     */
    async queryOpenRegistryMCP(businessName) {
        logger_1.logger.info(`[MCP Client] Querying OpenRegistry MCP for business: ${businessName}`);
     ...
     */
    async queryInfrastructurePipeline(domain) {
        logger_1.logger.info(`[MCP Client] Orchestrating Infrastructure Pipeline for domain: ${domain}`);
        const results = [];
        // A. WHOIS MCP
     ...
     */
    async queryBrightDataMCP(keyword) {
        logger_1.logger.info(`[MCP Client] Querying Bright Data MCP for keyword: ${keyword}`);
     ...
     */
    async queryCourtListenerMCP(name, state) {
        logger_1.logger.info(`[MCP Client] Querying CourtListener MCP for entity: ${name}`);
        const clData = await courtListener_service_1.courtListenerService.searchCivilLitigation(name, state);
        // Analyze findings
        const evictionMatches = clData.results.filter(r => r.natureOfSuit?.toLowerCase().includes('evict') ||
...
exports.OsintMCPClientService = OsintMCPClientService;
exports.osintMCPClient = new OsintMCPClientService();
//# sourceMappingURL=osintMCPClient.service.js.map