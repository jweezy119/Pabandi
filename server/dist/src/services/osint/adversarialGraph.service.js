...
exports.adversarialGraphService = exports.AdversarialGraphService = void 0;
const logger_1 = require("../../utils/logger");
const database_1 = require("../../utils/database");
const osintMCPClient_service_1 = require("./osintMCPClient.service");
class AdversarialGraphService {
    constructor() {
        this.nodes = new Map();
        this.edges = [];
    }
     ...
     */
    async buildConstellation(seedId, seedType, seedLabel, maxDepth = 3) {
        logger_1.logger.info(`[AdversarialGraph] Building constellation from seed: ${seedLabel} (${seedType})`);
        this.nodes.clear();
        this.edges = [];
        // Add seed node
        this.addNode({
        ...
        logger_1.logger.info(`[AdversarialGraph] Constellation ${constellationId} complete: ${report.totalNodes} nodes, ${report.totalEdges} edges, threat=${report.threatLevel}`);
        return report;
    }
    /**
        ...
        try {
            switch (node.type) {
                case 'IDENTITY': {
                    ...
                    const maigret = await osintMCPClient_service_1.osintMCPClient.queryMaigretMCP(node.label);
                ...
                case 'DOMAIN': {
                    ...
                    const infraResults = await osintMCPClient_service_1.osintMCPClient.queryInfrastructurePipeline(node.label);
                ...
                case 'DEVICE': {
                    ...
                    const clusters = await database_1.prisma.identityCluster.findMany({
                        where: {
            ...
            logger_1.logger.warn(`[AdversarialGraph] Expansion failed for ${node.id}: ${err.message}`);
        }
        return discoveries;
    }
    /**
    ...
    computeContagion() {
        ...
        for (const unflagged of unflaggedNodes) {
            ...
            const visited = new Set([unflagged.id]);
            const queue = [{ id: unflagged.id, hops: 0 }];
     ...
     */
    predictActivations(communities) {
        const predictions = [];
        for (const community of communities) {
            if (community.flaggedRatio <= 0.3)
                continue; // Skip low-threat communities
            for (const nodeId of community.nodes) {
                const node = this.nodes.get(nodeId);
                if (!node || node.flagged)
                    continue; // Only predict for unflagged nodes
     ...
     */
    assessThreatLevel(communities, contagionNodes) {
        ...
        if (maxFlaggedRatio > 0.7 || highContagionCount > 5 || totalNodes > 20)
            return 'CRITICAL';
        if (maxFlaggedRatio > 0.5 || highContagionCount > 2 || totalNodes > 10)
            return 'HIGH';
        if (maxFlaggedRatio > 0.3 || highContagionCount > 0)
            return 'MEDIUM';
        return 'LOW';
    }
    // ── Utility Methods ──────────────────────────────────────────────────
...
exports.AdversarialGraphService = AdversarialGraphService;
exports.adversarialGraphService = new AdversarialGraphService();
//# sourceMappingURL=adversarialGraph.service.js.map