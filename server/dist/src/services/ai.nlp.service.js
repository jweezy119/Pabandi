...
function validateAndSanitizeInput(message) {
    ...
    for (const pattern of INJECTION_PATTERNS) {
        if (pattern.test(trimmed)) {
            logger_1.logger.warn('[AI NLP] Potential prompt injection detected', {
...
function validateClassificationOutput(data) {
    if (!data || typeof data !== 'object')
        return false;
    if (!['booking', 'support', 'cancellation', 'general', 'sales', 'book_table', 'check_menu', 'ask_question'].includes(data.intent))
        return false;
...
class AiNlpService {
    ...
    async classifyIntentAndLanguage(message) {
        ...
        if (validation.blocked) {
            logger_1.logger.warn('[AI NLP] Input blocked', { reason: validation.reason, messageLength: message.length });
            return {
                intent: 'general',
                language: 'en',
                sentiment: 'neutral',
  ...
  "confidence": (number between 0 and 1)
        ...
        try {
            ...
            if (content) {
                ...
                if (validateClassificationOutput(parsed)) {
                    logger_1.logger.info('[AI NLP] Classification result', { intent: parsed.intent, language: parsed.language, confidence: parsed.confidence });
                    return parsed;
                }
                logger_1.logger.warn('[AI NLP] Invalid classification output from LLM', { raw: jsonStr.substring(0, 200) });
            }
        }
        catch (error) {
            ...
            logger_1.logger.error('[AI NLP] Error classifying message:', error.response?.data || error.message);
        }
        const entities = extractBookingEntities(cleanMessage);
        return {
            intent: entities.intent === 'book_table' ? 'booking' : entities.intent === 'check_menu' || entities.intent === 'ask_question' ? 'support' : 'general',
    ...
    async generateCopy(template, contextVars) {
        ...
        if (template.length > 500) {
            logger_1.logger.warn('[AI NLP] Template too long, truncating', { length: template.length });
...
`;
        try {
            ...
            logger_1.logger.error('[AI NLP] Error generating copy:', error.response?.data || error.message);
