...
function sanitizeTemplateVars(targetAudience, goal, sampleLeadNames) {
    ...
    for (const pattern of DANGEROUS_INJECTION_PATTERNS) {
        if (pattern.test(targetAudience) || pattern.test(goal)) {
            logger_1.logger.warn('[AI Campaign] Prompt injection attempt detected in campaign inputs');
            return {
    ...
    const safeNames = sampleLeadNames.slice(0, MAX_NAMES_LENGTH).map((n) => typeof n === 'string' ? n.substring(0, 50) : String(n));
...
Write a single, high-converting WhatsApp message template for this campaign. Make sure to use '{name}' where the customer's name should be.
    `;
        try {
            ...
            const response = await axios_1.default.post('https://dashscope.aliyuncs.com/api/v1/services/aigc/text-generation/generation', payload, {
                headers: {
            ...
            logger_1.logger.error('[AI Campaign] Error generating copy:', error.response?.data || error.message);
