...
exports.agentOrchestrator = exports.revenueMaxService = exports.guestFinderService = exports.venueBrainService = exports.promoterAutonService = void 0;
...
exports.promoterAutonService = {
    ...
    async perceive(context) {
        ...
        const noShowRate = recentLists.filter((g) => g.status === 'NO_SHOW').length / Math.max(recentLists.length, 1);
        const arrivalRate = recentLists.filter((g) => g.status === 'ARRIVED').length / Math.max(recentLists.length, 1);
        // Detect anomalies
        if (noShowRate > 0.5)
            anomalies.push('HIGH_NO_SHOW_RATE');
        if (arrivalRate > 0.9)
            signals.push('HIGH_PERFORMER');
        if (recentLists.length > 10 && noShowRate < 0.2)
            signals.push('ELITE_PROMOTER');
        // Get venue relationships
        const venueIds = [...new Set(recentLists.map((g) => g.venueId))];
    ...
    async reason(perception, context) {
        ...
        if (noShowRate > 0.4) {
            actions.push({
                ...
                payload: {
                    ...
                    targetGuests: recentLists.filter((g) => g.noShowProbability > 0.4).map((g) => g.userId),
                },
                priority: 'HIGH',
        ...
        for (const event of upcomingEvents) {
            ...
            const totalPartySize = eventGuestLists.reduce((sum, g) => sum + g.partySize, 0);
        ...
        const highValueGuests = recentLists
            .filter((g) => g.status === 'ARRIVED' && g.noShowProbability < 0.2)
            .slice(0, 10);
        if (highValueGuests.length >= 3) {
            actions.push({
                type: 'RECRUIT_LOOKALIKE_GUESTS',
                payload: {
                    sourceGuestIds: highValueGuests.map((g) => g.userId),
                    targetEventIds: upcomingEvents.map((e) => e.id),
        ...
        const highRiskGuests = recentLists.filter((g) => g.noShowProbability > 0.6 && g.status === 'CONFIRMED');
        if (highRiskGuests.length > 0) {
            actions.push({
                type: 'SEND_SMART_REMINDERS',
                payload: {
                    guestIds: highRiskGuests.map((g) => g.id),
    ...
    async executeDepositAdjustment(payload) {
    ...
    async executeGuestRecruitment(payload) {
    ...
    async executeSmartReminders(payload) {
...
exports.venueBrainService = {
    ...
    async perceive(context) {
        ...
        const todayArrivals = venue.guestLists?.filter((g) => g.status === 'ARRIVED').length || 0;
        const capacityUsed = venue.capacity > 0 ? todayArrivals / venue.capacity : 0;
        // Revenue analysis
        const bottleRevenue = venue.bottleReservations?.reduce((sum, r) => sum + r.totalPrice, 0) || 0;
        const estimatedDoorRevenue = todayArrivals * 20; // Avg $20 cover
        // Detect anomalies
        if (capacityUsed > 0.95)
            signals.push('AT_CAPACITY');
        if (capacityUsed < 0.3)
            anomalies.push('LOW_ATTENDANCE');
        if (bottleRevenue > 10000)
            signals.push('HIGH_BOTTLE_REVENUE');
        // Get promoter performance data
        const promoterIds = [...new Set(venue.guestLists?.map((g) => g.userId) || [])];
    ...
    async reason(perception, context) {
        ...
        const topPromoters = promoterScores.filter((p) => p.avgRating >= 4 && p.fraudRisk < 30);
        const riskyPromoters = promoterScores.filter((p) => p.fraudRisk >= 50);
        if (topPromoters.length > 0) {
            actions.push({
                ...
                payload: {
                    venueId: venue.id,
                    tier: 'PREFERRED',
                    promoterIds: topPromoters.map((p) => p.id),
        ...
        if (riskyPromoters.length > 0) {
            actions.push({
                ...
                payload: {
                    venueId: venue.id,
                    tier: 'RESTRICTED',
                    promoterIds: riskyPromoters.map((p) => p.id),
    ...
    async executeCoverChargeChange(payload) {
    ...
    async executePromoterTierAssignment(payload) {
...
exports.guestFinderService = {
    ...
    async perceive(context) {
        ...
        const guestSegments = {
            vip: recentGuests.filter((g) => g.noShowProbability < 0.2 && g.status === 'ARRIVED'),
            regular: recentGuests.filter((g) => g.noShowProbability >= 0.2 && g.noShowProbability < 0.5),
            risky: recentGuests.filter((g) => g.noShowProbability >= 0.5),
            churned: recentGuests.filter((g) => g.status === 'CANCELLED' || g.status === 'NO_SHOW'),
        };
        // Detect trends
        if (guestSegments.vip.length > 20)
            signals.push('STRONG_VIP_BASE');
        if (guestSegments.churned.length > guestSegments.vip.length)
            anomalies.push('HIGH_CHURN_RATE');
        return {
            timestamp: new Date(),
    ...
    async reason(perception, context) {
        ...
        if (guestSegments.churned.length > 5) {
            actions.push({
                ...
                payload: {
                    guestIds: guestSegments.churned.slice(0, 20).map((g) => g.userId),
        ...
        if (guestSegments.vip.length >= 3) {
            actions.push({
                ...
                payload: {
                    guestIds: guestSegments.vip.map((g) => g.userId),
        ...
        if (guestSegments.risky.length > 0) {
            actions.push({
                ...
                payload: {
                    guestIds: guestSegments.risky.map((g) => g.userId),
                    warningLevel: 'MEDIUM',
    ...
    async executeReengagement(payload) {
    ...
    async executeAmbassadorUpgrade(payload) {
    ...
    async executeRiskWarning(payload) {
...
exports.revenueMaxService = {
    ...
    async perceive(context) {
        ...
        const totalRevenue = thisWeek.reduce((sum, a) => sum + a.totalRevenue, 0);
        const avgSpend = thisWeek.reduce((sum, a) => sum + a.avgSpendPerGuest, 0) / Math.max(thisWeek.length, 1);
        // Detect trends
        if (totalRevenue > 50000)
            signals.push('HIGH_REVENUE_WEEK');
        if (avgSpend < 50)
            anomalies.push('LOW_SPEND_PER_GUEST');
        return {
            timestamp: new Date(),
    ...
    async executeBottlePriceIncrease(payload) {
    ...
    async executeUpsellPromotion(payload) {
...
exports.agentOrchestrator = {
    async runAllAgents(context) {
        const results = await Promise.allSettled([
            exports.promoterAutonService.runLoop({ ...context, agentId: `promoter_${Date.now()}`, agentType: 'PROMOTER_AUTON' }),
            exports.venueBrainService.runLoop({ ...context, agentId: `venue_${Date.now()}`, agentType: 'VENUE_BRAIN' }),
            exports.guestFinderService.runLoop({ ...context, agentId: `guest_${Date.now()}`, agentType: 'GUEST_FINDER' }),
            exports.revenueMaxService.runLoop({ ...context, agentId: `revenue_${Date.now()}`, agentType: 'REVENUE_MAX' }),
        ]);
    ...
    detectConflicts(agentResults) {
        const conflicts = [];
        const allActions = agentResults.flatMap((r) => r.actions || []);
        // Detect price conflicts (two agents adjusting same price)
        const priceActions = allActions.filter((a) => a.type.includes('PRICE') || a.type.includes('COVER'));
...
//# sourceMappingURL=agentic.service.js.map