...
exports.processWhatsAppMessage = exports.findBusinessByPublicPhone = exports.sendWhatsAppMessage = void 0;
...
const sendWhatsAppMessage = async (toPhone, message, options) => {
    if (!process.env.OPENWA_API_KEY && !process.env.EVOLUTION_API_KEY) {
        logger_1.logger.debug(`[WhatsApp MOCK] To: ${toPhone}`);
        return;
    }
    try {
        ...
        logger_1.logger.info(`[WhatsApp] Sent message via Provider to ${toPhone} (ID: ${result.messageId})`);
        return result.messageId;
    }
    catch (error) {
        logger_1.logger.error(`[WhatsApp] Error sending message to ${toPhone}:`, error.response?.data || error.message);
        return null;
    }
};
...
const processWhatsAppMessage = async (customerPhone, businessPhone, message, user) => {
    logger_1.logger.info(`[AI] Processing message from ${customerPhone} to ${businessPhone}`);
    ...
    if (openwa_drop_bot_service_1.openwaDropBotService.isDropEngineCommand(message)) {
        try {
            ...
            logger_1.logger.error('[Drop Bot Error]', e);
        }
    }
    ...
    try {
        ...
        logger_1.logger.debug('[AI NLP] Classification result', classification);
        ...
        logger_1.logger.error('[Plugin] Pre-AI plugin handling failed:', pluginErr);
    }
};
...
async function handleWhatsAppCancellation(phoneNumber, user) {
    try {
        ...
        if (reservation.depositPaid) {
            if (reservation.cryptoDepositTxHash && !reservation.cryptoDepositTxHash.startsWith('pending_')) {
                try {
                    ...
                    logger_1.logger.error('[WhatsApp Cancel] Failed to trigger crypto refund', e);
                }
            }
            else {
        ...
        logger_1.logger.error('[WhatsApp Cancel Error]:', error);
