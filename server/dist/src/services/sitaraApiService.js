...
exports.__test__ = exports.BusinessCategory = void 0;
...
async function searchPOI(query, lat, lng, radius) {
    return rateLimited(async () => {
        try {
            const axios = (await Promise.resolve().then(() => __importStar(require('axios')))).default;
            const params = {
...
async function discoverBusinesses(lat, lng, radius, category) {
    try {
    ...
    `;
        const axios = (await Promise.resolve().then(() => __importStar(require('axios')))).default;
        const response = await axios.post(OVERPASS_BASE, `data=${encodeURIComponent(query)}`, {
            headers: {
...
async function getRoute(from, to) {
    try {
        const axios = (await Promise.resolve().then(() => __importStar(require('axios')))).default;
        const apiKey = process.env.OPENROUTESERVICE_API_KEY;
        const response = await axios.post(`${ORS_BASE}/v2/directions/driving-car`, {
            coordinates: [
                [from.lng, from.lat],
                [to.lng, to.lat],
            ],
...
async function getIsochrone(lat, lng, minutes) {
    try {
        const axios = (await Promise.resolve().then(() => __importStar(require('axios')))).default;
        const apiKey = process.env.OPENROUTESERVICE_API_KEY;
        const response = await axios.post(`${ORS_BASE}/v2/isochrones/driving-car`, {
            locations: [[lng, lat]],
...
async function sendBookingConfirmation(data) {
    ...
    if (!apiKey) {
        console.log(`[SitaraApi] RESEND_API_KEY not set — booking confirmation email logged to console only.\n` +
...
// ── Resend: Promo email ───────────────────────────────────────────────────
async function sendPromoEmail(data) {
    const apiKey = process.env.RESEND_API_KEY;
    if (!apiKey) {
        console.log(`[SitaraApi] RESEND_API_KEY not set — promo email logged to console only.\n` +
...
exports.__test__ = { cacheGet, cacheSet, geocodeCache };
//# sourceMappingURL=sitaraApiService.js.map