...
class WebhookService {
    ...
    async create(managerId, url, events, secret) {
        return database_1.prisma.webhook.create({
            data: { managerId, url, events, secret },
        });
    }
    async list(managerId) {
        return database_1.prisma.webhook.findMany({ where: { managerId }, orderBy: { createdAt: 'desc' } });
    }
    async update(managerId, webhookId, data) {
        const webhook = await database_1.prisma.webhook.findFirst({ where: { id: webhookId, managerId } });
        if (!webhook)
            throw new Error('Webhook not found');
        return database_1.prisma.webhook.update({ where: { id: webhookId }, data });
    }
    async delete(managerId, webhookId) {
        const webhook = await database_1.prisma.webhook.findFirst({ where: { id: webhookId, managerId } });
        if (!webhook)
            throw new Error('Webhook not found');
        await database_1.prisma.webhook.delete({ where: { id: webhookId } });
        return { success: true };
    }
}
