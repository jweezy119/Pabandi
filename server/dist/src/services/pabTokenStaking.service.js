...
exports.pabTokenStakingService = exports.PabTokenStakingService = exports.REWARD_RATES = exports.SLASH_RATES = exports.STAKING_TIERS = void 0;
/**
...
class PabTokenStakingService {
    ...
    getTier(totalStaked) {
        ...
        for (const [tier, config] of Object.entries(exports.STAKING_TIERS)) {
     ...
     */
    async getTrustMultiplier(userId) {
        try {
            const activeStake = await database_1.prisma.stakingPosition.findMany({
            ...
            logger_1.logger.warn('[PabStaking] Failed to compute multiplier for user', userId, error);
     ...
     */
    async stakeTokens(userId, amount, txHash) {
        if (amount <= 0) {
        ...
        try {
            ...
            const position = await database_1.prisma.stakingPosition.create({
                data: {
                    userId,
                    amount,
                    status: 'ACTIVE',
            ...
            await database_1.prisma.stakeTransaction.create({
                data: {
                    userId,
                    amount,
                    type: 'STAKE',
            ...
            if (user.walletAddress) {
                await blockchain_service_1.blockchainService.mintPabOnChain(user.walletAddress, amount, `stake:${position.id}`, 'STAKE_CONFIRMED');
            }
            const estimatedDailyReward = await this.estimateDailyReward(userId);
            logger_1.logger.info(`[PabStaking] User ${userId} staked ${amount} $PAB → Tier: ${tier}, Multiplier: ${multiplier}x`);
            return {
                success: true,
                userId,
                amount,
            ...
            logger_1.logger.error('[PabStaking] Stake failed:', error.message);
            return {
                success: false,
                userId,
                amount,
     ...
     */
    async unstakeTokens(userId, positionId) {
        try {
            ...
            if (slashingPenalty > 0 && penalty.reason) {
                ...
                logger_1.logger.warn(`[PabStaking] Penalized user ${userId}: ${slashingPenalty} $PAB slashed (${penalty.reason})`);
            }
            const totalStaked = await this.getTotalStaked(userId);
            const { tier, multiplier } = this.getTier(totalStaked);
            const user = await database_1.prisma.user.findUnique({ where: { id: userId }, select: { walletAddress: true } });
            if (user?.walletAddress && netAmount > 0) {
                // In production: return tokens on-chain
                await blockchain_service_1.blockchainService.mintPabOnChain(user.walletAddress, netAmount, `unstake:${positionId}`, 'UNSTAKE_RETURN');
            }
            return {
                success: true,
                userId,
            ...
            logger_1.logger.error('[PabStaking] Unstake failed:', error.message);
            return {
                success: false,
                userId,
                amount: 0,
     ...
     */
    async slashStake(userId, reason, reservationId) {
        const slashRate = exports.SLASH_RATES[reason];
        if (slashRate === undefined) {
            return { success: false, userId, reason, slashedAmount: 0, remainingStake: 0, newMultiplier: 1.0 };
        }
        try {
            const activeStakes = await database_1.prisma.stakingPosition.findMany({
                where: { userId, status: 'ACTIVE' },
            ...
            logger_1.logger.warn(`[PabStaking] Slashed ${totalToSlash} $PAB from user ${userId} for ${reason}${reservationId ? ` (reservation: ${reservationId})` : ''}`);
            return {
                success: true,
                userId,
                reason,
            ...
            logger_1.logger.error('[PabStaking] Slash failed:', error.message);
            return { success: false, userId, reason, slashedAmount: 0, remainingStake: 0, newMultiplier: 1.0 };
        }
    }
    /**
     ...
     */
    async rewardUser(userId, trigger, referenceId) {
        const rewardRate = exports.REWARD_RATES[trigger];
        ...
        try {
            ...
            if (trigger === 'COMPLETED_BOOKING' && recentCompletions >= 5) {
                rewardAmount += exports.REWARD_RATES.STREAK_BONUS;
            }
            ...
            const activePosition = await database_1.prisma.stakingPosition.findFirst({
                where: { userId, status: 'ACTIVE' },
            });
            if (activePosition) {
                await database_1.prisma.stakingPosition.update({
                ...
                await database_1.prisma.stakingPosition.create({
            ...
            const user = await database_1.prisma.user.findUnique({ where: { id: userId }, select: { walletAddress: true } });
            if (user?.walletAddress) {
                await blockchain_service_1.blockchainService.mintPabOnChain(user.walletAddress, rewardAmount, `reward:${trigger}:${referenceId || ''}`, 'REWARD_MINT');
            }
            logger_1.logger.info(`[PabStaking] Rewarded user ${userId}: ${rewardAmount} $PAB for ${trigger}`);
            return {
                success: true,
                userId,
                rewardAmount,
            ...
            logger_1.logger.error('[PabStaking] Reward failed:', error.message);
     ...
     */
    async getEffectiveDeposit(userId, baseDeposit) {
    ...
    // ── Internal helpers ────────────────────────────────────────────────────────
    async getTotalStaked(userId) {
        const result = await database_1.prisma.stakingPosition.aggregate({
            where: { userId, status: 'ACTIVE' },
            _sum: { amount: true },
        });
    ...
    async estimateDailyReward(userId) {
        const totalStaked = await this.getTotalStaked(userId);
        const apy = await this.getCurrentApy();
        return Number((totalStaked * apy / 365).toFixed(8));
    }
    async getRecentCompletedBookings(userId, days) {
        const since = new Date();
        since.setDate(since.getDate() - days);
        const count = await database_1.prisma.reservation.count({
            where: {
                customerId: userId,
                status: 'COMPLETED',
                createdAt: { gte: since },
    ...
    async computePendingPenalties(userId) {
        // Check recent no-shows in the last 7 days
        const since = new Date();
        since.setDate(since.getDate() - 7);
        const recentNoShows = await database_1.prisma.reservation.count({
            where: {
                customerId: userId,
                status: 'NO_SHOW',
                createdAt: { gte: since },
        ...
        if (recentNoShows > 0) {
            return { rate: exports.SLASH_RATES.NO_SHOW, reason: 'recent_no_show' };
        }
        // Check lost disputes (where the user filed the dispute but it was dismissed)
        const lostDisputes = await database_1.prisma.dispute.count({
            where: {
                reportedById: userId,
                outcome: 'DISMISSED',
                createdAt: { gte: since },
        ...
        if (lostDisputes > 0) {
            return { rate: exports.SLASH_RATES.DISPUTE_LOST, reason: 'lost_dispute' };
        }
        return { rate: 0 };
    }
}
exports.PabTokenStakingService = PabTokenStakingService;
exports.pabTokenStakingService = new PabTokenStakingService();
//# sourceMappingURL=pabTokenStaking.service.js.map