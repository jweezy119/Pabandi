...
exports.agentReputationService = void 0;
exports.processReputation = processReputation;
/**
...
async function getTrustScore(agentId) {
    const agent = await database_1.prisma.web3Agent.findUnique({ where: { id: agentId }, select: { balancePab: true } });
    if (!agent)
        return 50;
    return Math.max(0, Math.min(100, 50 + (agent.balancePab || 0) * 2));
}
async function recordAuditTrail(userId, previousScore, newScore, reason, severity, metadata) {
...
async function processReputation(input) {
    ...
    if (input.outcome === 'COMPLETED') {
        ...
        await database_1.prisma.agentTransaction.create({
            data: {
                ...
                metadata: { ...input.metadata, outcome: input.outcome, bookingId: input.bookingId, gigId: input.gigId },
            },
        });
    }
    else if (input.outcome === 'NO_SHOW') {
        const stake = await database_1.prisma.agentStake.findUnique({ where: { agentId: input.agentId } });
    ...
    if (stakeDelta !== 0) {
        const currentStake = await database_1.prisma.agentStake.findUnique({ where: { agentId: input.agentId } });
    ...
    try {
        ...
        logger_1.logger.warn(`[agentReputation] attestation re-issue failed: ${e.message}`);
    }
    return {
        ok: true,
        agentId: input.agentId,
...
exports.agentReputationService = {
    processReputation,
};
//# sourceMappingURL=agentReputation.service.js.map