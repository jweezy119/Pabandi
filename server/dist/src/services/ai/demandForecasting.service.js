...
class DemandForecastingService {
    ...
    async forecast(features) {
        try {
            ...
            const predictions = this.generatePredictions(timeSeries, decomposition, features);
            ...
            logger_1.logger.error('Error in demand forecasting', error);
     ...
     */
    async getCapacityPlanning(businessId, days = 30) {
        const features = {
            businessId,
            category: 'GENERAL',
            historicalDemand: [],
        ...
        const staffing = this.calculateStaffingRecommendations(Array.from({ length: 7 }, (_, i) => {
            const date = new Date();
    ...
    decomposeTimeSeries(timeSeries) {
        ...
        const seasonal = [];
        const seasonalPattern = Array(7).fill(1);
    ...
    generatePredictions(timeSeries, decomposition, features) {
        const predictions = [];
        ...
        for (let i = 1; i <= 30; i++) {
            ...
            if (eventAdjustment !== 0)
                factors.events = eventAdjustment;
            if (promoAdjustment !== 0)
                factors.promotions = promoAdjustment;
            predictions.push({
    ...
    identifyPeakLowDays(predictions) {
    ...
    extractSeasonalityPattern(predictions) {
    ...
    async getBusinessCapacity(businessId) {
        // Would fetch from business settings
        return 100;
    }
    generateStaffingSchedule(predictions) {
    ...
    calculateCostSavings(predictions) {
    ...
    calculateStaffingRecommendations(weeklyDemand) {
...
//# sourceMappingURL=demandForecasting.service.js.map