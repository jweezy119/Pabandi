...
exports.accioAgentService = exports.AccioAgentService = void 0;
const database_1 = require("../../utils/database");
const logger_1 = require("../../utils/logger");
const dashscope_service_1 = require("./dashscope.service");
class AccioAgentService {
    /**
     * Analyze upcoming reservations for a business to detect supply needs.
     */
    async analyzeUpcomingDemand(businessId) {
        try {
            ...
            const upcomingReservations = await database_1.prisma.reservation.findMany({
                where: {
                    businessId,
                    reservationDate: {
                        gte: new Date(),
            ...
            logger_1.logger.error('Accio Agent Analysis Error', error);
            throw error;
        }
    }
    /**
     ...
     */
    async draftSourcingOrder(businessId, needs) {
        if (!needs.length)
            return null;
        const totalCost = needs.reduce((sum, item) => sum + (item.quantity * item.estimatedPricePKR), 0);
        const order = await database_1.prisma.sourcingOrder.create({
            data: {
                businessId,
                items: needs,
    ...
    async confirmOrder(orderId) {
        ...
        const updated = await database_1.prisma.sourcingOrder.update({
            where: { id: orderId },
            data: {
                status: 'ORDERED',
     ...
     */
    async discoverTrends(businessId) {
        try {
            const business = await database_1.prisma.business.findUnique({ where: { id: businessId } });
            if (!business)
                throw new Error('Business not found');
            ...
            if (apiKey && apiKey !== 'REPLACE_WITH_YOUR_DASHSCOPE_API_KEY') {
                try {
                    logger_1.logger.info(`[Accio Sourcing Opportunities] Requesting Qwen AI suggestion for business: ${businessId}`);
          ...
          `;
                    const aiResponse = await dashscope_service_1.dashscopeService.generateText(systemPrompt, userPrompt);
                    ...
                    logger_1.logger.error(`[DashScope Sourcing Opportunity] AI Sourcing suggestion failed, falling back to heuristics: ${apiError.message}`);
                }
            }
            projectedRoiPercent = servicePrice > 0 ? ((bookings * servicePrice) / cost) * 100 : projectedRoiPercent;
            // Check if it already exists
            const existing = await database_1.prisma.trendSuggestion.findFirst({
                where: { businessId, equipmentName }
            });
            if (existing)
                return [existing];
            const suggestion = await database_1.prisma.trendSuggestion.create({
                data: {
                    businessId,
                    equipmentName,
                    description,
            ...
            logger_1.logger.error('Accio Agent Trend Error', error);
            throw error;
        }
    }
    /**
     ...
     */
    async launchTrendService(trendId) {
        const trend = await database_1.prisma.trendSuggestion.findUnique({ where: { id: trendId } });
        if (!trend)
            throw new Error('Trend not found');
        const updated = await database_1.prisma.trendSuggestion.update({
            where: { id: trendId },
...
exports.AccioAgentService = AccioAgentService;
exports.accioAgentService = new AccioAgentService();
//# sourceMappingURL=accioAgent.service.js.map