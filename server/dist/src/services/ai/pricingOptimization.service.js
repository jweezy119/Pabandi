...
exports.pricingOptimizationService = exports.PricingOptimizationService = void 0;
const logger_1 = require("../../utils/logger");
class PricingOptimizationService {
    /**
     * Generate optimal pricing recommendation
     */
    async optimizePrice(features) {
        try {
            // Calculate price elasticity of demand
            const elasticity = this.calculateElasticity(features.historicalDemand, features.historicalPrices);
            ...
            const sensitivityAnalysis = this.generateSensitivityAnalysis(demandCurve, features.cost, features.currentPrice);
            // Determine risk level
            const riskLevel = this.assessRisk(features, optimalPrice, elasticity);
            // Implementation strategy
            const implementationStrategy = this.determineImplementationStrategy(features.currentPrice, optimalPrice, riskLevel);
            // Generate reasoning
            const reasoning = this.generateReasoning(features, optimalPrice, elasticity, revenueChangePercent, competitorAnalysis);
            return {
            ...
            logger_1.logger.error('Error in price optimization', error);
     ...
     */
    calculateElasticity(demand, prices) {
     ...
     */
    estimateDemandCurve(features, elasticity) {
     ...
     */
    calculateOptimalPrice(features, demandCurve, elasticity) {
        ...
        if (features.inventoryConstraints) {
            ...
            if (predictedDemand > features.inventoryConstraints.maxCapacity) {
                ...
                optimalPrice = this.findPriceForDemand(features.inventoryConstraints.maxCapacity * 0.95, features);
            }
        }
     ...
     */
    generateSensitivityAnalysis(demandCurve, cost, currentPrice) {
        const pricePoints = [];
     ...
     */
    analyzeCompetitors(currentPrice, competitorPrices) {
     ...
     */
    assessRisk(features, newPrice, elasticity) {
        ...
        if (features.historicalPrices.length < 6)
            riskScore += 20;
        else if (features.historicalPrices.length < 12)
            riskScore += 10;
        if (riskScore >= 50)
            return 'HIGH';
        if (riskScore >= 25)
     ...
     */
    determineImplementationStrategy(currentPrice, newPrice, riskLevel) {
        const changePercent = Math.abs((newPrice - currentPrice) / currentPrice) * 100;
        if (riskLevel === 'HIGH')
            return 'TEST_THEN_SCALE';
        if (riskLevel === 'MODERATE' || changePercent > 15)
            return 'GRADUAL';
        return 'IMMEDIATE';
    }
    /**
     ...
     */
    generateReasoning(features, newPrice, elasticity, revenueChange, competitorAnalysis) {
        const reasoning = [];
    ...
    calculateConfidence(features, elasticity) {
        let confidence = 40;
        // More historical data = higher confidence
        confidence += Math.min(30, features.historicalPrices.length * 2);
        // More elasticity data points = higher confidence
        if (features.historicalDemand.length >= 12)
            confidence += 20;
        else if (features.historicalDemand.length >= 6)
            confidence += 15;
        // Competitor data
        if (features.competitorPrices && features.competitorPrices.length > 0)
            confidence += 10;
        ...
        const absElasticity = Math.abs(elasticity);
        if (absElasticity > 0.5 && absElasticity < 3)
            confidence += 10;
    ...
    predictDemandAtPrice(price, features) {
        const elasticity = this.calculateElasticity(features.historicalDemand, features.historicalPrices);
        const demandCurve = this.estimateDemandCurve(features, elasticity);
        return demandCurve(price);
    }
    findPriceForDemand(targetDemand, features) {
        const elasticity = this.calculateElasticity(features.historicalDemand, features.historicalPrices);
        const demandCurve = this.estimateDemandCurve(features, elasticity);
...
exports.PricingOptimizationService = PricingOptimizationService;
exports.pricingOptimizationService = new PricingOptimizationService();
//# sourceMappingURL=pricingOptimization.service.js.map