...
exports.bestFitEngineService = exports.BestFitEngineService = void 0;
const database_1 = require("../../utils/database");
const logger_1 = require("../../utils/logger");
class BestFitEngineService {
    /**
     * Ingests a completed or cancelled gig, records it, and updates the provider's behavior vector.
     */
    async ingestGigOutcome(input) {
        logger_1.logger.info(`[BestFitEngine] Ingesting gig outcome for Passport ${input.passportId}`);
        ...
        await database_1.prisma.passportGigHistory.create({
            data: {
     ...
     */
    async recomputeVector(passportId) {
        const history = await database_1.prisma.passportGigHistory.findMany({
            where: { passportId },
        });
        if (history.length === 0)
            return;
        ...
        await database_1.prisma.passportVector.upsert({
            where: { passportId },
            create: {
                passportId,
        ...
        logger_1.logger.info(`[BestFitEngine] Recomputed vector for ${passportId}. Samples: ${history.length}`);
    }
    /**
     * Predicts the best fit providers for an open gig.
     */
    async predictBestFit(req) {
        ...
        const passports = await database_1.prisma.trustPassport.findMany({
        ...
        for (const p of passports) {
            if (!p.vector)
                continue;
            const providerVector = p.vector.behaviorVector;
            if (!Array.isArray(providerVector) || providerVector.length !== 14)
                continue;
...
exports.BestFitEngineService = BestFitEngineService;
exports.bestFitEngineService = new BestFitEngineService();
//# sourceMappingURL=bestFitEngine.service.js.map