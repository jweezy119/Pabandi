...
exports.revenueForecastingService = exports.RevenueForecastingService = void 0;
...
class RevenueForecastingService {
    ...
    async forecast(features) {
        try {
            ...
            const predictions = this.generatePredictions(historicalRevenue, trend, seasonality, features);
            ...
            const recommendations = this.generateRecommendations(predictions, trend, features);
            // Overall confidence
            ...
            logger_1.logger.error('Error in revenue forecasting', error);
     ...
     */
    async getMonthlyPredictions(businessId, months = 12) {
        const features = { businessId, category: 'general' };
     ...
     */
    async getHistoricalRevenue(businessId, months) {
    ...
    calculateTrend(revenue) {
        ...
        if (slope > avgRevenue * 0.02)
            direction = 'GROWING';
        else if (slope < -avgRevenue * 0.02)
            direction = 'DECLINING';
     ...
     */
    generatePredictions(historical, trend, seasonality, features) {
        const predictions = [];
     ...
     * Calculate confidence intervals
     */
    calculateConfidenceIntervals(historical, predictions) {
     ...
     */
    identifyKeyDrivers(features, historical) {
        const drivers = [];
        // Trend driver
        if (historical.length >= 3) {
            ...
            if (recentAvg > olderAvg * 1.1)
                drivers.push('Strong recent growth momentum');
            else if (recentAvg < olderAvg * 0.9)
                drivers.push('Recent revenue decline');
        }
        // Seasonality
     ...
     */
    generateRecommendations(predictions, trend, features) {
     ...
     */
    calculateConfidence(historical, features) {
    ...
    calculateGrowthRate(historical, predictions) {
...
exports.RevenueForecastingService = RevenueForecastingService;
exports.revenueForecastingService = new RevenueForecastingService();
//# sourceMappingURL=revenueForecasting.service.js.map