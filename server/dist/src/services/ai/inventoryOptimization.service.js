...
exports.inventoryOptimizationService = exports.InventoryOptimizationService = void 0;
const logger_1 = require("../../utils/logger");
class InventoryOptimizationService {
    /**
     * Optimize inventory levels across all items
     */
    async optimizeInventory(features) {
        try {
            ...
            for (const item of features.items) {
                ...
                if (recommendation.stockoutRisk > 70)
                    stockoutRiskItems++;
                if (recommendation.excessStockRisk > 70)
                    excessStockItems++;
            }
            // Sort by priority (stockout risk first, then savings)
            recommendations.sort((a, b) => {
                if (a.stockoutRisk !== b.stockoutRisk)
            ...
            logger_1.logger.error('Error in inventory optimization', error);
    ...
    // ── Private Methods ──
    async optimizeSingleItem(item, features) {
        ...
        // Estimated savings
        const estimatedAnnualSavings = this.calculateSavings(item, recommendedOrderQuantity, reorderPoint, maxStock);
        return {
            itemId: item.itemId,
            itemName: item.name,
    ...
    calculateStockoutRisk(item, avgDailyDemand, reorderPoint) {
        ...
        if (daysUntilReorder <= 2)
            return 70;
        if (daysUntilReorder <= 5)
            return 40;
        if (daysUntilReorder <= 10)
            return 20;
        return 10;
    }
    calculateExcessStockRisk(item, avgDailyDemand, maxStock) {
    ...
    calculateSavings(item, orderQty, reorderPoint, maxStock) {
    ...
    calculateHealthScore(recommendations) {
    ...
    generateSummary(recommendations, stockoutItems, excessItems) {
...
exports.inventoryOptimizationService = new InventoryOptimizationService();
//# sourceMappingURL=inventoryOptimization.service.js.map