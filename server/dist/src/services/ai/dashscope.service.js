...
exports.dashscopeService = exports.DashscopeService = void 0;
...
class DashscopeService {
    ...
    async embed(text) {
        ...
        const response = await axios_1.default.post('https://dashscope.aliyuncs.com/api/v1/services/embeddings/text-embedding/generation', {
     ...
     */
    async generateTrustProfile(userId) {
        try {
            ...
            if (!user)
                return "Insufficient data to generate Trust Profile.";
            const total = user.reservations.length;
            if (total === 0)
                return "New patron on Pabandi. Awaiting history.";
      ...
      `;
            ...
            if (apiKey && apiKey !== 'REPLACE_WITH_YOUR_DASHSCOPE_API_KEY') {
                try {
                    logger_1.logger.info(`[DashScope] Calling Alibaba Cloud API for user: ${userId}`);
                    const response = await axios_1.default.post('https://ws-ueieid4zr4rlge79.ap-southeast-1.maas.aliyuncs.com/api/v1/services/aigc/text-generation/generation', {
                        model: 'qwen-turbo',
                        input: {
                            messages: [
                    ...
                    logger_1.logger.error(`[DashScope API Error] Failed to generate profile via API, falling back to heuristics: ${apiError.message}`);
                }
            }
            else {
                logger_1.logger.warn('[DashScope] DASHSCOPE_API_KEY is not set or placeholder. Falling back to heuristic Trust Profile.');
            }
            ...
            logger_1.logger.error('Error generating AI Trust Profile via DashScope:', error);
...
exports.DashscopeService = DashscopeService;
exports.dashscopeService = new DashscopeService();
//# sourceMappingURL=dashscope.service.js.map