...
class CustomerIntelligenceService {
    ...
    async predictCLV(features) {
        try {
            ...
            logger_1.logger.error('Error in CLV prediction', error);
     ...
     */
    async getCustomerIntelligence(customerId, businessId) {
    ...
    calculateChurnRisk(features) {
        ...
        if (features.orderFrequency < expectedFrequency * 0.5)
            risk += 15;
        else if (features.orderFrequency < expectedFrequency)
            risk += 5;
        // Cancellation/No-show history
        if (features.cancellationRate > 0.2)
            risk += 20;
        else if (features.cancellationRate > 0.1)
            risk += 10;
        if (features.noShowRate > 0.1)
            risk += 15;
        else if (features.noShowRate > 0.05)
            risk += 5;
        // Low engagement
        if (features.engagementScore !== undefined && features.engagementScore < 30)
            risk += 15;
        // Low review scores
        if (features.reviewScore !== undefined && features.reviewScore < 3.5)
            risk += 10;
        // No referrals
        if (!features.referralCount || features.referralCount === 0)
            risk += 5;
    ...
    estimateTimeToChurn(features, churnProb) {
        if (churnProb < 30)
            return 365; // Low risk - likely to stay a year+
        if (churnProb < 50)
            return 180; // Moderate - 6 months
        if (churnProb < 70)
            return 90; // High - 3 months
        return 30; // Critical - 1 month
    }
    identifyRiskFactors(features) {
        const factors = [];
        if (features.daysSinceLastOrder > 90)
            factors.push(`Inactive for ${features.daysSinceLastOrder} days`);
        if (features.orderFrequency < 0.5)
            factors.push('Order frequency below 0.5/month');
        if (features.cancellationRate > 0.2)
            factors.push(`${Math.round(features.cancellationRate * 100)}% cancellation rate`);
        if (features.noShowRate > 0.1)
            factors.push(`${Math.round(features.noShowRate * 100)}% no-show rate`);
        if (features.engagementScore !== undefined && features.engagementScore < 30)
            factors.push('Low engagement score');
        if (features.reviewScore !== undefined && features.reviewScore < 3.5)
            factors.push('Below average review scores');
        if (!features.referralCount || features.referralCount === 0)
            factors.push('No referrals generated');
        return factors.length > 0 ? factors : ['No significant risk factors identified'];
    }
    generateRetentionActions(features, churnProb) {
        const actions = [];
        if (churnProb > 70) {
            return [
    ...
    determineOptimalIntervention(features, churnProb) {
        if (churnProb > 70)
            return 'Immediate personal outreach with executive sponsorship';
        if (churnProb > 50)
            return 'Automated re-engagement campaign with personalized offer';
        if (features.daysSinceLastOrder > 60)
            return 'Win-back email campaign with 20% discount';
        if (features.orderFrequency < 0.5)
            return 'Enroll in loyalty program with welcome bonus';
        return 'Monthly personalized newsletter with recommendations';
    }
    segmentCustomer(predictedCLV, riskOfChurn, features) {
        if (predictedCLV > 5000 && features.totalOrders > 20 && riskOfChurn < 30)
            return 'CHAMPION';
        if (predictedCLV > 2000 && features.totalOrders > 10 && riskOfChurn < 40)
            return 'LOYAL';
        if (predictedCLV > 1000 && riskOfChurn < 50)
            return 'POTENTIAL';
        if (riskOfChurn > 50 || features.daysSinceLastOrder > 90)
            return 'AT_RISK';
        return 'LOST';
    }
    determineValueTier(predictedCLV) {
        if (predictedCLV > 10000)
            return 'PLATINUM';
    ...
    determineSegment(clvSegment, predictedCLV) {
        return clvSegment;
    }
    generateCLVRecommendations(features, segment, risk) {
        const recs = [];
        switch (segment) {
            case 'CHAMPION':
    ...
    calculateOptimalDiscount(features, risk) {
        if (risk < 30)
            return undefined; // No discount needed
        if (risk > 70)
            return 25;
        if (risk > 50)
            return 20;
    ...
    determineNextBestAction(features, segment, risk) {
    ...
    calculateConfidence(features) {
        ...
        // Engagement data
        if (features.engagementScore !== undefined)
            confidence += 10;
        if (features.reviewScore !== undefined)
            confidence += 5;
        return Math.max(40, Math.min(95, confidence));
    }
    fallbackCLV(features) {
    ...
    async fetchCustomerFeatures(customerId, businessId) {
        ...
        const totalRevenue = completed.reduce((sum, r) => sum + (r.totalAmount || 0), 0);
...
exports.customerIntelligenceService = new CustomerIntelligenceService();
//# sourceMappingURL=customerIntelligence.service.js.map