...
exports.nightlifeService = {
    ...
    async listVenues(params) {
        ...
        if (params?.genre)
            where.musicGenres = { has: params.genre };
    ...
    // ── Bottle Service ────────────────────────────────────────────────────────
    async createBottlePackage(data) {
        return prisma.bottlePackage.create({ data });
    },
    async getAvailableTables(venueId, date, guestCount) {
        ...
        for (const table of tableTypes) {
            const reservedCount = existingReservations.filter((r) => r.tableTypeId === table.id).length;
    ...
    async createBottleReservation(data) {
        ...
        if (!table || !pkg)
            throw new Error('Invalid table or package');
        ...
        return prisma.bottleReservation.create({
            data: {
                ...
                totalPrice,
                status: 'PENDING',
                confirmationCode: `BTL-${crypto_1.default.randomBytes(4).toString('hex').toUpperCase()}`,
            },
        });
    },
    ...
    // ── Cover Charge ──────────────────────────────────────────────────────────
    async createCoverCharge(data) {
        return prisma.coverCharge.create({ data });
    },
    async getCoverCharge(venueId, params) {
        ...
        let filtered = validCharges.filter((c) => !c.gender || c.gender === 'ALL' || c.gender === params.gender);
        if (params.isGuestList) {
            const guestListCharge = filtered.find((c) => c.guestListIncluded);
            if (guestListCharge)
                return { amount: 0, type: 'GUEST_LIST', name: guestListCharge.name };
        }
        if (params.isVIP) {
            const vipCharge = filtered.find((c) => c.type === 'VIP');
            if (vipCharge)
                return { amount: vipCharge.amount, type: 'VIP', name: vipCharge.name };
        }
        const generalCharge = filtered.find((c) => c.type === 'GENERAL');
    ...
    // ── Demand Forecasting ───────────────────────────────────────────────────
    async forecastDemand(venueId, daysAhead = 7) {
        ...
        const demand = historicalData.map((d) => d.totalAttendees);
        const alpha = 0.3;
    ...
    async estimateWaitTime(venueId) {
        const venue = await prisma.bookingVenue.findUnique({ where: { id: venueId } });
        if (!venue)
    ...
    // ── Fraud Detection ──────────────────────────────────────────────────────
    async detectFraudulentPromoter(promoterId) {
    ...
    // ── Analytics ────────────────────────────────────────────────────────────
    async trackAnalytics(venueId, data) {
...
//# sourceMappingURL=nightlife.service.js.map