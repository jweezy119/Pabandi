...
const activeSealStore = new Map();
class TrustSealService {
    /**
     * Register a new trust seal for a merchant.
     * Returns the seal ID and embed code.
     */
    async registerSeal(merchantId, merchantName, domain, tier = 'STARTER') {
        const sealId = `seal_${crypto_1.default.randomBytes(12).toString('hex')}`;
        ...
        const attestation = ptp_spec_1.ptpEngine.issueAttestation(merchantId, 'BUSINESS', trustData.trustScore, trustData.velocity);
        const seal = {
            sealId,
            merchantId,
            merchantName,
        ...
        logger_1.logger.info(`[TrustSeal] Seal registered: ${sealId} for ${merchantName} (${tier}, $${pricing.priceUSD}/mo)`);
     ...
     */
    async renderSeal(sealId, requestDomain) {
        ...
        if (requestDomain && !this.isDomainAuthorized(seal.domain, requestDomain)) {
            logger_1.logger.warn(`[TrustSeal] Unauthorized domain ${requestDomain} attempted to render seal ${sealId}`);
            return null;
        }
        ...
        // Record impression
        seal.impressions++;
        const bandSpec = ptp_spec_1.PTP_RISK_BANDS[seal.riskBand];
    ...
    async verifySeal(sealId) {
        ...
        const verification = ptp_spec_1.ptpEngine.verifyAttestation(seal.attestation);
        return {
            merchantName: seal.merchantName,
    ...
    // ── Private Methods ──────────────────────────────────────────────────
    async refreshSeal(seal) {
        try {
            const trustData = await this.getMerchantTrustData(seal.merchantId);
            seal.attestation = ptp_spec_1.ptpEngine.issueAttestation(seal.merchantId, 'BUSINESS', trustData.trustScore, trustData.velocity);
            seal.riskBand = seal.attestation.assessment.riskBand;
            seal.lastRefreshedAt = Date.now();
        }
        catch (err) {
            logger_1.logger.warn(`[TrustSeal] Refresh failed for ${seal.sealId}: ${err.message}`);
        }
    }
    async getMerchantTrustData(merchantId) {
        try {
            const flux = await trustFlux_service_1.trustFluxService.computeTrustFlux(merchantId);
    ...
    isDomainAuthorized(authorized, requested) {
    ...
    `;
        ...
        const trendHTML = tier !== 'STARTER' ? `<span style="font-size:11px;color:${data.color};opacity:0.8">${data.trendArrow} Band ${data.riskBand}</span>` : '';
...
exports.TrustSealService = TrustSealService;
exports.trustSealService = new TrustSealService();
//# sourceMappingURL=trustSeal.service.js.map