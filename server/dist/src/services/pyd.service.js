...
exports.pydService = exports.PydService = exports.YIELD_POOLS = void 0;
exports.computeDepositTerms = computeDepositTerms;
const database_1 = require("../utils/database");
const logger_1 = require("../utils/logger");
const ptp_spec_1 = require("../protocol/ptp.spec");
const geoRiskOracle_service_1 = require("./geoRiskOracle.service");
...
function computeDepositTerms(input) {
    const riskBand = ptp_spec_1.ptpEngine.scoreToRiskBand(input.tenantTrustScore ?? 50);
    ...
    if (geoRiskBand === 'D')
        propertyRiskTrim = 0.05;
    else if (geoRiskBand === 'E')
        propertyRiskTrim = 0.10;
...
class PydService {
    ...
    async createDeposit(input) {
        const tenant = await database_1.prisma.user.findUnique({ where: { id: input.tenantId } });
        if (!tenant)
        ...
        let bcCheckId = input.beneficiaryBackgroundCheckId;
        const deposit = await database_1.prisma.securityDeposit.create({
            data: {
        ...
        logger_1.logger.info(`[PYD] Deposit ${deposit.id} (${input.depositContext ?? 'PROPERTY'}): payer Band ${riskBand} → ` +
     ...
     */
    async proposeYieldAgreement(depositId, tenantId, landlordId, pool = 'JITO_STSOL') {
        const poolSpec = exports.YIELD_POOLS[pool];
        const agreement = await database_1.prisma.yieldAgreement.create({
            data: {
                depositId,
                tenantId,
                landlordId,
        ...
        // Link to deposit
        await database_1.prisma.securityDeposit.update({
            where: { id: depositId },
            data: { yieldAgreementId: agreement.id, yieldOptIn: true },
        });
        logger_1.logger.info(`[PYD] Yield agreement ${agreement.id} proposed: pool ${pool}, tenant APY ${poolSpec.tenantApy}%, Pabandi spread ${poolSpec.spreadPct}%`);
        return agreement;
    }
    /** Tenant signs the yield agreement. */
    async signAsTenant(agreementId, tenantId) {
        const ag = await database_1.prisma.yieldAgreement.findUnique({ where: { id: agreementId } });
        if (!ag || ag.tenantId !== tenantId)
            throw new Error('Agreement not found / unauthorized');
        return database_1.prisma.yieldAgreement.update({
            where: { id: agreementId },
            data: { tenantSignedAt: new Date(), status: ag.landlordSignedAt ? 'ACTIVE' : 'PROPOSED' },
        });
    }
    /** Landlord signs the yield agreement. */
    async signAsLandlord(agreementId, landlordId) {
        const ag = await database_1.prisma.yieldAgreement.findUnique({ where: { id: agreementId } });
        if (!ag || ag.landlordId !== landlordId)
            throw new Error('Agreement not found / unauthorized');
        const updated = await database_1.prisma.yieldAgreement.update({
            where: { id: agreementId },
        ...
        if (updated.status === 'ACTIVE') {
            ...
            logger_1.logger.info(`[PYD] Yield agreement ${agreementId} ACTIVE — deposit ${ag.depositId} ready for non-custodial funding`);
        }
        return updated;
    }
    /**
     ...
     */
    async fundEscrow(depositId, tenantWallet) {
        ...
        return database_1.prisma.securityDeposit.update({
            where: { id: depositId },
            data: {
                escrowContract,
                escrowTxHash: txHash,
    ...
    async projectYield(depositId, months) {
        const deposit = await database_1.prisma.securityDeposit.findUnique({
            where: { id: depositId },
    ...
    async getDeposit(depositId) {
        return database_1.prisma.securityDeposit.findUnique({
            where: { id: depositId },
            include: { yieldAgreement: true },
        });
    }
     ...
     */
    priceRent(baseRentUSD, geoRiskBand, tenantTrustBand) {
        return geoRiskOracle_service_1.geoRiskOracle.priceWithDualRisk({ baseRentUSD, geoRiskBand, tenantTrustBand });
    }
     ...
     */
    async processTokenizedRent(tenantId, landlordId, rentAmountUSD, holdingDays) {
        const APY = exports.YIELD_POOLS.ONDO_USDC.expectedApy; // e.g., 4.5%
        ...
        logger_1.logger.info(`[PYD-TokenizedRent] Processed $${rentAmountUSD} rent held for ${holdingDays} days in Ondo USDY. ` +
...
exports.PydService = PydService;
exports.pydService = new PydService();
//# sourceMappingURL=pyd.service.js.map