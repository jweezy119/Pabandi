...
exports.recommendationEngine = exports.RecommendationService = void 0;
...
class RecommendationService {
    ...
    async recommendProperties(userId, availableProperties) {
        const user = await database_1.prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            return { recommended: [], reasoning: 'User not found', confidence: 0 };
        const request = {
            model: this.model,
            state: {
                ...
                avg_price: availableProperties.reduce((s, p) => s + (p.monthlyRent || 0), 0) / Math.max(1, availableProperties.length),
                min_price: Math.min(...availableProperties.map((p) => p.monthlyRent || Infinity)),
                max_price: Math.max(...availableProperties.map((p) => p.monthlyRent || 0)),
            },
            questions: {
                price_sensitivity: {
                    type: 'choice',
        ...
        let filtered = availableProperties.filter((p) => {
            if (trustRequired && user.trustScore < (p.minTrustScore || 0))
                return false;
            if (priceRange === 'budget' && p.monthlyRent > 1000)
                return false;
            if (priceRange === 'premium' && p.monthlyRent < 2000)
                return false;
            return true;
        });
        if (filtered.length === 0)
            filtered = availableProperties.slice(0, 3);
        return {
    ...
    async recommendTrustTier(userId) {
        const user = await database_1.prisma.user.findUnique({ where: { id: userId } });
        if (!user) {
    ...
    async recommendPaymentMethod(userId, amountUsd) {
        const user = await database_1.prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            return { method: 'usdc', pabPercent: 0, savings: 0, reasoning: 'User not found', confidence: 0 };
        const request = {
            model: this.model,
            state: {
                user_pab_balance: user.pabBalance ?? 0,
        ...
        const response = await this.callJev(request);
        const method = (response?.answers?.payment_method?.choice ?? 'usdc');
        let pabPercent = 0;
        let savings = 0;
        if (method === 'pab') {
            pabPercent = 100;
    ...
    async recommendNextFeature(userId) {
        const user = await database_1.prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            return { feature: 'complete_profile', reason: 'New user', priority: 'high', confidence: 0 };
        ...
        const hasMadePayment = await database_1.prisma.bookingRecord.count({ where: { userId } }) > 0;
        const request = {
            model: this.model,
            state: {
...
exports.recommendationEngine = new RecommendationService();
