...
class OdooService {
    ...
    async authenticate() {
        ...
        try {
            const response = await axios_1.default.post(`${this.url}/jsonrpc`, {
                jsonrpc: '2.0',
                method: 'call',
                params: {
                    service: 'common',
            ...
            logger_1.logger.info(`Successfully authenticated with Odoo CRM (UID: ${this.uid})`);
            return this.uid;
        }
        catch (error) {
            logger_1.logger.error('Odoo Authentication Error:', error.message || error);
            throw error;
        }
    }
    async executeKw(model, method, args, kwargs = {}) {
        ...
        try {
            ...
            logger_1.logger.error(`Odoo execute_kw Error (${model}.${method}):`, error.message || error);
            throw error;
        }
    }
    async syncNewBusiness(data) {
        if (!this.isConfigured()) {
            logger_1.logger.warn('Skipping Odoo sync: Odoo credentials are not configured.');
            return;
        }
        try {
            ...
            logger_1.logger.info(`Successfully synced business ${data.businessName} to Odoo CRM.`);
        }
        catch (error) {
            logger_1.logger.error(`Failed to sync business ${data.businessName} to Odoo:`, error.message || error);
        }
    }
}
exports.odooService = new OdooService();
