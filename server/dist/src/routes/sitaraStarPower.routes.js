 ...
 */
router.get('/star-finder', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        ...
        const minTierIndex = pabandiReview_controller_1.STAR_TIER_ORDER.indexOf(minTier);
        ...
        const results = [...byCustomer.values()]
            ...
            return pabandiReview_controller_1.STAR_TIER_ORDER.indexOf(userTier) >= minTierIndex;
        })
            .map(entry => {
            const tier = (entry.customer.starPowerTier || (0, pabandiReview_controller_1.calculateStarTier)(entry.totalPoints));
            const tierInfo = pabandiReview_controller_1.STAR_TIER_POINTS[tier];
            ...
            return {
                ...
                avgRating: custReviews.length > 0
                    ...
                    : 0,
                totalUpvotes: custReviews.reduce((sum, r) => sum + (r.upvoteCount || 0), 0),
                lastReview: custReviews[0]?.createdAt,
                topReview: [...custReviews].sort((a, b) => (b.rating * 10 + b.upvoteCount) - (a.rating * 10 + a.upvoteCount))[0],
            };
        })
 ...
 */
router.post('/promos', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        ...
        const promo = await database_1.prisma.starPromo.create({
            data: {
 ...
 */
router.get('/my-promos', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const userId = req.user.id;
        const dbUser = await database_1.prisma.user.findUnique({ where: { id: userId }, select: { starPowerTier: true } });
        const userTier = dbUser?.starPowerTier || 'tara';
        const now = new Date();
        // Get active promos that match the user's tier (or are for all tiers)
        const promos = await database_1.prisma.starPromo.findMany({
            where: {
                isActive: true,
                expiresAt: { gt: now },
                OR: [
        ...
        const userRedemptions = await database_1.prisma.starPromoRedemption.findMany({
 ...
 */
router.post('/redeem/:code', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const { code } = req.params;
        const userId = req.user.id;
        const now = new Date();
        const promo = await database_1.prisma.starPromo.findUnique({
        ...
        const dbRedeemer = await database_1.prisma.user.findUnique({ where: { id: userId }, select: { starPowerTier: true } });
        ...
        // Create redemption
        const redemption = await database_1.prisma.starPromoRedemption.create({
            data: {
                promoId: promo.id,
                userId,
            },
        ...
        await database_1.prisma.starPromo.update({
            where: { id: promo.id },
 ...
 */
router.get('/promos', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
 ...
 */
router.post('/redemptions/:id/use', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const { id } = req.params;
        const redemption = await database_1.prisma.starPromoRedemption.findUnique({
            where: { id: String(id) },
            include: {
        ...
        const updated = await database_1.prisma.starPromoRedemption.update({
 ...
 */
router.get('/business/:businessId/stars', async (req, res, next) => {
    try {
        ...
        const groups = await database_1.prisma.pabandiReview.groupBy({
            by: ['rating'],
            where: { businessId },
            _count: { rating: true },
        });
        ...
        const recent = await database_1.prisma.pabandiReview.findMany({
            where: { businessId },
            orderBy: { createdAt: 'desc' },
            take: 3,
            select: {
...
function shortCode(prefix) {
    return `${prefix}-${crypto_1.default.randomBytes(3).toString('hex').toUpperCase()}`;
}
// POST /api/v1/sitara/lists — create a list (auth: promoter or business owner)
router.post('/lists', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const { businessId, venueName, title, date, capacity } = req.body || {};
        if (!title || !date)
            return res.status(400).json({ error: 'title and date are required' });
        if (businessId) {
            const owned = await database_1.prisma.business.findFirst({ where: { id: String(businessId), ownerId: req.user.id } });
        ...
        for (let i = 0; i < 3; i++) {
            const clash = await database_1.prisma.sitaraGuestList.findUnique({ where: { code } });
            if (!clash)
                break;
            code = shortCode('GL');
        }
        const list = await database_1.prisma.sitaraGuestList.create({
            data: {
...
router.get('/lists/mine', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const lists = await database_1.prisma.sitaraGuestList.findMany({
        ...
        const data = lists.map((l) => {
            const heads = l.joins.reduce((s, j) => s + (j.partySize || 1), 0);
            const arrived = l.joins.filter((j) => j.status === 'ARRIVED').length;
            const bySource = {};
            for (const j of l.joins)
                bySource[j.source || 'link'] = (bySource[j.source || 'link'] || 0) + 1;
            return {
...
router.get('/lists/code/:code', async (req, res, next) => {
    try {
        const list = await database_1.prisma.sitaraGuestList.findUnique({
        ...
        const heads = await database_1.prisma.sitaraGuestJoin.aggregate({
...
router.post('/lists/code/:code/join', async (req, res, next) => {
    try {
        const list = await database_1.prisma.sitaraGuestList.findUnique({
            where: { code: String(req.params.code).toUpperCase() },
        });
        ...
        const join = await database_1.prisma.sitaraGuestJoin.create({
            data: {
                listId: list.id,
...
router.get('/lists/business/:businessId', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const business = await database_1.prisma.business.findFirst({
...
router.post('/lists/joins/:id/checkin', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        const join = await database_1.prisma.sitaraGuestJoin.findUnique({
        ...
        if (!owned && join.list.businessId) {
            const b = await database_1.prisma.business.findFirst({
        ...
        const updated = await database_1.prisma.sitaraGuestJoin.update({
            where: { id: join.id },
            data: { status: 'ARRIVED' },
        });
        res.json({ success: true, data: updated });
...
//# sourceMappingURL=sitaraStarPower.routes.js.map