...
router.post('/webhook', async (req, res, next) => {
    try {
        ...
        if (rawPayload.event === 'messages.upsert') {
            ...
            let ackLevel = 0;
            if (updateData.status === 'SERVER_ACK')
                ackLevel = 1;
            else if (updateData.status === 'DELIVERY_ACK')
                ackLevel = 2;
        ...
        openwa_webhook_manager_service_1.webhookManager.dispatch(normalizedPayload).catch(error => {
            logger_1.logger.error(`[EvolutionWebhook] Dispatch error: ${error?.message || error}`);
        });
    }
    catch (error) {
        logger_1.logger.error(`[EvolutionWebhook] Processing error: ${error?.message || error}`);
    }
});
exports.default = router;
//# sourceMappingURL=evolution.webhook.routes.js.map