...
router.post('/quote-buy', (req, res) => {
    try {
        ...
        const pabOut = autonomousEconomy_service_1.autonomousEconomy.quotePabOut(lamports);
...
router.post('/buy-pab', async (req, res) => {
    try {
        const buyer = new web3_js_1.PublicKey(req.body.buyerAddress);
        const sol = parseFloat(req.body.solAmount);
        if (!buyer || !sol || sol <= 0)
            return res.status(400).json({ error: 'buyerAddress + solAmount required' });
...
router.post('/settle-buy', async (req, res) => {
    try {
        ...
        const sol = parseFloat(req.body.solAmount);
        if (!buyer || !solTx || !sol)
            return res.status(400).json({ error: 'buyerAddress + solTx + solAmount required' });
        // (In production: verify the tx actually landed SOL in the treasury via getTransaction.)
        const lamports = Math.floor(sol * 1e9);
        const result = await autonomousEconomy_service_1.autonomousEconomy.buyPab(buyer, lamports);
        logger_1.logger.info(`[Econ] settled buy for ${buyer.toBase58()}: ${result.pabOut / 1e9} PAB`);
...
router.get('/net-revenue', async (_req, res) => {
    try {
        const r = await autonomousEconomy_service_1.autonomousEconomy.netSolRevenue(30);
        res.json(r);
    }
    catch (e) {
...
//# sourceMappingURL=autonomousEconomy.routes.js.map