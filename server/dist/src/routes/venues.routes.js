 ...
 */
async function searchBusinesses(lat, lng, radiusMeters, limit, categories, q) {
    ...
    if (lat !== undefined && lng !== undefined && isFinite(lat) && isFinite(lng) && radiusMeters !== undefined && isFinite(radiusMeters)) {
        return businesses
            ...
            .filter((b) => b != null && b._distance <= radiusMeters)
 ...
 */
function mergeWithDatabase(dbResults, yelp, fsq, osm) {
    ...
    for (const { data, label } of externalSources) {
        for (const v of data) {
            ...
            if (existing) {
                ...
                if (!existing.rating && v.rating != null)
                    existing.rating = v.rating;
                if (!existing.reviewCount && v.reviewCount != null)
                    existing.reviewCount = v.reviewCount;
                if (!existing.phone && v.phone)
                    existing.phone = v.phone;
                if (!existing.website && v.website)
                    existing.website = v.website;
                if (!existing.price && v.price)
                ...
                if (!existing.city && v.city)
                    existing.city = v.city;
                if (v.hours && !existing.hours)
...
router.get('/sitara/discover', async (req, res) => {
    try {
        ...
        const dbResults = await searchBusinesses(numLat, numLng, numRadius, numLimit, category, q).catch(() => []);
        // Secondary: external enrichment (only when keys configured)
        const [yelpResults, fsqResults, osmResults] = await Promise.allSettled([
            searchYelp(String(numLat), String(numLng), q, String(numRadius), String(numLimit)),
            searchFoursquare(String(numLat), String(numLng), q, String(numRadius), String(numLimit)),
            searchOSM(String(numLat), String(numLng), category, String(numRadius), String(numLimit)),
        ]);
        ...
        const slim = merged.slice(0, numLimit).map((v) => ({
            id: v.id,
            name: v.name,
...
router.get('/search', async (req, res) => {
    try {
        ...
        const dbResults = await searchBusinesses(numLat, numLng, numRadiusMeters, numLimit, categories, q).catch(() => []);
        // Secondary: external enrichment (only when keys configured)
        const [yelpResults, fsqResults, osmResults] = await Promise.allSettled([
            searchYelp(lat, lng, q, radius, limit),
            searchFoursquare(lat, lng, q, radius, limit),
            searchOSM(lat, lng, categories, radius, limit),
        ]);
        const yelp = yelpResults.status === 'fulfilled' ? yelpResults.value : [];
        const fsq = fsqResults.status === 'fulfilled' ? fsqResults.value : [];
        const osm = osmResults.status === 'fulfilled' ? osmResults.value : [];
...
router.get('/venues/:source/:id', async (req, res) => {
    try {
...
// ── Get Reviews (Yelp) ────────────────────────────────────────────────────
router.get('/reviews/:source/:id', async (req, res) => {
    try {
        const { source, id } = req.params;
        let reviews = [];
        if (source === 'yelp') {
...
// ── Get Photos ─────────────────────────────────────────────────────────────
router.get('/photos/:source/:id', async (req, res) => {
    try {
        const { source, id } = req.params;
        let photos = [];
        if (source === 'yelp') {
...
router.get('/menu', async (req, res) => {
    try {
        ...
        const menu = await searchOpenMenu(name, lat, lng);
...
// ═══════════════════════════════════════════════════════════════════════════
async function searchYelp(lat, lng, q, radius, limit) {
    if (!YELP_API_KEY)
        return [];
    const params = {
        latitude: lat,
    ...
    if (q)
        params.term = q;
    const response = await axios_1.default.get('https://api.yelp.com/v3/businesses/search', {
        headers: yelpHeaders,
        params,
    });
...
async function getYelpBusiness(id) {
    if (!YELP_API_KEY)
        return null;
    const response = await axios_1.default.get(`https://api.yelp.com/v3/businesses/${id}`, {
        headers: yelpHeaders,
    });
...
async function getYelpReviews(id) {
    if (!YELP_API_KEY)
        return [];
    const response = await axios_1.default.get(`https://api.yelp.com/v3/businesses/${id}/reviews`, {
        headers: yelpHeaders,
    });
...
// ═══════════════════════════════════════════════════════════════════════════
async function searchFoursquare(lat, lng, q, radius, limit) {
    ...
    if (q)
        params.query = q;
    const response = await axios_1.default.get('https://api.foursquare.com/v3/places/search', {
...
async function getFoursquareVenue(id) {
    if (!FOURSQUARE_API_KEY)
        return null;
    const response = await axios_1.default.get(`https://api.foursquare.com/v3/places/${id}`, {
        headers: foursquareHeaders,
    });
...
async function getFoursquareTips(id) {
    if (!FOURSQUARE_API_KEY)
        return [];
    const response = await axios_1.default.get(`https://api.foursquare.com/v3/places/${id}/tips`, {
        headers: foursquareHeaders,
    });
...
async function getFoursquarePhotos(id) {
    if (!FOURSQUARE_API_KEY)
        return [];
    const response = await axios_1.default.get(`https://api.foursquare.com/v3/places/${id}/photos`, {
        headers: foursquareHeaders,
    });
...
async function searchOSM(lat, lng, category, radius, limit) {
    const categoryMap = {
  ...
  `;
    ...
    for (const mirror of OVERPASS_MIRRORS) {
        try {
            const response = await axios_1.default.post(mirror, body, {
            ...
            if (osmCache.size > 200) {
                const now = Date.now();
                for (const [k, v] of osmCache)
                    if (now - v.at > OSM_TTL_MS)
                        osmCache.delete(k);
            }
            return mapped;
        }
...
async function getOSMDetails(id) {
...
// ═══════════════════════════════════════════════════════════════════════════
async function searchOpenMenu(name, lat, lng) {
    if (!OPENMENU_API_KEY || !name)
        return null;
    try {
        const params = { name };
        if (lat && lng) {
        ...
        const response = await axios_1.default.get('https://api.openmenu.com/v1/menu', {
            params,
            headers: { 'X-API-Key': OPENMENU_API_KEY },
        });
        return {
            restaurantName: response.data?.restaurant_name,
            menus: response.data?.menus?.map((m) => ({
                name: m.name,
...
// ═══════════════════════════════════════════════════════════════════════════
function mergeResults(yelp, fsq, osm) {
    const merged = new Map();
    ...
    for (const venue of yelp) {
        ...
        const existing = merged.get(key);
        if (existing) {
            if (!existing.rating && venue.rating)
                existing.rating = venue.rating;
            if (!existing.reviewCount && venue.reviewCount)
                existing.reviewCount = venue.reviewCount;
            if (!existing.website && venue.website)
                existing.website = venue.website;
            if (!existing.phone && venue.phone)
                existing.phone = venue.phone;
            if (!existing.price && venue.price)
                existing.price = venue.price;
            if (!existing.imageUrl && venue.imageUrl)
    ...
    for (const venue of osm) {
        ...
        if (existing) {
            ...
            if (!existing.website && venue.website)
                existing.website = venue.website;
            if (!existing.hours && venue.hours)
                existing.hours = venue.hours;
            if (!existing.address && venue.address)
