...
router.use(rateLimiter_1.aiRateLimiter);
// ── AI Lease Analyzer ───────────────────────────────────────────────────────
// POST /api/v1/ai/analyze-lease
router.post('/analyze-lease', async (req, res, next) => {
    try {
        const { text } = req.body;
        if (!text)
            return res.status(400).json({ success: false, error: 'Lease text is required' });
...
router.post('/maintenance-advice', async (req, res, next) => {
    try {
        ...
        const advice = getMaintenanceAdvice(issue, propertyType, budget, urgency);
...
router.post('/property-valuation', async (req, res, next) => {
    try {
...
function analyzeLeaseTerms(text) {
...
function getMaintenanceAdvice(issue, propertyType, budget, urgency) {
    const lower = issue.toLowerCase();
    const category = categorizeIssue(lower);
    const urgencyLevel = urgency || detectUrgency(lower);
    const advice = {
...
function categorizeIssue(text) {
    if (/leak|water|drip|pipe|plumb/.test(text))
        return 'Plumbing';
    if (/electric|outlet|power|circuit|wire/.test(text))
        return 'Electrical';
    if (/heat|ac|air condition|furnace|hvac/.test(text))
        return 'HVAC';
    if (/pest|bug|roach|mouse|rat|ant|insect/.test(text))
        return 'Pest Control';
    if (/appliance|fridge|washer|dryer|dishwasher|oven|stove/.test(text))
        return 'Appliance';
    if (/crack|wall|floor|ceiling|foundation|structural/.test(text))
        return 'Structural';
    if (/lock|key|security|break/.test(text))
        return 'Security';
    return 'General';
}
function detectUrgency(text) {
    if (/flood|fire|spark|gas|emergency|immediately|burst|collapse/.test(text))
        return 'emergency';
    if (/no heat|no AC|leak|broken|not working|urgent/.test(text))
        return 'high';
    if (/slow|minor|small|drip|noise/.test(text))
        return 'medium';
    return 'low';
}
// ── Market Insights Logic ────────────────────────────────────────────────────
async function getMarketInsights(city, state, propertyType) {
    // Get local data from our platform
    const localListings = await database_1.prisma.property.findMany({
        where: { city: { contains: city, mode: 'insensitive' } },
        select: { pricePerNight: true, bedrooms: true, city: true, category: true },
        take: 50,
    });
    const localPMProperties = await database_1.prisma.propertyManagerProperty.findMany({
        where: { city: { contains: city, mode: 'insensitive' } },
...
function generateMarketInsights(city, avgRent, avgNightly, occupancy) {
    const insights = [];
    if (avgRent)
        insights.push(`Average monthly rent in ${city}: $${Math.round(avgRent).toLocaleString()}`);
    if (avgNightly)
        insights.push(`Average nightly rate: $${Math.round(avgNightly).toLocaleString()}`);
    if (occupancy)
        insights.push(`Current occupancy rate: ${Math.round(occupancy)}%`);
    if (occupancy && occupancy > 85)
        insights.push('High demand market — favorable for landlords');
    if (occupancy && occupancy < 60)
        insights.push('Lower demand — consider competitive pricing');
    return insights;
}
function generateMarketRecommendations(city, avgRent, occupancy) {
    const recs = [];
    if (occupancy && occupancy > 80)
        recs.push('Market supports rent increases — consider 3-5% annual adjustments');
    if (occupancy && occupancy < 70)
        recs.push('Focus on tenant retention and property improvements');
    if (avgRent && avgRent > 2000)
        recs.push('Premium market — emphasize quality and amenities');
    if (avgRent && avgRent < 1000)
        recs.push('Budget-friendly market — target value-seeking tenants');
...
function generateDocument(type, params) {
    const warnings = ['This is a template document. Have it reviewed by a legal professional before use.'];
    switch (type) {
...
function generateLeaseAgreement(params) {
...
function generateNoticeToVacate(params) {
...
function generateRentReceipt(params) {
...
function generateMaintenanceRequest(params) {
...
function generateMoveInChecklist(params) {
...
function generatePetAddendum(params) {
...
async function getPropertyValuation(params) {
    ...
    const comparables = await database_1.prisma.propertyManagerProperty.findMany({
        where: {
...
async function aiChat(message, context) {
    ...
    const nlpResult = await ai_nlp_service_1.aiNlpService.classifyIntentAndLanguage(message);
...
//# sourceMappingURL=aiRealEstate.routes.js.map