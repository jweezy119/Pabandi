...
function analyzePropertyIntelligence(input) {
    ...
    const conditionMultipliers = { excellent: 1.1, good: 1.0, fair: 0.9, poor: 0.75 };
    baseValue *= input.condition ? (conditionMultipliers[input.condition] || 1) : 1;
    if (input.hasGarage)
        baseValue += 15000;
    if (input.hasPool)
        baseValue += 25000;
    const typeMultipliers = { single_family: 1.0, condo: 0.9, townhouse: 0.95, multi_family: 1.2 };
    ...
    if (neighborhood.schools >= 60)
        neighborhood.factors.push('Above-average schools');
    if (neighborhood.walkability >= 70)
        neighborhood.factors.push('Highly walkable');
    if (neighborhood.transit >= 60)
        neighborhood.factors.push('Good transit access');
    if (neighborhood.safety >= 55)
        neighborhood.factors.push('Above-average safety');
    if (cityData.populationGrowth > 0.01)
        neighborhood.factors.push('Fast-growing population');
    if (cityData.vacancyRate < 0.06)
        neighborhood.factors.push('Low vacancy — high demand');
    // ── Price Velocity ──
    const priceVelocity = {
    ...
    if (rentalYield > 0.08)
        investmentScore += 15;
    else if (rentalYield > 0.06)
        investmentScore += 8;
    if (cityData.appreciationRate > 0.06)
        investmentScore += 10;
    if (neighborhood.rentalDemand > 92)
        investmentScore += 10;
    if (cityData.vacancyRate < 0.06)
        investmentScore += 5;
    if (neighborhood.schools >= 60)
    ...
    // ── Risks & Opportunities ──
    const risks = [];
    if (cityData.vacancyRate > 0.08)
        risks.push('High vacancy rate — may be hard to find tenants');
    if (cityData.appreciationRate < 0.03)
        risks.push('Low appreciation market');
    if (neighborhood.safety < 45)
        risks.push('Below-average safety score');
    if (input.yearBuilt && input.yearBuilt < 1990)
        risks.push('Older property — potential maintenance costs');
    if (cityData.inventoryMonths > 4)
        risks.push('High inventory — buyer\'s market');
    const opportunities = [];
    if (cityData.populationGrowth > 0.01)
        opportunities.push('Fast-growing area — increasing demand');
    if (cityData.vacancyRate < 0.06)
        opportunities.push('Low vacancy — strong rental demand');
    if (neighborhood.walkability >= 70)
        opportunities.push('Walkable area commands premium rent');
    if (cityData.appreciationRate > 0.06)
        opportunities.push('Strong appreciation trend');
    if (input.propertyType === 'multi_family')
        opportunities.push('Multiple income streams reduce risk');
    return {
        valuation: {
...
function planScenarios(input) {
    ...
    if (baseScenario.cashOnCashReturn > 10 && baseScenario.dscr > 1.25)
        recommendation = 'strong_buy';
    else if (baseScenario.cashOnCashReturn > 6 && baseScenario.dscr > 1.0)
        recommendation = 'buy';
    else if (baseScenario.cashOnCashReturn < 2 || baseScenario.dscr < 0.8)
...
function analyzePortfolio(properties) {
    ...
    for (const k of Object.keys(byType))
        byType[k] = Math.round((byType[k] / totalValue) * 100);
    for (const k of Object.keys(byLocation))
        byLocation[k] = Math.round((byLocation[k] / totalValue) * 100);
    for (const k of Object.keys(byValue))
        byValue[k] = Math.round((byValue[k] / totalValue) * 100);
    ...
    const diversificationRecs = [];
    if (Object.values(byType).some((v) => v > 60))
        diversificationRecs.push('Over-concentrated in one property type — diversify');
    if (Object.values(byLocation).some((v) => v > 50))
        diversificationRecs.push('Geographic concentration risk — expand to new markets');
    if (Object.keys(byLocation).length < 3)
        diversificationRecs.push('Consider expanding to additional markets');
    // Performance
    ...
    const riskRecs = [];
    if (maxLocConcentration > 50)
        riskRecs.push('High geographic concentration');
    if (avgLtv > 0.7)
        riskRecs.push('High leverage — consider paying down debt');
    if (properties.length < 3)
        riskRecs.push('Small portfolio — diversify to reduce idiosyncratic risk');
    return {
        summary: {
...
function analyzeTenantIntelligence(input) {
    const incomeToRentRatio = input.monthlyIncome / input.monthlyRent;
    // Income stability
    let stabilityScore = 50;
    if (input.employmentLength) {
        if (input.employmentLength >= 24)
            stabilityScore += 25;
        else if (input.employmentLength >= 12)
            stabilityScore += 15;
        else if (input.employmentLength >= 6)
            stabilityScore += 5;
        else
            stabilityScore -= 10;
    }
    ...
    if (input.debtToIncome) {
        if (input.debtToIncome > 0.43)
            debtBurden = 'Very High';
        else if (input.debtToIncome > 0.36)
            debtBurden = 'High';
        else if (input.debtToIncome > 0.20)
            debtBurden = 'Moderate';
        else
            debtBurden = 'Low';
    }
    ...
    if (input.evictionHistory)
        score -= 25;
    if (input.criminalHistory)
        score -= 10;
    if (input.references && input.references >= 2)
        score += 5;
    if (monthsOfReserve >= 3)
        score += 5;
    if (input.employmentLength && input.employmentLength >= 12)
        score += 5;
    ...
    if (input.employmentLength && input.employmentLength >= 12)
        factors.push(`Stable employment: ${input.employmentLength} months`);
    if (input.creditScore && input.creditScore >= 670)
        factors.push('Good credit history');
    if (monthsOfReserve >= 3)
        factors.push(`Financial reserve: ${monthsOfReserve} months`);
    if (input.references && input.references >= 2)
    ...
    if (input.creditScore && input.creditScore < 600)
        risks.push('Below-average credit score');
    if (input.evictionHistory)
        risks.push('Prior eviction on record');
    if (monthsOfReserve < 3)
        risks.push('Limited financial reserve');
    if (input.employmentLength && input.employmentLength < 6)
        risks.push('Short employment tenure');
    const suggestions = [];
    if (incomeToRentRatio < 3)
        suggestions.push('Consider requiring a larger security deposit');
    if (input.creditScore && input.creditScore < 600)
        suggestions.push('Request a co-signer or guarantor');
    if (monthsOfReserve < 3)
        suggestions.push('Require proof of savings or prepaid rent');
    if (!input.references || input.references < 2)
        suggestions.push('Request additional references');
    return {
        score,
        band,
        recommendation,
...
router.post('/tenant-intelligence', async (req, res) => {
    try {
        const input = req.body;
...
function analyzeMarketIntelligence(input) {
    const cityKey = input.city?.toLowerCase() || '';
    const cityData = CITY_INTELLIGENCE[cityKey] || DEFAULT_CITY;
    // Supply/Demand
    ...
    // Signals
    const signals = [];
    if (cityData.inventoryMonths < 2.5)
        signals.push('Very low inventory — competitive market');
    if (cityData.daysOnMarket < 30)
        signals.push('Fast-moving properties');
    if (cityData.vacancyRate < 0.06)
        signals.push('Low vacancy — strong rental demand');
    if (cityData.populationGrowth > 0.01)
        signals.push('Population growing — sustained demand');
    // Opportunities
    const opportunities = [];
    if (supplyDemandScore < 45)
        opportunities.push('Buyer\'s market — negotiate aggressively');
    if (cityData.vacancyRate < 0.06)
        opportunities.push('Strong rental demand — good for landlords');
    if (cityData.populationGrowth > 0.01)
        opportunities.push('Growing market — appreciation potential');
    // Risks
    const risks = [];
    if (supplyDemandScore > 65)
        risks.push('Seller\'s market — paying premium');
    if (cityData.inventoryMonths > 4)
        risks.push('High inventory — prices may soften');
    if (cityData.vacancyRate > 0.08)
        risks.push('High vacancy — rental income risk');
    return {
        supplyDemand: {
...
function predictMaintenance(input) {
    const predictions = [];
    ...
    const seasonalTips = [];
    if (month >= 3 && month <= 5)
        seasonalTips.push('Spring: Focus on HVAC prep, gutter cleaning, and pest prevention');
    if (month >= 6 && month <= 8)
        seasonalTips.push('Summer: Monitor AC usage, check for water leaks, maintain landscaping');
    if (month >= 9 && month <= 11)
        seasonalTips.push('Fall: Clean gutters, service heating system, winterize outdoor plumbing');
    if (month >= 11 || month <= 2)
        seasonalTips.push('Winter: Prevent ice dams, check for frozen pipes, monitor heating system');
...
router.post('/maintenance-predictor', async (req, res) => {
    try {
        const input = req.body;
...
//# sourceMappingURL=aiAdvanced.routes.js.map