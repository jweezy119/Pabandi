...
function analyzeInvestment(input) {
    ...
    if (cashOnCashReturn > 10 && capRate > 7)
        recommendation = 'strong_buy';
    else if (cashOnCashReturn > 6 && capRate > 5)
        recommendation = 'buy';
    else if (cashOnCashReturn < 2 || capRate < 3)
...
router.post('/analyze-investment', auth_middleware_1.authenticate, rateLimiter_1.aiRateLimiter, async (req, res) => {
    try {
...
function optimizeRent(input) {
    ...
    if (amenities.length > 0)
        factors.push(`Amenities: ${amenities.join(', ')}`);
    return {
        suggestedRent,
        range,
...
router.post('/optimize-rent', auth_middleware_1.authenticate, rateLimiter_1.aiRateLimiter, async (req, res) => {
    try {
        const input = req.body;
...
router.post('/match-properties', auth_middleware_1.authenticate, rateLimiter_1.aiRateLimiter, async (req, res) => {
    try {
        const input = req.body;
        ...
        if (input.bedrooms)
            where.bedrooms = { gte: input.bedrooms - 1, lte: input.bedrooms + 1 };
        if (input.bathrooms)
            where.bathrooms = { gte: input.bathrooms - 0.5, lte: input.bathrooms + 0.5 };
        if (input.maxRent)
            where.rentAmount = { lte: input.maxRent };
...
function screenTenant(input, monthlyRent = 1500) {
    let score = 70;
    const factors = [];
    const risks = [];
    // Income analysis
    ...
    if (input.employmentStatus === 'employed') {
        ...
        score -= 10;
        risks.push('Unemployment or unstable income');
    }
    // History
    ...
    if (input.criminalHistory) {
        score -= 15;
        risks.push('Criminal history reported');
    }
    // References
...
router.post('/screen-tenant', auth_middleware_1.authenticate, rateLimiter_1.aiRateLimiter, async (req, res) => {
    try {
...
const aiConversation_service_1 = require("../services/ai/aiConversation.service");
const aiPropertyPhoto_service_1 = require("../services/ai/aiPropertyPhoto.service");
const aiRecommendation_service_1 = require("../services/ai/aiRecommendation.service");
const aiRouter_service_1 = require("../services/ai/aiRouter.service");
...
router.post('/conversation/chat', async (req, res) => {
    try {
        ...
        const result = await aiConversation_service_1.aiConversationService.chat(sessionId || `user-${userId}-${Date.now()}`, message, systemPrompt, userId);
        res.json({ success: true, data: result });
    }
    catch (e) {
...
router.get('/conversation/:sessionId/history', async (req, res) => {
    try {
        const history = await aiConversation_service_1.aiConversationService.getHistory(req.params.sessionId);
...
router.post('/analyze-photo', async (req, res) => {
    try {
        ...
        const result = await aiPropertyPhoto_service_1.aiPropertyPhotoService.analyzePhoto(photoUrl, { propertyType, roomType });
        res.json({ success: true, data: result });
    }
    catch (e) {
...
router.get('/providers', async (_req, res) => {
    try {
        const status = aiRouter_service_1.aiRouter.getStatus();
...
router.post('/properties/:id/embed', async (req, res) => {
    try {
        const result = await aiRecommendation_service_1.aiRecommendationService.embedProperty(req.params.id);
        res.json({ success: true, data: result });
    }
    catch (e) {
...
router.get('/properties/:id/similar', async (req, res) => {
    try {
        ...
        const results = await aiRecommendation_service_1.aiRecommendationService.findSimilarProperties(req.params.id, limit);
