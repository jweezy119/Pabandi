...
const storage = multer_1.default.diskStorage({
    ...
    filename: (_req, file, cb) => {
        const unique = `${Date.now()}-${Math.random().toString(36).slice(2, 9)}${path_1.default.extname(file.originalname)}`;
        cb(null, unique);
    },
});
...
router.post('/units', async (req, res) => {
    try {
        ...
        if (!propertyId || !unitNumber)
            return res.status(400).json({ error: 'propertyId and unitNumber are required' });
...
router.get('/units', async (req, res) => {
    try {
        ...
        const units = await prisma.propertyUnit.findMany({
            where: { propertyId: propertyId, property: { managerId: profile.id } },
...
router.post('/rent-payments', async (req, res) => {
    try {
        ...
        if (!propertyId || !tenantEmail || !amount || !dueDate)
            return res.status(400).json({ error: 'propertyId, tenantEmail, amount, dueDate are required' });
...
router.patch('/rent-payments/:id', async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/financials', async (req, res) => {
    try {
        ...
        if (!propertyId || !type || !category || !amount)
            return res.status(400).json({ error: 'propertyId, type, category, amount are required' });
...
router.get('/financials', async (req, res) => {
    try {
        ...
        const financials = await prisma.propertyFinancial.findMany({
            where: { propertyId: propertyId, ...(typeFilter && { type: typeFilter }) },
            orderBy: { date: 'desc' },
        });
...
router.post('/amenities', async (req, res) => {
    try {
        ...
        if (!propertyId || !amenityId)
            return res.status(400).json({ error: 'propertyId and amenityId are required' });
...
router.post('/rate-plans', async (req, res) => {
    try {
        ...
        if (!propertyId || !name || !rentAmount)
            return res.status(400).json({ error: 'propertyId, name, and rentAmount are required' });
...
router.get('/favorites', async (req, res) => {
    try {
        ...
        const favs = await prisma.propertyFavorite.findMany({ where: { userId: userId }, include: { property: true, unit: true } });
...
router.post('/messages', async (req, res) => {
    try {
        ...
        if (!propertyId || !conversationId || !body)
            return res.status(400).json({ error: 'propertyId, conversationId, and body are required' });
...
router.get('/messages', async (req, res) => {
    try {
        ...
        const msgs = await prisma.propertyMessage.findMany({ where: { conversationId: conversationId }, orderBy: { createdAt: 'asc' } });
...
router.get('/tasks', async (req, res) => {
    try {
        ...
        const tasks = await prisma.task.findMany({ where: { managerId: profile.id, ...(status ? { status: status } : {}) } });
...
router.get('/communications', async (req, res) => {
    try {
        ...
        const comms = await prisma.communication.findMany({ where: { managerId: profile.id, ...(contactId ? { contactId: contactId } : {}), ...(type ? { type: type } : {}) }, orderBy: { createdAt: 'desc' } });
...
router.post('/rent/:id/pay', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (payment.status === 'PAID')
            return res.status(400).json({ error: 'Rent already paid' });
        const { paylioService } = await Promise.resolve().then(() => __importStar(require('../services/paylio.service')));
...
router.get('/rent/:id/paylio/callback', async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/leases/:id/sign', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const callbackToken = crypto_1.default.randomBytes(32).toString('hex');
        const API_BASE = (process.env.API_URL || process.env.FRONTEND_URL || 'http://localhost:5000/api/v1').replace(/\/$/, '');
        let signingUrl = `${API_BASE}/property/sign/${callbackToken}`;
        let externalId = '';
        try {
            const { signingService } = await Promise.resolve().then(() => __importStar(require('../services/signing.service')));
            ...
            logger_1.logger.warn('[sign] provider failed, using custom fallback', e.message);
        }
...
router.get('/sign/:token', async (req, res) => {
    try {
        ...
        if (signingRequest.status === 'SIGNED')
            return res.status(400).send('Document already signed');
        if (signingRequest.status === 'EXPIRED')
            return res.status(400).send('Signing link expired');
...
router.post('/sign/:token/confirm', async (req, res) => {
    try {
        const signingRequest = await prisma.signingRequest.findUnique({
            where: { callbackToken: req.params.token },
            include: { lease: true },
...
router.post('/rent/automation/run', auth_middleware_1.authenticate, async (_req, res) => {
    try {
        const result = await rentAutomation_service_1.rentAutomationService.runDailyRentAutomation();
...
router.post('/rent/:id/mark-paid', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { method, reference } = req.body || {};
        const payment = await rentAutomation_service_1.rentAutomationService.markRentPaid(req.params.id, method || 'MANUAL', reference);
        res.json({ success: true, data: payment });
    }
    catch (e) {
