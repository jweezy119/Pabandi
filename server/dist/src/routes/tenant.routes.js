...
router.get('/rent-payments', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userEmail = req.user?.email;
        if (!userEmail)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.get('/applications/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userEmail = req.user?.email;
        if (!userEmail)
            return res.status(401).json({ error: 'Unauthorized' });
