...
exports.OFFLINE_BUSINESSES = void 0;
exports.seedOfflineBusinesses = seedOfflineBusinesses;
...
router.post('/admin', (req, res) => {
    ...
    (0, admin_controller_1.setupAdmin)(req, res, (err) => {
        if (err) {
            logger_1.logger.error('[Seed] admin bootstrap failed', err);
...
router.post('/freelancers', async (req, res) => {
    if (isProduction(req)) {
        return (0, apiResponse_1.fail)(res, 'Seed endpoints are disabled in production', 404);
    }
    try {
...
router.post('/real-businesses', async (_req, res) => {
    ...
    const canSeed = !isProduction(_req) || await database_1.prisma.business.count() === 0;
...
async function osmQuery(bbox, tagKey, tagVal) {
    ...
    for (const url of mirrors) {
        try {
            ...
            const els = (json.elements || []).filter((el) => el.tags && el.tags.name);
            if (els.length > 0)
                return els;
        }
        catch (e) {
...
router.post('/osm-businesses', async (req, res) => {
    ...
    const canSeed = !isProduction(req) || await database_1.prisma.business.count({ where: { latitude: { not: null }, longitude: { not: null } } }) === 0;
    if (!canSeed) {
...
// ── Real Projects Seed ──────────────────────────────────────────────────────
const realProjects_1 = require("../data/realProjects");
router.post('/real-projects', async (_req, res) => {
    if (isProduction(_req)) {
        return (0, apiResponse_1.fail)(res, 'Seed endpoints are disabled in production', 404);
    }
    const summary = { created: 0, skipped: 0, errors: [] };
    try {
        for (const p of realProjects_1.REAL_PROJECTS) {
            try {
                const existing = await database_1.prisma.project.findFirst({ where: { title: p.title } });
                if (existing) {
                    summary.skipped++;
                    continue;
                }
...
router.post('/real-jobs', async (_req, res) => {
    if (isProduction(_req)) {
        return (0, apiResponse_1.fail)(res, 'Seed endpoints are disabled in production', 404);
    }
    const summary = { created: 0, skipped: 0, errors: [] };
    try {
        for (const j of REAL_JOBS) {
            const existing = await database_1.prisma.jobPosting.findFirst({ where: { title: j.title } });
            if (existing) {
                summary.skipped++;
                continue;
            }
            await database_1.prisma.jobPosting.create({
                data: {
...
const realFreelancers_1 = require("../data/realFreelancers");
router.post('/real-freelancers', async (_req, res) => {
    if (isProduction(_req)) {
        return (0, apiResponse_1.fail)(res, 'Seed endpoints are disabled in production', 404);
    }
    const summary = { created: 0, skipped: 0, errors: [] };
    try {
        for (const f of realFreelancers_1.REAL_FREELANCERS) {
            try {
                ...
                const passwordHash = await bcryptjs_1.default.hash('pabandi2026', 10);
                const user = await database_1.prisma.user.create({
                    data: {
                        email, passwordHash,
                        firstName: f.name.split(' ')[0], lastName: f.name.split(' ').slice(1).join(' '),
                        role: client_1.UserRole.CUSTOMER, isEmailVerified: true,
...
exports.OFFLINE_BUSINESSES = OFFLINE_BUSINESSES;
router.post('/offline-businesses', async (_req, res) => {
    const canSeed = !isProduction(_req) || await database_1.prisma.business.count({ where: { latitude: { not: null }, longitude: { not: null } } }) === 0;
    if (!canSeed) {
    ...
    try {
        for (const b of OFFLINE_BUSINESSES) {
            const existing = await database_1.prisma.business.findFirst({ where: { slug: b.slug } });
            const data = {
            ...
            if (existing) {
                ...
                await database_1.prisma.business.create({ data: { ...data, id: b.slug } });
                summary.created++;
            }
        }
        ...
        logger_1.logger.error('[Seed] offline-businesses failed', e);
...
async function seedOfflineBusinesses() {
    const count = await database_1.prisma.business.count({ where: { latitude: { not: null }, longitude: { not: null } } });
    if (count > 0)
    ...
    for (const b of OFFLINE_BUSINESSES) {
        try {
            ...
            logger_1.logger.warn(`[Seed] offline business insert failed for ${b.slug}: ${e.message}`);
        }
    }
    return inserted;
}
