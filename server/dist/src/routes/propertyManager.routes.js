...
const pmWebhook_service_1 = require("../services/pmWebhook.service");
const crypto_1 = __importDefault(require("crypto"));
const prisma = new client_1.PrismaClient();
const router = (0, express_1.Router)();
function generateSlug(name) {
    return `${name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}-${crypto_1.default.randomBytes(3).toString('hex')}`;
}
// POST /api/v1/property-manager/enroll
router.post('/enroll', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const existing = await prisma.propertyManagerProperty.findUnique({ where: { userId } });
        if (existing)
            return res.json({ success: true, profile: existing, created: false });
        const { companyName, slug, brandColor, tagline, businessType } = req.body || {};
        const profile = await prisma.propertyManagerProperty.create({
            data: {
                userId,
                managerId: userId,
                title: companyName || `Business ${userId.slice(-6)}`,
                companyName: companyName || null,
                slug: slug || generateSlug(companyName || `business-${userId.slice(-6)}`),
...
router.get('/me', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        const profile = await prisma.propertyManagerProperty.findUnique({ where: { userId }, include: { _count: { select: { tenants: true } } } });
        if (!profile)
...
router.patch('/profile', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const updated = await prisma.propertyManagerProperty.update({ where: { id: profile.id }, data: updateData });
        (0, pmWebhook_service_1.logActivity)(profile.id, 'profile.updated', 'profile', profile.id, `Profile updated: ${Object.keys(updateData).join(', ')}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'profile.updated', { profileId: profile.id, ...updateData });
...
router.post('/properties', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'property.created', 'property', property.id, `Property added: ${title}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'property.created', { propertyId: property.id, title });
...
router.post('/tenants', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, existing ? 'tenant.updated' : 'tenant.created', 'tenant', tenant.id, existing ? `Tenant updated: ${normalizedEmail}` : `Tenant added: ${normalizedEmail}`);
        if (!existing)
            (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'tenant.created', { tenantId: tenant.id, email: normalizedEmail });
...
router.post('/screen', auth_middleware_1.authenticate, rateLimit_middleware_1.courtListenerLimiter, rateLimit_middleware_1.courtListenerDailyLimiter, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (!tenantEmail)
            return res.status(400).json({ error: 'tenantEmail is required' });
        ...
        let courtCheckId = null;
        let courtCheckResult = null;
        let screeningDetails = null;
        if ((source === 'COURTLISTENER' || !source)) {
            try {
                const result = await courtCheck_service_1.courtCheckService.screenParty({ subjectType: 'TENANT', name, state: state || undefined });
                if (result && result.id) {
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'screening.created', 'screening', screening.id, `Screening completed for ${tenantEmail}: ${resolvedBand}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'screening.created', { screeningId: screening.id, tenantEmail, band: resolvedBand, source: screenedSource });
        res.status(201).json({
            success: true,
...
router.post('/appointments', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (!tenantEmail || !startsAt)
            return res.status(400).json({ error: 'tenantEmail and startsAt are required' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'appointment.created', 'appointment', appointment.id, `Showing scheduled for ${tenantEmail}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'appointment.created', { appointmentId: appointment.id, tenantEmail, startsAt });
...
router.patch('/appointments/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/leases', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        if (!tenantEmail || !startDate || !endDate || !rentAmount)
            return res.status(400).json({ error: 'tenantEmail, startDate, endDate, rentAmount are required' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'lease.created', 'lease', lease.id, `Lease created for ${tenantEmail}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'lease.created', { leaseId: lease.id, tenantEmail, rentAmount });
...
router.patch('/leases/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'lease.updated', 'lease', lease.id, `Lease updated: ${status}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'lease.updated', { leaseId: lease.id, status });
...
router.post('/maintenance', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'maintenance.created', 'maintenance', maintenance.id, `Maintenance reported: ${title}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'maintenance.created', { maintenanceId: maintenance.id, title, priority });
...
router.patch('/maintenance/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'maintenance.updated', 'maintenance', maintenance.id, `Maintenance updated: ${status}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'maintenance.updated', { maintenanceId: maintenance.id, status });
...
router.patch('/applications/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'application.updated', 'application', application.id, `Application ${status}: ${application.email}`);
        (0, pmWebhook_service_1.fireWebhooks)(profile.id, 'application.updated', { applicationId: application.id, status, email: application.email });
...
router.post('/webhooks', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'webhook.created', 'webhook', webhook.id, `Webhook created: ${url}`);
...
router.get('/webhooks', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.delete('/webhooks/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        (0, pmWebhook_service_1.logActivity)(profile.id, 'webhook.deleted', 'webhook', req.params.id, `Webhook deleted`);
...
router.get('/activity', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
// Documents
router.get('/documents', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/documents', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.delete('/documents/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/portal/:slug/apply', async (req, res) => {
    try {
...
//# sourceMappingURL=propertyManager.routes.js.map