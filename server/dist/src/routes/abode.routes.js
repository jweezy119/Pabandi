...
const abode_service_1 = require("../services/abode.service");
...
router.get('/revenue', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const summary = await abode_service_1.abodeRevenue.getRevenueSummary(userId, period);
...
router.get('/revenue/collection-rate', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        const rate = await abode_service_1.abodeRevenue.getRentCollectionRate(userId);
...
router.get('/revenue/top-properties', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        const properties = await abode_service_1.abodeRevenue.getTopProperties(userId);
...
router.get('/tenants', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        const tenants = await abode_service_1.abodeTenant.getTenants(userId);
...
router.get('/tenants/:id', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const tenant = await abode_service_1.abodeTenant.getTenantDetail(req.params.id);
        if (!tenant)
...
router.put('/tenants/:id/risk', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const tenant = await abode_service_1.abodeTenant.updateTenantRisk(req.params.id, riskBand);
        res.json({ success: true, data: tenant });
    }
    catch (e) {
...
router.get('/leases', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const leases = await abode_service_1.abodeLease.getLeases(userId, status);
...
router.post('/leases', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        const lease = await abode_service_1.abodeLease.createLease({ ...req.body, managerId: userId });
...
router.put('/leases/:id/renew', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const lease = await abode_service_1.abodeLease.renewLease(req.params.id, req.body);
...
router.put('/leases/:id/terminate', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const lease = await abode_service_1.abodeLease.terminateLease(req.params.id, reason);
        res.json({ success: true, data: lease });
    }
    catch (e) {
...
router.get('/maintenance', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const userId = req.user?.id;
        const status = req.query.status;
        const requests = await abode_service_1.abodeMaintenance.getRequests(userId, status);
...
router.post('/maintenance', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const request = await abode_service_1.abodeMaintenance.submitRequest(tenantId, data);
...
router.put('/maintenance/:id/status', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const request = await abode_service_1.abodeMaintenance.updateRequestStatus(req.params.id, status, notes);
...
router.post('/maintenance/:id/assign', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const request = await abode_service_1.abodeMaintenance.assignVendor(req.params.id, vendorId);
        res.json({ success: true, data: request });
    }
    catch (e) {
...
router.post('/messages', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const result = await abode_service_1.abodeCommunication.sendMessage(propertyId, senderEmail, recipientEmail, body);
...
router.post('/broadcast', auth_middleware_1.authenticate, async (req, res) => {
    try {
        ...
        const result = await abode_service_1.abodeCommunication.broadcastToTenants(propertyId, message);
...
router.get('/messages/:conversationId', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const messages = await abode_service_1.abodeCommunication.getMessageHistory(req.params.conversationId);
...
//# sourceMappingURL=abode.routes.js.map