...
const zkRealestateProver_service_1 = require("../services/zkRealestateProver.service");
...
router.post('/zk-proof', async (req, res) => {
    try {
        ...
        const proof = await zkRealestateProver_service_1.zkRealestateProver.prove({
        ...
        await database_1.prisma.trustAuditTrail.create({
            data: {
                ...
                metadata: { commitment: proof.commitment, publicInputs: proof.publicInputs, zkType: proof.zkType, anchor, issuedAt: proof.issuedAt },
            },
        }).catch((e) => logger_1.logger.warn(`[REAL-ESTATE-ZK] audit persist skipped: ${e.message}`));
...
router.get('/zk-proof/:proofId/verify', async (req, res) => {
    ...
    const stored = await database_1.prisma.trustAuditTrail.findFirst({
    ...
    const result = await zkRealestateProver_service_1.zkRealestateProver.verify({
