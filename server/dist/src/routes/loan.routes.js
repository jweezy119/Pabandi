 ...
 */
router.get('/power', auth_middleware_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.post('/:id/repay', auth_middleware_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.get('/reputation/quote', auth_middleware_1.authenticate, async (req, res) => {
    try {
 ...
 */
router.post('/reputation/request', auth_middleware_1.authenticate, async (req, res) => {
    try {
        const { usdcAmount } = req.body;
        if (!usdcAmount || usdcAmount <= 0)
            return res.status(400).json({ error: 'Valid USDC amount required' });
