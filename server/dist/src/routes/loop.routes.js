...
router.post('/heartbeat', async (_req, res) => {
    ...
    setTimeout(() => {
        Promise.resolve().then(() => __importStar(require('../services/renterEquity.service'))).then((m) => m.renterEquityService.settleAllPending().catch((e) => require('../utils/logger').logger.error(`[heartbeat] rent-stream settlement failed: ${e.message}`))).catch(() => { });
    }, 7000);
});
