 ...
 */
router.get('/feed/linkedin.xml', async (req, res) => {
    try {
        const jobs = await database_1.prisma.jobPosting.findMany({
...
router.post('/generate', async (req, res) => {
    ...
    const job = await database_1.prisma.jobPosting.create({
        data: {
...
router.get('/:jobId', jobs_controller_1.getJobDetails);
router.post('/:jobId/apply', auth_middleware_1.authenticate, jobs_controller_1.applyForJob);
