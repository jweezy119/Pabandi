...
async function getNoShowPredictor() {
    return (await Promise.resolve().then(() => __importStar(require('../services/ai/noShowPredictor')))).noShowPredictor;
}
...
const createReservation = async (req, res, next) => {
    try {
        ...
        // DISABLED: Transaction hash verification (Solana-only now, no ethers)
        // Get AI prediction — lazy-loaded for optional ML deps
        let prediction = { riskScore: 30, factors: {} };
        try {
            const predictor = await getNoShowPredictor();
...
const cancelReservation = async (req, res, next) => {
    try {
        ...
        if (['CANCELLED', 'COMPLETED', 'CHECKED_IN', 'NO_SHOW'].includes(reservation.status)) {
            throw new errorHandler_1.CustomError(`Reservation is already ${reservation.status.toLowerCase()}`, 400);
        }
