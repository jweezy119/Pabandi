...
const onramp_service_1 = require("../services/onramp.service");
const paylio_service_1 = require("../services/paylio.service");
...
const initiateCashAppCheckout = async (req, res) => {
    try {
        const { id } = req.params;
        const session = await database_1.prisma.checkoutSession.findUnique({
            where: { id },
        ...
        const link = await cashapp_service_1.cashAppService.createPaymentLink({
            amount: session.amount,
            currency: session.currency || 'USD',
        ...
        return (0, apiResponse_1.ok)(res, { url: link.url, gateway: 'cashapp', status: link.status }, 201);
    }
    catch (error) {
        logger_1.logger.error('Error initiating Cash App checkout', error);
        return (0, apiResponse_1.fail)(res, 'Internal server error', 500);
    }
};
exports.initiateCashAppCheckout = initiateCashAppCheckout;
const getOnrampQuotes = async (req, res) => {
    try {
        const { fiatAmount, fiatCurrency = 'USD', cryptoCurrency = 'USDC' } = req.query;
        if (!fiatAmount || isNaN(Number(fiatAmount))) {
            return (0, apiResponse_1.fail)(res, 'fiatAmount is required', 400);
        }
        const quotes = await onramp_service_1.onrampService.getQuotes({
            fiatAmount: Number(fiatAmount),
            fiatCurrency: fiatCurrency,
            cryptoCurrency,
        });
        return (0, apiResponse_1.ok)(res, { quotes });
    }
    catch (error) {
        logger_1.logger.error('Error fetching on-ramp quotes', error);
        return (0, apiResponse_1.fail)(res, 'Internal server error', 500);
    }
};
exports.getOnrampQuotes = getOnrampQuotes;
const createOnrampSession = async (req, res) => {
    try {
        ...
        if (!provider || !fiatAmount || !walletAddress) {
            return (0, apiResponse_1.fail)(res, 'provider, fiatAmount, and walletAddress are required', 400);
        }
        const reference = `onramp-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        const session = await onramp_service_1.onrampService.createSession({
            provider: provider,
            fiatAmount: Number(fiatAmount),
            fiatCurrency,
            cryptoCurrency,
        ...
        return (0, apiResponse_1.ok)(res, session, 201);
    }
    catch (error) {
        logger_1.logger.error('Error creating on-ramp session', error);
        return (0, apiResponse_1.fail)(res, error?.message || 'Internal server error', 500);
    }
};
...
const initiatePayLioCheckout = async (req, res) => {
    try {
        const { id } = req.params;
        const { walletAddress, email, note } = req.body;
        const session = await database_1.prisma.checkoutSession.findUnique({
            where: { id },
            include: { business: true },
        });
        ...
        const checkout = await paylio_service_1.paylioService.createCheckout({
        ...
        return (0, apiResponse_1.ok)(res, { url: checkout.url, gateway: 'paylio', status: checkout.status, ipnToken: checkout.ipnToken }, 201);
    }
    catch (error) {
        logger_1.logger.error('Error initiating PayLio checkout', error);
        return (0, apiResponse_1.fail)(res, 'Internal server error', 500);
    }
};
exports.initiatePayLioCheckout = initiatePayLioCheckout;
const paylioCallback = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (finalStatus !== session.status) {
            ...
            logger_1.logger.info(`[PayLio] callback for ${id}: ${status} -> ${finalStatus}`);
        }
        ...
        logger_1.logger.error('[PayLio] callback error:', error);
        return res.status(500).send('Callback processing failed');
    }
};
exports.paylioCallback = paylioCallback;
