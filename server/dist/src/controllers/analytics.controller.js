...
exports.getDetailedAnalytics = exports.getAnalytics = void 0;
...
const getAnalytics = async (req, res, next) => {
    try {
        ...
        const [totalReservations, upcomingReservations, recentNoShows, revenue, completedCount, noShowCount, cancelledCount, last30DaysReservations, last7DaysReservations, upcomingRisky, protectedRevenue, noShowByDay, noShowByHour,] = await Promise.all([
            ...
            noShowPredictor_1.noShowPredictor.getNoShowByDayOfWeek(business.id),
            noShowPredictor_1.noShowPredictor.getNoShowByHour(business.id),
        ]);
        // Calculate rates
    ...
    try {
        ...
        for (const r of recentReservations) {
            ...
            if (dailyMap[key]) {
                ...
                if (r.status === 'NO_SHOW')
                    dailyMap[key].noShows++;
                if (r.status === 'CANCELLED')
                    dailyMap[key].cancelled++;
                if (r.depositAmount)
                    dailyMap[key].depositTotal += r.depositAmount;
            }
        }
        ...
        for (const r of recentReservations) {
            ...
            if (r.status === 'NO_SHOW')
                customerMap[r.customerId].noShows++;
            if (r.status === 'COMPLETED')
                customerMap[r.customerId].completed++;
        }
