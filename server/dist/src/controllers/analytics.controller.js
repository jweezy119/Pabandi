 ...
 */
let _noShowPredictor = null;
async function getNoShowPredictor() {
    if (_noShowPredictor)
        return _noShowPredictor;
    const mod = await Promise.resolve().then(() => __importStar(require('../services/ai/noShowPredictor')));
...
const getAnalytics = async (req, res, next) => {
    try {
        ...
        const [totalReservations, upcomingReservations, recentNoShows, revenue, completedCount, noShowCount, cancelledCount, last30DaysReservations, last7DaysReservations, upcomingRisky, protectedRevenue,] = await Promise.all([
