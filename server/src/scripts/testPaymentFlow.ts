...
async function runTest() {
  ...
  let business = await prisma.business.findFirst({ where: { isActive: true } });
  if (!business) {
  ...
  // Verify reservation
  const reservation = await prisma.reservation.findUnique({ where: { id: bookingResult.reservationId } });
  console.log(`  Reservation status: ${reservation?.status}`);
  console.log(`  Deposit status: ${reservation?.depositStatus}`);

  // 5. Check-in
  console.log('\n--- Step 3: Check-In ---');
  const qrResult = await checkInService.generateCheckInToken(bookingResult.reservationId!);
  if (qrResult.success) {
    console.log(`  QR Code generated: ${qrResult.code}`);
  }

  const checkInResult = await checkInService.verifyCheckIn({
  ...
  console.log(`  Location verified: ${(checkInResult as any).locationVerified || false}`);
  ...
  // 7. Final state
  console.log('\n--- Final State ---');
  const finalReservation = await prisma.reservation.findUnique({ where: { id: bookingResult.reservationId } });
