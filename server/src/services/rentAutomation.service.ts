...
export const rentAutomationService = {
   ...
   */
  async runDailyRentAutomation(): Promise<RentAutomationRun> {
    const result: RentAutomationRun = { processed: 0, remindersSent: 0, lateFeesApplied: 0, errors: [] };
    ...
    try {
      ...
      for (const lease of activeLeases) {
        try {
          ...
          if (!tenantEmail || !managerEmail) continue;
          ...
          if (!existing) {
            ...
            try {
              ...
              logger.warn('[rent] reminder failed', e.message);
            }
          }
      ...
      logger.error('[rent] automation failed', e);
   ...
   */
  async markRentPaid(paymentId: string, method: string, reference?: string) {
    ...
    try {
      ...
      logger.warn('[rent] receipt failed', e.message);
    }
