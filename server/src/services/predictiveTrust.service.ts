...
export interface RiskProfile {
  ...
  noShow: number;
  cancelled: number;
  noShowRate: number; // shrunk posterior mean (0..1)
  completionRate: number; // 1 - noShowRate
  confidence: number; // 0..1, grows with sample size
}

function posteriorNoShow(completed: number, noShow: number, cancelled: number): number {
...
async function aggregateRisk(whereField: 'customerId' | 'businessId', id: string): Promise<RiskProfile> {
  const [completed, noShow, cancelled] = await Promise.all([
    prisma.reservation.count({ where: { [whereField]: id, status: 'COMPLETED' } }),
    prisma.reservation.count({ where: { [whereField]: id, status: 'NO_SHOW' } }),
    prisma.reservation.count({ where: { [whereField]: id, status: 'CANCELLED' } }),
  ]);
...
export const predictiveTrustService = {
  /** Per-customer no-show / completion risk from history. */
  async customerRisk(customerId: string): Promise<RiskProfile> {
    return aggregateRisk('customerId', customerId);
  },

  /** Per-business reliability from history. */
  async businessRisk(businessId: string): Promise<RiskProfile> {
  ...
  async predictBooking(input: {
    ...
    let wSum = 0;
    if (cust) { base += (cust.noShowRate - PRIOR_NO_SHOW) * cust.confidence; wSum += cust.confidence; }
    if (biz) { base += (biz.noShowRate - PRIOR_NO_SHOW) * biz.confidence; wSum += biz.confidence; }
    if (wSum > 0) base = PRIOR_NO_SHOW + (base - PRIOR_NO_SHOW); // already blended above
    ...
    if (reservationTime) {
      ...
      const leadHrs = (t - Date.now()) / 3_600_000;
   ...
   */
  async demandForecast(businessId: string): Promise<{ hour: number; count: number }[]> {
  ...
  async recommendSlots(input: {
    ...
    businessId: string;
    daysAhead?: number;
  }): Promise<{ slot: string; predictedCompletion: number; demand: number }[]> {
    ...
    const out: { slot: string; predictedCompletion: number; demand: number }[] = [];
    ...
    for (let d = 1; d <= daysAhead; d++) {
      ...
      for (const hour of [12, 18, 15]) {
        ...
        (out[out.length - 1] as any)._score = score;
      }
    }
    out.sort((a, b) => ((b as any)._score ?? 0) - ((a as any)._score ?? 0));
   ...
   */
  async attachPredictionToReservation(reservationId: string, input: {
    customerId?: string;
    businessId?: string;
    reservationTime?: string | Date;
  }) {
    try {
      ...
      logger.warn(`[PredictiveTrust] attach failed for ${reservationId}: ${e.message}`);
      return null;
    }
  },
};
