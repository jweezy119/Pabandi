...
import { pabTokenStakingService, SLASH_RATES } from './pabTokenStaking.service';
...
export interface DisputeEvidence {
  ...
  messages: { role: ArbitratorRole; text: string; timestamp: string }[];
...
export interface ArbitrationResult {
  ...
  pabReward?: { recipient: string; amount: number };
  pabSlash?: { target: string; amount: number; reason: string };
...
export class TrustArbitratorService {
  /**
   * Arbitrate a dispute using AI (Qwen) + contextual rules
   */
  async arbitrate(evidence: DisputeEvidence): Promise<ArbitrationResult> {
    try {
      ...
      logger.error('[TrustArbitrator] Arbitration failed:', error.message);
      return {
   ...
   */
  private applyRuleOverrides(evidence: DisputeEvidence): {
  ...
  private async finalizeRuleBased(
    rule: ReturnType<TrustArbitratorService['applyRuleOverrides']>,
   ...
   */
  private async analyzeWithQwen(evidence: DisputeEvidence): Promise<{
    ruling: Ruling;
    confidence: number;
    reasoning: string;
    factors: Record<string, number>;
...
`;
    ...
    try {
      const response = await axios.post(
        'https://dashscope-intl.alicloud.com/api/v1/services/aigc/text-generation/generation',
        {
          model: 'qwen-turbo',
          input: {
            messages: [
      ...
      logger.warn('[TrustArbitrator] Qwen API failed, falling back to heuristic:', error.message);
      return this.heuristicAnalysis(evidence);
    }
  }
   ...
   */
  private heuristicAnalysis(evidence: DisputeEvidence): {
    ruling: Ruling;
    confidence: number;
    reasoning: string;
    factors: Record<string, number>;
   ...
   */
  private async computePabIncentives(evidence: DisputeEvidence, ruling: Ruling): Promise<{ recipient: string; amount: number } | undefined> {
    if (ruling === 'NEEDS_MORE_INFO') return undefined;
    ...
    const { multiplier } = await pabTokenStakingService.getTrustMultiplier(winnerId);
   ...
   */
  private async computePabSlashing(evidence: DisputeEvidence, ruling: Ruling): Promise<{ target: string; amount: number; reason: string } | undefined> {
    if (ruling === 'NEEDS_MORE_INFO') return undefined;

    const loserId = ruling === 'BUYER_WINS' ? evidence.businessId : evidence.customerId;
    if (!loserId) return undefined;

    const { totalStaked } = await pabTokenStakingService.getTrustMultiplier(loserId);
...
export const trustArbitratorService = new TrustArbitratorService();
