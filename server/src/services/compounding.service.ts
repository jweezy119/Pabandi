...
export class CompoundingService {
  ...
  // ─── UPGRADE SETTINGS ──────────────────────────────────
  upgrade(params: { feeRate?: number; taskValue?: number }): void {
    if (params.feeRate) this.feeRate = params.feeRate;
    if (params.taskValue) this.taskValue = params.taskValue;
  ...
  async compound(): Promise<{ compounded: number; newReserve: number; newTaskValue: number }> {
    ...
    const newTaskValue = Math.min(
      MAX_TASK_VALUE,
      BASE_TASK_VALUE * (1 + Math.log10(this.reserve / 49.14))
    );
...
export const compoundingService = new CompoundingService(49.14);
