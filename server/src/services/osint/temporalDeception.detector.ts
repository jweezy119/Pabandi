...
export interface TemporalEvent {
  ...
  action: string;           // e.g. 'LOGIN', 'TRANSACTION', 'PAGE_VIEW'
...
export interface TemporalAnomaly {
  type: 'IMPOSSIBLE_TRAVEL' | 'TIMEZONE_PARADOX' | 'SESSION_VELOCITY_ABUSE' |
        'CIRCADIAN_ANOMALY' | 'TEMPORAL_COLLUSION' | 'TIMESTAMP_FORGERY';
...
export interface CircadianProfile {
  activityHistogram: number[];  // 24 elements, one per hour (normalized 0-1)
  sleepWindow: { startHour: number; endHour: number } | null;
  peakHours: number[];          // hours with highest activity
  is24x7Active: boolean;        // true = likely bot
  hasMultiplePeaks: boolean;    // true = possible account sharing
  entropy: number;              // Shannon entropy of activity distribution
}
...
export class TemporalDeceptionDetector {
  private eventBuffer: Map<string, TemporalEvent[]> = new Map();

  /**
   * Ingest a temporal event for a user. Events are buffered for batch analysis.
   */
  public recordEvent(event: TemporalEvent): void {
   ...
   */
  public async analyzeUser(userId: string): Promise<TemporalAnomalyReport> {
    logger.info(`[ChronoForensics] Analyzing temporal patterns for user ${userId}`);
    ...
    let overallRisk: TemporalAnomalyReport['overallRisk'] = 'CLEAN';
    if (criticalCount > 0 || highCount > 2) overallRisk = 'COMPROMISED';
    else if (highCount > 0 || anomalies.length > 3) overallRisk = 'SUSPICIOUS';
  ...
  private detectImpossibleTravel(events: TemporalEvent[]): TemporalAnomaly[] {
    const anomalies: TemporalAnomaly[] = [];
  ...
  private detectTimezoneParadox(events: TemporalEvent[]): TemporalAnomaly[] {
    const anomalies: TemporalAnomaly[] = [];
    ...
    const geoEvents = events.filter(e => e.geoLocation);
    if (geoEvents.length < 10) return anomalies;
  ...
  private detectSessionVelocityAbuse(events: TemporalEvent[]): TemporalAnomaly[] {
    const anomalies: TemporalAnomaly[] = [];

    // Group events by session
    const sessions = new Map<string, TemporalEvent[]>();
  ...
  private buildCircadianProfile(events: TemporalEvent[]): CircadianProfile {
    ...
    let sleepWindow: CircadianProfile['sleepWindow'] = null;
  ...
  private analyzeCircadian(profile: CircadianProfile): TemporalAnomaly[] {
    const anomalies: TemporalAnomaly[] = [];
  ...
  private async detectTemporalCollusion(
    userId: string,
    ...
    const txEvents = events.filter(e => 
      e.action === 'TRANSACTION' || e.action === 'BOOKING' || e.action === 'REVIEW'
    );
    if (txEvents.length < 3) return anomalies;
    ...
    for (const tx of txEvents) {
      ...
      if (counterpartyId) {
        if (!counterparties.has(counterpartyId)) counterparties.set(counterpartyId, []);
  ...
  private detectTimestampForgery(events: TemporalEvent[]): TemporalAnomaly[] {
    const anomalies: TemporalAnomaly[] = [];
  ...
  private shannonEntropy(distribution: number[]): number {
  ...
  private async loadEventsFromDB(userId: string): Promise<TemporalEvent[]> {
    try {
      const recentActivity = await prisma.trustAuditTrail.findMany({
        where: { userId },
...
export const temporalDeceptionDetector = new TemporalDeceptionDetector();
