...
export class WhatsAppBotService {
  ...
  async processMessage(phone: string, text: string): Promise<BotResponse> {
    ...
    try {
      ...
      logger.error(`[WhatsApp Bot] Error: ${error.message}`);
      return {
        success: false,
...
_Powered by Pabandi - پابانڈی_`,
   ...
   * Get user bookings
   */
  private async getMyBookings(phone: string): Promise<BotResponse> {
...
_کوئی بکنگ نہیں ملی_`,
    ...
    const bookingList = user.reservations
      .map((r: any, i: number) => `${i + 1}. ${r.business?.name || 'Unknown'} - ${r.status}`)
      .join('\n');
   ...
   * Get user payments
   */
  private async getMyPayments(phone: string): Promise<BotResponse> {
    const user = await prisma.user.findFirst({
      where: { phone },
      include: {
        userPayments: {
...
_کوئی ادائیگی نہیں ملی_`,
    ...
    const paymentList = user.userPayments
      .map((p: any, i: number) => `${i + 1}. PKR ${p.amount?.toLocaleString()} - ${p.status}`)
      .join('\n');
   ...
   */
  private async getInstallmentStatus(phone: string): Promise<BotResponse> {
    const user = await prisma.user.findFirst({
      where: { phone },
      include: {
        buyerProfile: {
...
_سمارٹ قسط منصوبہ_`,
   ...
   */
  private async searchBusinesses(category: string, city: string): Promise<BotResponse> {
...
_کوئی نتیجہ نہیں ملا_`,
    ...
    const businessList = businesses
      .slice(0, 5)
      .map((b: any, i: number) => `${i + 1}. *${b.name}* - ${b.address || b.city}`)
      .join('\n');
...
_مزید کے لیے کیٹگری تبدیل کریں_`,
   ...
   * Book a table
   */
  private async bookTable(phone: string, restaurantName: string, timeSpec: string): Promise<BotResponse> {
    const business = await prisma.business.findFirst({
      where: {
        name: { contains: restaurantName, mode: 'insensitive' as any },
        isActive: true,
      },
    });
...
_بکنگ کی درخواست موصول ہوئی_`,
   ...
   * Process payment
   */
  private async processPayment(phone: string, amount: number, businessName: string): Promise<BotResponse> {
    ...
    const business = await prisma.business.findFirst({
      where: {
        name: { contains: businessName, mode: 'insensitive' as any },
        isActive: true,
      },
    });
