...
import { jevDecision } from './jevDecision.service';
 ...
 */
export class JevSecurityService {
  ...
  async checkAgentSecurity(agentId: string, agentHistory: {
    ...
    try {
      ...
      const riskTier = (answers.risk_tier?.choice ?? 'medium') as 'low' | 'medium' | 'high';
...
export const jevSecurity = new JevSecurityService();
