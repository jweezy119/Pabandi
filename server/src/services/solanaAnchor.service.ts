...
export interface AnchorResult {
  artifactType: string;
  artifactHash: string;       // sha256 of the artifact payload
  signature: string;          // real Solana tx signature, or 'sim_<hash>' when degraded
  slot?: number;
  simulated: boolean;
  rpc?: string;
  anchoredAt: string;
...
export function hashArtifact(artifact: any): string {
  return createHash('sha256').update(JSON.stringify(artifact, Object.keys(artifact).sort())).digest('hex');
}
 ...
 */
export async function anchorOnSolana(artifactType: string, artifact: any, anchorSeed = 'PABANDI'): Promise<AnchorResult> {
  const artifactHash = hashArtifact(artifact);

  if (!hasRealChain) {
    // Deterministic simulated commitment — flagged, never presented as a real signature.
    const sig = `sim_${createHash('sha256').update(`${anchorSeed}:${artifactType}:${artifactHash}`).digest('hex').slice(0, 40)}`;
    logger.warn(`[SolanaAnchor] No anchor keypair configured — simulated commitment for ${artifactType} (${artifactHash.slice(0, 12)}…)`);
    return { artifactType, artifactHash, signature: sig, simulated: true, anchoredAt: new Date().toISOString() };
  }

  try {
    const { Connection, Keypair, Transaction, TransactionInstruction, sendAndConfirmTransaction, PublicKey } = await import('@solana/web3.js');
    ...
    logger.info(`[SolanaAnchor] Anchored ${artifactType} → ${signature} @ slot ${slot}`);
    return { artifactType, artifactHash, signature, slot, simulated: false, rpc: RPC_URL, anchoredAt: new Date().toISOString() };
  } catch (e: any) {
    logger.error(`[SolanaAnchor] on-chain anchor failed for ${artifactType}: ${e.message}`);
...
export const solanaAnchor = { anchorOnSolana, hashArtifact };
