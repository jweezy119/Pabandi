...
export class ChainlinkService {
   ...
   */
  public generateAppointmentCheckInAttestation(reservationId: string, userId: string, locationData: { lat: number; lng: number; radius: number }) {
    logger.info(`[Chainlink] Generating AppointmentCheckIn schema for Reservation ${reservationId}`);
   ...
   */
  public generateFreelanceMilestoneAttestation(escrowId: string, milestoneId: string, deliveryIpfsHash: string) {
    logger.info(`[Chainlink] Generating FreelanceMilestoneDelivery schema for Escrow ${escrowId}`);
   ...
   */
  public generateTrustScoreAttestation(userId: string, trustScore: number) {
    logger.info(`[Chainlink] Generating TrustScore schema for User ${userId}`);
