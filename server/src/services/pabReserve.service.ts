...
export class PabReserve {
  ...
  async creditAgent(params: {
    agentId: string;
    taskValue: number;
  }): Promise<{ usdcCredit: number; pabCredit: number }> {
   ...
   * Called once per day
   */
  async settleDaily(): Promise<SettlementResult> {
    const totalCredits = Array.from(this.agentCredits.values()).reduce(
      (sum, a) => sum + a.usdcCredit,
      0
    );

    const totalPab = Array.from(this.agentCredits.values()).reduce(
      (sum, a) => sum + a.pabCredit,
      0
    );
   ...
   */
  async runDailyCycle(batches: number, batchSize: number, taskValue: number): Promise<SettlementResult> {
  ...
  getReport(): ReserveReport {
    const totalOutstanding = Array.from(this.agentCredits.values()).reduce(
      (sum, a) => sum + a.usdcCredit,
      0
    );
...
export const pabReserve = new PabReserve(28);
