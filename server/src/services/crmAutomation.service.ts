...
export const crmAutomationService = {
  // ── Rent Generation ──────────────────────────────────────────────────────────
  async generateMonthlyRent(): Promise<{ generated: number; errors: string[] }> {
    ...
    try {
      ...
      logger.error('[crm] generateMonthlyRent failed', e);
  ...
  // ── Late Fee Engine ─────────────────────────────────────────────────────────
  async applyLateFees(): Promise<{ applied: number; totalAmount: number; errors: string[] }> {
    const result = { applied: 0, totalAmount: 0, errors: [] as string[] };
    try {
      const now = new Date();
      ...
      for (const payment of overduePayments) {
        try {
          const lease = await prisma.propertyLease.findFirst({
          ...
          if (now > graceEnd && lateFee > 0) {
            ...
            const existingFee = await prisma.tenantLedger.findFirst({
              where: {
                tenantId: lease ? (await prisma.propertyTenant.findFirst({ where: { email: payment.tenantEmail } }))?.id || '' : '',
                type: 'LATE_FEE',
            ...
            if (!existingFee) {
              ...
              const lateCount = await prisma.tenantLedger.count({
                where: {
                  tenantId: tenant?.id || '',
                  type: 'LATE_FEE',
      ...
      logger.error('[crm] applyLateFees failed', e);
      result.errors.push(`Global: ${e.message}`);
    }
    return result;
  },
  ...
  async checkLeaseExpirations(): Promise<{ notified: number; errors: string[] }> {
    ...
    try {
      ...
      logger.error('[crm] checkLeaseExpirations failed', e);
      result.errors.push(`Global: ${e.message}`);
    }
    return result;
  },
  ...
  async autoAssignVendor(maintenanceId: string): Promise<{ vendorId: string; score: number } | null> {
    try {
      const maintenance = await prisma.propertyMaintenance.findUnique({
      ...
      await prisma.propertyMaintenance.update({
        where: { id: maintenanceId },
      ...
      logger.error('[crm] autoAssignVendor failed', e);
      return null;
    }
  },

  // ── Financial Reports ────────────────────────────────────────────────────────
  async generatePropertyFinancials(propertyId: string, startDate: Date, endDate: Date) {
    ...
    const property = await prisma.propertyManagerProperty.findUnique({ where: { id: propertyId } });
  ...
  // ── Cash Flow Forecast ───────────────────────────────────────────────────────
  async generateCashFlowForecast(propertyId: string, months: number = 12) {
    const now = new Date();
    const activeLeases = await prisma.propertyLease.findMany({
      where: { propertyId, status: 'ACTIVE' },
    });

    const monthlyRent = activeLeases.reduce((s, l) => s + l.rentAmount, 0);
    const units = await prisma.propertyUnit.findMany({ where: { propertyId } });
  ...
  // ── Tenant Risk Scoring ──────────────────────────────────────────────────────
  async calculateTenantRiskScore(tenantId: string): Promise<RiskScoreResult> {
    ...
    if (!tenant) return { score: 0, band: 'HIGH', factors: [{ label: 'Tenant not found', impact: -100 }] };
    ...
    if (disputes > 0) factors.push({ label: `${disputes} disputes`, impact: -disputePenalty });

    // Maintenance request frequency
    const maintenanceCount = await prisma.propertyMaintenance.count({
      where: { tenantEmail: tenant.email },
    });
    const maintPenalty = maintenanceCount > 5 ? 10 : 0;
    score -= maintPenalty;
    if (maintPenalty > 0) factors.push({ label: 'High maintenance requests', impact: -maintPenalty });

    // Lease violations (notes-based)
    const leases = await prisma.propertyLease.findMany({ where: { tenantEmail: tenant.email } });
    const violationPenalty = leases.filter(l => l.notes?.toLowerCase().includes('violation')).length * 15;
    score -= violationPenalty;
    if (violationPenalty > 0) factors.push({ label: 'Lease violations', impact: -violationPenalty });

    score = Math.max(0, Math.min(100, score));
    const band: RiskScoreResult['band'] = score >= 70 ? 'LOW' : score >= 40 ? 'MEDIUM' : 'HIGH';
