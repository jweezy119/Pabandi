...
export type WalletBucket =
  | 'OPERATING'     // Available for agent funding
  | 'AGENT_ESCROW'  // Locked in active projects
  | 'PLATFORM_REV'  // Fees collected
  | 'YIELD'         // DeFi yield earned
  | 'RESERVE';      // Emergency reserve

// ─── Single Wallet Treasury ──────────────────────────────
export class SingleWalletTreasury {
  ...
  async fundWallet(params: {
    ...
    const record = await prisma.treasuryPosition.create({
      data: {
        bucket: 'OPERATING',
        amount: amountUsd,
        txHash: txHash || null,
    ...
    logger.info(`[Treasury] Wallet funded: $${amountUsd}`, { txHash, note });
  ...
  async disburseToAgent(params: {
    ...
    logger.info(`[Treasury] Disbursed $${disburseAmount.toFixed(4)} to agent (fee: $${platformFeeUsd.toFixed(4)})`, { agentId, projectId });
  ...
  async collectRevenue(params: {
    ...
    logger.info(`[Treasury] Revenue collected: $${amountUsd.toFixed(4)} from ${source}`, { referenceId });
  ...
  // ─── BALANCE QUERIES ─────────────────────────────────
  async getBucketBalance(bucket: WalletBucket): Promise<number> {
    const result = await prisma.treasuryPosition.aggregate({
  ...
  async getFullBreakdown(): Promise<{
    operating: number;
    agentEscrow: number;
  ...
  async recycleProfitsToOperating(params: {
    ...
    logger.info(`[Treasury] Recycled $${amountUsd.toFixed(4)} from revenue back to operating`);
...
export const singleWalletTreasury = new SingleWalletTreasury();
