...
export class MicroProfitEngine {
   ...
   */
  async runBatch(capitalUsd: number): Promise<BatchResult> {
  ...
  getOptimalBatchSize(capitalUsd: number): number {
    if (capitalUsd < 10) return 10;       // $10 → 10 tasks × $0.05 = $0.50
    if (capitalUsd < 25) return 20;       // $25 → 20 tasks × $0.05 = $1.00
    if (capitalUsd < 50) return 25;       // $50 → 25 tasks × $0.05 = $1.25
    if (capitalUsd < 100) return 30;      // $100 → 30 tasks × $0.05 = $1.50
...
export const microProfitEngine = new MicroProfitEngine();
