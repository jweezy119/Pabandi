...
export function computeInvestorProfile(
  investments: Array<{ poolId: string; amount: number; status: string; pool: Pool }>,
  ...
  const riskBandCount: Record<string, number> = {};
  ...
  for (const inv of activeInvestments) {
    ...
    if (pool?.city) regionSet.add(pool.city);
    if (pool?.state) regionSet.add(pool.state);
    if (pool?.expectedApy) totalApy += pool.expectedApy;
  }
  ...
  if (preferredRiskBands.length === 0) {
    ...
    if (userTrustScore >= 70) preferredRiskBands.push('MEDIUM', 'HIGH');
    else if (userTrustScore >= 40) preferredRiskBands.push('LOW', 'MEDIUM');
    else preferredRiskBands.push('LOW');
  }

  // Preferred regions
  const preferredRegions = userCity ? [userCity] : [...regionSet].slice(0, 3);
  if (preferredRegions.length === 0 && userCity) preferredRegions.push(userCity);
  ...
  // Investment style
  let investmentStyle: 'CONSERVATIVE' | 'MODERATE' | 'AGGRESSIVE' = 'MODERATE';
  if (riskToleranceScore < 30) investmentStyle = 'CONSERVATIVE';
  else if (riskToleranceScore > 70) investmentStyle = 'AGGRESSIVE';
 ...
 */
export function scorePoolForInvestor(pool: Pool, profile: InvestorProfile, investorCity?: string, investorCountry?: string): MatchResult {
  ...
  if (profile.preferredCategories.map(c => c.toUpperCase()).includes(poolCategory)) {
    ...
    const familyMatch = Object.entries(CATEGORY_FAMILIES).find(([, cats]) =>
      cats.includes(poolCategory) && profile.preferredCategories.some(pc => cats.includes(pc.toUpperCase()))
    );
    if (familyMatch) {
  ...
  if (profile.preferredRiskBands.map(r => r.toUpperCase()).includes(poolRisk)) {
    ...
    const poolIdx = RISK_BANDS.indexOf(poolRisk as RiskBand);
    const preferredIndices = profile.preferredRiskBands
      .map(b => RISK_BANDS.indexOf(b.toUpperCase() as RiskBand))
      .filter(i => i >= 0);
...
function poolMaxFromPool(pool: Pool): number | undefined {
  if (pool.maxInvestment) return pool.maxInvestment;
  // If no explicit max, use remaining capacity as soft max
  return pool.targetAmount - pool.currentAmount;
}

// ─── c. Recommend Pools for Investor ───────────────────────────────────────────
 ...
 */
export function recommendPoolsForInvestor(
  allPools: Pool[],
  profile: InvestorProfile,
  options: { limit?: number; investorCity?: string; investorCountry?: string; excludePoolIds?: string[] } = {}
): ScoredPool[] {
  const { limit = 10, investorCity, investorCountry, excludePoolIds = [] } = options;

  const eligiblePools = allPools.filter(p =>
    ['OPEN', 'ACTIVE', 'FULLY_SUBSCRIBED'].includes(p.status?.toUpperCase()) &&
...
// ─── d. Recommend Investors for Pool ───────────────────────────────────────────
 ...
 */
export function recommendInvestorsForPool(
  pool: Pool,
  investors: Array<{ userId: string; profile: InvestorProfile; city?: string; country?: string }>,
 ...
 */
export function generateMatchesForPool(
  pool: Pool,
  investors: Array<{ userId: string; profile: InvestorProfile; city?: string; country?: string }>,
  options: { limit?: number } = {}
): MatchRecord[] {
...
export function generateInsights(
  ...
  if (investorCity) {
    const cityPools = allPools.filter(p =>
      p.city?.toLowerCase() === investorCity.toLowerCase() &&
