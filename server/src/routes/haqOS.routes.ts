...
import { haqRevenue, haqTenant, haqLease, haqMaintenance, haqCommunication } from '../services/haqOS.service';
...
router.get('/revenue', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const summary = await haqRevenue.getRevenueSummary(userId, period);
...
router.get('/revenue/collection-rate', authenticate, async (req: any, res: Response) => {
  try {
    const userId = req.user?.id;
    const rate = await haqRevenue.getRentCollectionRate(userId);
...
router.get('/revenue/top-properties', authenticate, async (req: any, res: Response) => {
  try {
    const userId = req.user?.id;
    const properties = await haqRevenue.getTopProperties(userId);
...
router.get('/tenants', authenticate, async (req: any, res: Response) => {
  try {
    const userId = req.user?.id;
    const tenants = await haqTenant.getTenants(userId);
...
router.get('/tenants/:id', authenticate, async (req: any, res: Response) => {
  try {
    const tenant = await haqTenant.getTenantDetail(req.params.id);
...
router.put('/tenants/:id/risk', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const tenant = await haqTenant.updateTenantRisk(req.params.id, riskBand);
    res.json({ success: true, data: tenant });
...
router.get('/leases', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const leases = await haqLease.getLeases(userId, status);
...
router.post('/leases', authenticate, async (req: any, res: Response) => {
  try {
    const userId = req.user?.id;
    const lease = await haqLease.createLease({ ...req.body, managerId: userId });
...
router.put('/leases/:id/renew', authenticate, async (req: any, res: Response) => {
  try {
    const lease = await haqLease.renewLease(req.params.id, req.body);
...
router.put('/leases/:id/terminate', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const lease = await haqLease.terminateLease(req.params.id, reason);
    res.json({ success: true, data: lease });
...
router.get('/maintenance', authenticate, async (req: any, res: Response) => {
  try {
    const userId = req.user?.id;
    const status = req.query.status as string | undefined;
    const requests = await haqMaintenance.getRequests(userId, status);
...
router.post('/maintenance', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const request = await haqMaintenance.submitRequest(tenantId, data);
...
router.put('/maintenance/:id/status', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const request = await haqMaintenance.updateRequestStatus(req.params.id, status, notes);
...
router.post('/maintenance/:id/assign', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const request = await haqMaintenance.assignVendor(req.params.id, vendorId);
    res.json({ success: true, data: request });
...
router.post('/messages', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const result = await haqCommunication.sendMessage(propertyId, senderEmail, recipientEmail, body);
...
router.post('/broadcast', authenticate, async (req: any, res: Response) => {
  try {
    ...
    const result = await haqCommunication.broadcastToTenants(propertyId, message);
...
router.get('/messages/:conversationId', authenticate, async (req: any, res: Response) => {
  try {
    const messages = await haqCommunication.getMessageHistory(req.params.conversationId);
